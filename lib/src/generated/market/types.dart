// Auto-generated. DO NOT EDIT.

// ─── Enums ───────────────────────────────────────────────────

enum AgeVerified {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const AgeVerified(this.value);
  final String value;
}

enum AppId {
  v232090(232090),
  v252490(252490),
  v304930(304930),
  v322330(322330),
  v440(440),
  v570(570),
  v578080(578080),
  v730(730),
  v753(753);

  const AppId(this.value);
  final int value;
}

enum Autorenewal {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Autorenewal(this.value);
  final String value;
}

enum Bedrock {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Bedrock(this.value);
  final String value;
}

enum Billing {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Billing(this.value);
  final String value;
}

enum BirthdayAfterPeriod {
  day('day'),
  month('month'),
  year('year');

  const BirthdayAfterPeriod(this.value);
  final String value;
}

enum BirthdayPeriod {
  day('day'),
  month('month'),
  year('year');

  const BirthdayPeriod(this.value);
  final String value;
}

enum Bp {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Bp(this.value);
  final String value;
}

enum BrawlPass {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const BrawlPass(this.value);
  final String value;
}

enum CanChangeDetails {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const CanChangeDetails(this.value);
  final String value;
}

enum Cancel {
  v1(1);

  const Cancel(this.value);
  final int value;
}

enum CategoryId {
  v1(1),
  v10(10),
  v11(11),
  v12(12),
  v13(13),
  v14(14),
  v15(15),
  v16(16),
  v17(17),
  v18(18),
  v19(19),
  v20(20),
  v22(22),
  v24(24),
  v28(28),
  v3(3),
  v30(30),
  v31(31),
  v4(4),
  v5(5),
  v6(6),
  v7(7),
  v8(8),
  v9(9);

  const CategoryId(this.value);
  final int value;
}

enum ChangeEmail {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const ChangeEmail(this.value);
  final String value;
}

enum ChangeNickname {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const ChangeNickname(this.value);
  final String value;
}

enum ChangeableFn {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const ChangeableFn(this.value);
  final String value;
}

enum ClaimState {
  active('active'),
  rejected('rejected'),
  settled('settled'),
  solved('solved');

  const ClaimState(this.value);
  final String value;
}

enum Clan {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Clan(this.value);
  final String value;
}

enum Clans {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Clans(this.value);
  final String value;
}

enum ClashPass {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const ClashPass(this.value);
  final String value;
}

enum CookieLogin {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const CookieLogin(this.value);
  final String value;
}

enum Cookies {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Cookies(this.value);
  final String value;
}

enum Cs2MapRank {
  v1(1),
  v10(10),
  v11(11),
  v12(12),
  v13(13),
  v2(2),
  v3(3),
  v4(4),
  v5(5),
  v6(6),
  v7(7),
  v8(8),
  v9(9);

  const Cs2MapRank(this.value);
  final int value;
}

enum Currency {
  brl('brl'),
  byn('byn'),
  cny('cny'),
  eur('eur'),
  gbp('gbp'),
  jpy('jpy'),
  kzt('kzt'),
  rub('rub'),
  try$('try'),
  uah('uah'),
  usd('usd');

  const Currency(this.value);
  final String value;
}

enum D2LastMatchDatePeriod {
  day('day'),
  month('month'),
  year('year');

  const D2LastMatchDatePeriod(this.value);
  final String value;
}

enum Day {
  v0(0),
  v1(1),
  v10(10),
  v11(11),
  v12(12),
  v13(13),
  v14(14),
  v15(15),
  v16(16),
  v17(17),
  v18(18),
  v19(19),
  v2(2),
  v20(20),
  v21(21),
  v22(22),
  v23(23),
  v24(24),
  v25(25),
  v26(26),
  v27(27),
  v28(28),
  v3(3),
  v4(4),
  v5(5),
  v6(6),
  v7(7),
  v8(8),
  v9(9);

  const Day(this.value);
  final int value;
}

enum Dungeons {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Dungeons(this.value);
  final String value;
}

enum Ea {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Ea(this.value);
  final String value;
}

enum EditBtag {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const EditBtag(this.value);
  final String value;
}

enum Eg {
  vMinus1(-1),
  v0(0),
  v1(1),
  v2(2);

  const Eg(this.value);
  final int value;
}

enum Email {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Email(this.value);
  final String value;
}

enum EmailType {
  autoreg('autoreg'),
  native('native');

  const EmailType(this.value);
  final String value;
}

enum ExtendedGuarantee {
  vMinus1(-1),
  v0(0),
  v1(1);

  const ExtendedGuarantee(this.value);
  final int value;
}

enum Format {
  custom('custom'),
  mfaFileLogin('mfa_file_login'),
  mfaFileSteamId('mfa_file_steam_id'),
  short('short');

  const Format(this.value);
  final String value;
}

enum GameDonations {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const GameDonations(this.value);
  final String value;
}

enum Gifts {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Gifts(this.value);
  final String value;
}

enum HasActivatedKeys {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const HasActivatedKeys(this.value);
  final String value;
}

enum HasBan {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const HasBan(this.value);
  final String value;
}

enum HasFaceit {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const HasFaceit(this.value);
  final String value;
}

enum HoldLengthOption {
  day('day'),
  hour('hour'),
  month('month'),
  week('week'),
  year('year');

  const HoldLengthOption(this.value);
  final String value;
}

enum HypixelBan {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const HypixelBan(this.value);
  final String value;
}

enum HypixelBanParsed {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const HypixelBanParsed(this.value);
  final String value;
}

enum HypixelSkyblockApiEnabled {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const HypixelSkyblockApiEnabled(this.value);
  final String value;
}

enum ItemOrigin {
  autoreg('autoreg'),
  brute('brute'),
  dummy('dummy'),
  personal('personal'),
  phishing('phishing'),
  resale('resale'),
  retrieveViaSupport('retrieve_via_support'),
  selfRegistration('self_registration'),
  stealer('stealer');

  const ItemOrigin(this.value);
  final String value;
}

enum Java {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Java(this.value);
  final String value;
}

enum LastLoginHypixelPeriod {
  day('day'),
  month('month'),
  year('year');

  const LastLoginHypixelPeriod(this.value);
  final String value;
}

enum LastTransDatePeriod {
  day('day'),
  month('month'),
  year('year');

  const LastTransDatePeriod(this.value);
  final String value;
}

enum LastTransDatePeriodLater {
  day('day'),
  month('month'),
  year('year');

  const LastTransDatePeriodLater(this.value);
  final String value;
}

enum Legends {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Legends(this.value);
  final String value;
}

enum Limit {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Limit(this.value);
  final String value;
}

enum LoginWithoutCookies {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const LoginWithoutCookies(this.value);
  final String value;
}

enum Mafile {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Mafile(this.value);
  final String value;
}

enum MmBan {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const MmBan(this.value);
  final String value;
}

enum N2fa {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const N2fa(this.value);
  final String value;
}

enum Nitro {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Nitro(this.value);
  final String value;
}

enum NitroPeriod {
  day('day'),
  month('month'),
  year('year');

  const NitroPeriod(this.value);
  final String value;
}

enum NoBans {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const NoBans(this.value);
  final String value;
}

enum NotEmailProvider {
  firstmail('firstmail'),
  mailRu('mail_ru'),
  notletters('notletters'),
  other('other'),
  outlook('outlook'),
  rambler('rambler');

  const NotEmailProvider(this.value);
  final String value;
}

enum OrderBy {
  ddateToDown('ddate_to_down'),
  ddateToUp('ddate_to_up'),
  edateToDown('edate_to_down'),
  edateToUp('edate_to_up'),
  pdateToDown('pdate_to_down'),
  pdateToDownUpload('pdate_to_down_upload'),
  pdateToUp('pdate_to_up'),
  pdateToUpUpload('pdate_to_up_upload'),
  priceToDown('price_to_down'),
  priceToUp('price_to_up');

  const OrderBy(this.value);
  final String value;
}

enum Origin {
  autoreg('autoreg'),
  brute('brute'),
  dummy('dummy'),
  personal('personal'),
  phishing('phishing'),
  resale('resale'),
  retrieveViaSupport('retrieve_via_support'),
  selfRegistration('self_registration'),
  stealer('stealer');

  const Origin(this.value);
  final String value;
}

enum ParentControl {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const ParentControl(this.value);
  final String value;
}

enum Password {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Password(this.value);
  final String value;
}

enum Premium {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Premium(this.value);
  final String value;
}

enum PremiumExpirationPeriod {
  day('day'),
  month('month'),
  year('year');

  const PremiumExpirationPeriod(this.value);
  final String value;
}

enum PsnConnected {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const PsnConnected(this.value);
  final String value;
}

enum PsnLinkable {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const PsnLinkable(this.value);
  final String value;
}

enum Pve {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Pve(this.value);
  final String value;
}

enum R6Ban {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const R6Ban(this.value);
  final String value;
}

enum RealId {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const RealId(this.value);
  final String value;
}

enum RegPeriod {
  day('day'),
  month('month'),
  year('year');

  const RegPeriod(this.value);
  final String value;
}

enum Region {
  af('af'),
  as$('as'),
  cis('cis'),
  eu('eu'),
  me('me'),
  oc('oc'),
  us('us');

  const Region(this.value);
  final String value;
}

enum RoyalePass {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const RoyalePass(this.value);
  final String value;
}

enum Rt {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Rt(this.value);
  final String value;
}

enum Show {
  active('active'),
  awaiting('awaiting'),
  closed('closed'),
  deleted('deleted'),
  discountRequest('discount_request'),
  paid('paid'),
  preActive('pre_active'),
  stickied('stickied');

  const Show(this.value);
  final String value;
}

enum Side {
  bear('Bear'),
  savage('Savage');

  const Side(this.value);
  final String value;
}

enum Spam {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Spam(this.value);
  final String value;
}

enum Status {
  notPaid('not_paid'),
  paid('paid');

  const Status(this.value);
  final String value;
}

enum SteamConnected {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const SteamConnected(this.value);
  final String value;
}

enum Subscription {
  eAPlay('EA Play'),
  eAPlayPro('EA Play Pro'),
  pCGamePass('PC Game Pass'),
  robloxPremium1000('RobloxPremium1000'),
  robloxPremium100012Months('RobloxPremium100012Months'),
  robloxPremium1000OneMonth('RobloxPremium1000OneMonth'),
  robloxPremium2200('RobloxPremium2200'),
  robloxPremium2200OneMonth('RobloxPremium2200OneMonth'),
  robloxPremium450('RobloxPremium450'),
  robloxPremium450OneMonth('RobloxPremium450OneMonth'),
  xboxGamePassUltimate('Xbox Game Pass Ultimate'),
  basic('basic'),
  discordNitro('discord_nitro'),
  discordNitroBasic('discord_nitro_basic'),
  discordNitroTrial('discord_nitro_trial'),
  premium('premium'),
  premiumAnywhere('premiumAnywhere'),
  telegramPremium('telegram_premium');

  const Subscription(this.value);
  final String value;
}

enum SubscriptionPeriod {
  day('day'),
  month('month'),
  year('year');

  const SubscriptionPeriod(this.value);
  final String value;
}

enum Tel {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Tel(this.value);
  final String value;
}

enum TempEmail {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const TempEmail(this.value);
  final String value;
}

enum TradeBan {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const TradeBan(this.value);
  final String value;
}

enum TradeLimit {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const TradeLimit(this.value);
  final String value;
}

enum Transactions {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Transactions(this.value);
  final String value;
}

enum Type {
  agents('agents'),
  balanceExchange('balance_exchange'),
  buddies('buddies'),
  claimHold('claim_hold'),
  contest('contest'),
  dances('dances'),
  games('games'),
  gliders('gliders'),
  insuranceDeposit('insurance_deposit'),
  internalPurchase('internal_purchase'),
  invoice('invoice'),
  market('market'),
  moneyTransfer('money_transfer'),
  nomarket('nomarket'),
  paidItem('paid_item'),
  paidMail('paid_mail'),
  pickaxes('pickaxes'),
  profiles('profiles'),
  receivingMoney('receiving_money'),
  refilledBalance('refilled_balance'),
  skins('skins'),
  socialclub('socialclub'),
  soldItem('sold_item'),
  weapons('weapons'),
  withdrawalBalance('withdrawal_balance');

  const Type(this.value);
  final String value;
}

enum Verified {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Verified(this.value);
  final String value;
}

enum Voice {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const Voice(this.value);
  final String value;
}

enum XboxConnected {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const XboxConnected(this.value);
  final String value;
}

enum XboxLinkable {
  no('no'),
  nomatter('nomatter'),
  yes('yes');

  const XboxLinkable(this.value);
  final String value;
}

// ─── Component Schemas ────────────────────────────────────────

class DiscountModel {
  final num categoryId;
  final num discountId;
  final num discountPercent;
  final num discountUserId;
  final num maxPrice;
  final num minPrice;
  final num userId;

  const DiscountModel({
    required this.categoryId,
    required this.discountId,
    required this.discountPercent,
    required this.discountUserId,
    required this.maxPrice,
    required this.minPrice,
    required this.userId,
  });

  factory DiscountModel.fromJson(Map<String, dynamic> json) => DiscountModel(
    categoryId: json['category_id'] is num ? json['category_id'] as num : 0,
    discountId: json['discount_id'] is num ? json['discount_id'] as num : 0,
    discountPercent: json['discount_percent'] is num ? json['discount_percent'] as num : 0,
    discountUserId: json['discount_user_id'] is num ? json['discount_user_id'] as num : 0,
    maxPrice: json['max_price'] is num ? json['max_price'] as num : 0,
    minPrice: json['min_price'] is num ? json['min_price'] as num : 0,
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
  );
}

class UserModelCustomFields {
  final String n4;
  final List<dynamic> allowSelfUnban;
  final String banReason;
  final String discord;
  final String github;
  final String jabber;
  final String lztAwardUserTrophy;
  final String lztLikesIncreasing;
  final String lztLikesZeroing;
  final String lztSympathyIncreasing;
  final String lztSympathyZeroing;
  final String lztUnbanAmount;
  final String maecenasValue;
  final String scamURL;
  final String steam;
  final String telegram;
  final String vk;
  final String favoritePorn;
  final String favoriteVape;
  final String favoriteAnime;
  final String matrix;

  const UserModelCustomFields({
    required this.n4,
    required this.allowSelfUnban,
    required this.banReason,
    required this.discord,
    required this.github,
    required this.jabber,
    required this.lztAwardUserTrophy,
    required this.lztLikesIncreasing,
    required this.lztLikesZeroing,
    required this.lztSympathyIncreasing,
    required this.lztSympathyZeroing,
    required this.lztUnbanAmount,
    required this.maecenasValue,
    required this.scamURL,
    required this.steam,
    required this.telegram,
    required this.vk,
    required this.favoritePorn,
    required this.favoriteVape,
    required this.favoriteAnime,
    required this.matrix,
  });

  factory UserModelCustomFields.fromJson(Map<String, dynamic> json) => UserModelCustomFields(
    n4: json['_4'] is String ? json['_4'] as String : '',
    allowSelfUnban: json['allowSelfUnban'] is List ? json['allowSelfUnban'] as List<dynamic> : const [],
    banReason: json['ban_reason'] is String ? json['ban_reason'] as String : '',
    discord: json['discord'] is String ? json['discord'] as String : '',
    github: json['github'] is String ? json['github'] as String : '',
    jabber: json['jabber'] is String ? json['jabber'] as String : '',
    lztAwardUserTrophy: json['lztAwardUserTrophy'] is String ? json['lztAwardUserTrophy'] as String : '',
    lztLikesIncreasing: json['lztLikesIncreasing'] is String ? json['lztLikesIncreasing'] as String : '',
    lztLikesZeroing: json['lztLikesZeroing'] is String ? json['lztLikesZeroing'] as String : '',
    lztSympathyIncreasing: json['lztSympathyIncreasing'] is String ? json['lztSympathyIncreasing'] as String : '',
    lztSympathyZeroing: json['lztSympathyZeroing'] is String ? json['lztSympathyZeroing'] as String : '',
    lztUnbanAmount: json['lztUnbanAmount'] is String ? json['lztUnbanAmount'] as String : '',
    maecenasValue: json['maecenasValue'] is String ? json['maecenasValue'] as String : '',
    scamURL: json['scamURL'] is String ? json['scamURL'] as String : '',
    steam: json['steam'] is String ? json['steam'] as String : '',
    telegram: json['telegram'] is String ? json['telegram'] as String : '',
    vk: json['vk'] is String ? json['vk'] as String : '',
    favoritePorn: json['favoritePorn'] is String ? json['favoritePorn'] as String : '',
    favoriteVape: json['favoriteVape'] is String ? json['favoriteVape'] as String : '',
    favoriteAnime: json['favoriteAnime'] is String ? json['favoriteAnime'] as String : '',
    matrix: json['matrix'] is String ? json['matrix'] as String : '',
  );
}

class UserModelDob {
  final num year;
  final num month;
  final num day;

  const UserModelDob({
    required this.year,
    required this.month,
    required this.day,
  });

  factory UserModelDob.fromJson(Map<String, dynamic> json) => UserModelDob(
    year: json['year'] is num ? json['year'] as num : 0,
    month: json['month'] is num ? json['month'] as num : 0,
    day: json['day'] is num ? json['day'] as num : 0,
  );
}

class UserModelImapDataDomainZone {
  final String domain;
  final String imapServer;
  final num port;
  final bool secure;

  const UserModelImapDataDomainZone({
    required this.domain,
    required this.imapServer,
    required this.port,
    required this.secure,
  });

  factory UserModelImapDataDomainZone.fromJson(Map<String, dynamic> json) => UserModelImapDataDomainZone(
    domain: json['domain'] is String ? json['domain'] as String : '',
    imapServer: json['imap_server'] is String ? json['imap_server'] as String : '',
    port: json['port'] is num ? json['port'] as num : 0,
    secure: json['secure'] is bool ? json['secure'] as bool : false,
  );
}

class UserModelImapData {
  final UserModelImapDataDomainZone domainZone;

  const UserModelImapData({
    required this.domainZone,
  });

  factory UserModelImapData.fromJson(Map<String, dynamic> json) => UserModelImapData(
    domainZone: json['domain.zone'] is Map<String, dynamic>
            ? UserModelImapDataDomainZone.fromJson(json['domain.zone'] as Map<String, dynamic>)
            : UserModelImapDataDomainZone.fromJson(const {}),
  );
}

class UserModelRenderedAvatars {
  final String l;
  final String m;
  final String s;

  const UserModelRenderedAvatars({
    required this.l,
    required this.m,
    required this.s,
  });

  factory UserModelRenderedAvatars.fromJson(Map<String, dynamic> json) => UserModelRenderedAvatars(
    l: json['l'] is String ? json['l'] as String : '',
    m: json['m'] is String ? json['m'] as String : '',
    s: json['s'] is String ? json['s'] as String : '',
  );
}

class UserModelRenderedBackgrounds {
  final String l;
  final String m;

  const UserModelRenderedBackgrounds({
    required this.l,
    required this.m,
  });

  factory UserModelRenderedBackgrounds.fromJson(Map<String, dynamic> json) => UserModelRenderedBackgrounds(
    l: json['l'] is String ? json['l'] as String : '',
    m: json['m'] is String ? json['m'] as String : '',
  );
}

class UserModelRendered {
  final String username;
  final UserModelRenderedAvatars avatars;
  final UserModelRenderedBackgrounds backgrounds;
  final String link;

  const UserModelRendered({
    required this.username,
    required this.avatars,
    required this.backgrounds,
    required this.link,
  });

  factory UserModelRendered.fromJson(Map<String, dynamic> json) => UserModelRendered(
    username: json['username'] is String ? json['username'] as String : '',
    avatars: json['avatars'] is Map<String, dynamic>
            ? UserModelRenderedAvatars.fromJson(json['avatars'] as Map<String, dynamic>)
            : UserModelRenderedAvatars.fromJson(const {}),
    backgrounds: json['backgrounds'] is Map<String, dynamic>
            ? UserModelRenderedBackgrounds.fromJson(json['backgrounds'] as Map<String, dynamic>)
            : UserModelRenderedBackgrounds.fromJson(const {}),
    link: json['link'] is String ? json['link'] as String : '',
  );
}

class UserModelTelegramClient {
  final String telegramApiId;
  final String telegramApiHash;
  final String telegramDeviceModel;
  final String telegramSystemVersion;
  final String telegramAppVersion;
  final String telegramSystemLangCode;
  final String telegramLangCode;
  final String telegramLangPack;

  const UserModelTelegramClient({
    required this.telegramApiId,
    required this.telegramApiHash,
    required this.telegramDeviceModel,
    required this.telegramSystemVersion,
    required this.telegramAppVersion,
    required this.telegramSystemLangCode,
    required this.telegramLangCode,
    required this.telegramLangPack,
  });

  factory UserModelTelegramClient.fromJson(Map<String, dynamic> json) => UserModelTelegramClient(
    telegramApiId: json['telegram_api_id'] is String ? json['telegram_api_id'] as String : '',
    telegramApiHash: json['telegram_api_hash'] is String ? json['telegram_api_hash'] as String : '',
    telegramDeviceModel: json['telegram_device_model'] is String ? json['telegram_device_model'] as String : '',
    telegramSystemVersion: json['telegram_system_version'] is String ? json['telegram_system_version'] as String : '',
    telegramAppVersion: json['telegram_app_version'] is String ? json['telegram_app_version'] as String : '',
    telegramSystemLangCode: json['telegram_system_lang_code'] is String ? json['telegram_system_lang_code'] as String : '',
    telegramLangCode: json['telegram_lang_code'] is String ? json['telegram_lang_code'] as String : '',
    telegramLangPack: json['telegram_lang_pack'] is String ? json['telegram_lang_pack'] as String : '',
  );
}

class UserModel {
  final num activeItemsCount;
  final bool activityVisible;
  final num age;
  final String balance;
  final List<Map<String, dynamic>> balances;
  final num bumpItemPeriod;
  final bool canEdit;
  final bool canFollow;
  final bool canIgnore;
  final bool canPostProfile;
  final bool canViewProfile;
  final bool canViewProfilePosts;
  final bool canWarn;
  final num contestCount;
  final String convWelcomeMessage;
  final num convertedBalance;
  final num convertedDeposit;
  final num convertedHold;
  final String currency;
  final String currencyPhrase;
  final String customAccountDownloadFormat;
  final UserModelCustomFields customFields;
  final String customTitle;
  final num deposit;
  final UserModelDob dob;
  final dynamic feedbackData;
  final String hold;
  final String homepage;
  final UserModelImapData imapData;
  final bool isAdmin;
  final bool isBanned;
  final bool isFollowed;
  final bool isIgnored;
  final bool isModerator;
  final bool isStaff;
  final bool isSuperAdmin;
  final num joinedDate;
  final num lastActivity;
  final num like2Count;
  final num likeCount;
  final String location;
  final String marketCustomTitle;
  final num maxDiscountPercent;
  final num messageCount;
  final num paidMailLeft;
  final List<Map<String, dynamic>> publicTags;
  final num registerDate;
  final UserModelRendered rendered;
  final num restoreCount;
  final dynamic restoreData;
  final String shortLink;
  final num soldItemsCount;
  final List<Map<String, dynamic>> tags;
  final UserModelTelegramClient telegramClient;
  final num trophyPoints;
  final bool userAllowAskDiscount;
  final num userId;
  final String userTitle;
  final String username;
  final String viewUrl;
  final bool visible;
  final num warningPoints;

  const UserModel({
    required this.activeItemsCount,
    required this.activityVisible,
    required this.age,
    required this.balance,
    required this.balances,
    required this.bumpItemPeriod,
    required this.canEdit,
    required this.canFollow,
    required this.canIgnore,
    required this.canPostProfile,
    required this.canViewProfile,
    required this.canViewProfilePosts,
    required this.canWarn,
    required this.contestCount,
    required this.convWelcomeMessage,
    required this.convertedBalance,
    required this.convertedDeposit,
    required this.convertedHold,
    required this.currency,
    required this.currencyPhrase,
    required this.customAccountDownloadFormat,
    required this.customFields,
    required this.customTitle,
    required this.deposit,
    required this.dob,
    required this.feedbackData,
    required this.hold,
    required this.homepage,
    required this.imapData,
    required this.isAdmin,
    required this.isBanned,
    required this.isFollowed,
    required this.isIgnored,
    required this.isModerator,
    required this.isStaff,
    required this.isSuperAdmin,
    required this.joinedDate,
    required this.lastActivity,
    required this.like2Count,
    required this.likeCount,
    required this.location,
    required this.marketCustomTitle,
    required this.maxDiscountPercent,
    required this.messageCount,
    required this.paidMailLeft,
    required this.publicTags,
    required this.registerDate,
    required this.rendered,
    required this.restoreCount,
    required this.restoreData,
    required this.shortLink,
    required this.soldItemsCount,
    required this.tags,
    required this.telegramClient,
    required this.trophyPoints,
    required this.userAllowAskDiscount,
    required this.userId,
    required this.userTitle,
    required this.username,
    required this.viewUrl,
    required this.visible,
    required this.warningPoints,
  });

  factory UserModel.fromJson(Map<String, dynamic> json) => UserModel(
    activeItemsCount: json['active_items_count'] is num ? json['active_items_count'] as num : 0,
    activityVisible: json['activity_visible'] is bool ? json['activity_visible'] as bool : false,
    age: json['age'] is num ? json['age'] as num : 0,
    balance: json['balance'] is String ? json['balance'] as String : '',
    balances: json['balances'] is List ? (json['balances'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    bumpItemPeriod: json['bump_item_period'] is num ? json['bump_item_period'] as num : 0,
    canEdit: json['can_edit'] is bool ? json['can_edit'] as bool : false,
    canFollow: json['can_follow'] is bool ? json['can_follow'] as bool : false,
    canIgnore: json['can_ignore'] is bool ? json['can_ignore'] as bool : false,
    canPostProfile: json['can_post_profile'] is bool ? json['can_post_profile'] as bool : false,
    canViewProfile: json['can_view_profile'] is bool ? json['can_view_profile'] as bool : false,
    canViewProfilePosts: json['can_view_profile_posts'] is bool ? json['can_view_profile_posts'] as bool : false,
    canWarn: json['can_warn'] is bool ? json['can_warn'] as bool : false,
    contestCount: json['contest_count'] is num ? json['contest_count'] as num : 0,
    convWelcomeMessage: json['conv_welcome_message'] is String ? json['conv_welcome_message'] as String : '',
    convertedBalance: json['convertedBalance'] is num ? json['convertedBalance'] as num : 0,
    convertedDeposit: json['convertedDeposit'] is num ? json['convertedDeposit'] as num : 0,
    convertedHold: json['convertedHold'] is num ? json['convertedHold'] as num : 0,
    currency: json['currency'] is String ? json['currency'] as String : '',
    currencyPhrase: json['currencyPhrase'] is String ? json['currencyPhrase'] as String : '',
    customAccountDownloadFormat: json['custom_account_download_format'] is String ? json['custom_account_download_format'] as String : '',
    customFields: json['custom_fields'] is Map<String, dynamic>
            ? UserModelCustomFields.fromJson(json['custom_fields'] as Map<String, dynamic>)
            : UserModelCustomFields.fromJson(const {}),
    customTitle: json['custom_title'] is String ? json['custom_title'] as String : '',
    deposit: json['deposit'] is num ? json['deposit'] as num : 0,
    dob: json['dob'] is Map<String, dynamic>
            ? UserModelDob.fromJson(json['dob'] as Map<String, dynamic>)
            : UserModelDob.fromJson(const {}),
    feedbackData: json['feedback_data'],
    hold: json['hold'] is String ? json['hold'] as String : '',
    homepage: json['homepage'] is String ? json['homepage'] as String : '',
    imapData: json['imap_data'] is Map<String, dynamic>
            ? UserModelImapData.fromJson(json['imap_data'] as Map<String, dynamic>)
            : UserModelImapData.fromJson(const {}),
    isAdmin: json['is_admin'] is bool ? json['is_admin'] as bool : false,
    isBanned: json['is_banned'] is bool ? json['is_banned'] as bool : false,
    isFollowed: json['is_followed'] is bool ? json['is_followed'] as bool : false,
    isIgnored: json['is_ignored'] is bool ? json['is_ignored'] as bool : false,
    isModerator: json['is_moderator'] is bool ? json['is_moderator'] as bool : false,
    isStaff: json['is_staff'] is bool ? json['is_staff'] as bool : false,
    isSuperAdmin: json['is_super_admin'] is bool ? json['is_super_admin'] as bool : false,
    joinedDate: json['joined_date'] is num ? json['joined_date'] as num : 0,
    lastActivity: json['last_activity'] is num ? json['last_activity'] as num : 0,
    like2Count: json['like2_count'] is num ? json['like2_count'] as num : 0,
    likeCount: json['like_count'] is num ? json['like_count'] as num : 0,
    location: json['location'] is String ? json['location'] as String : '',
    marketCustomTitle: json['market_custom_title'] is String ? json['market_custom_title'] as String : '',
    maxDiscountPercent: json['max_discount_percent'] is num ? json['max_discount_percent'] as num : 0,
    messageCount: json['message_count'] is num ? json['message_count'] as num : 0,
    paidMailLeft: json['paid_mail_left'] is num ? json['paid_mail_left'] as num : 0,
    publicTags: json['public_tags'] is List ? (json['public_tags'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    registerDate: json['register_date'] is num ? json['register_date'] as num : 0,
    rendered: json['rendered'] is Map<String, dynamic>
            ? UserModelRendered.fromJson(json['rendered'] as Map<String, dynamic>)
            : UserModelRendered.fromJson(const {}),
    restoreCount: json['restore_count'] is num ? json['restore_count'] as num : 0,
    restoreData: json['restore_data'],
    shortLink: json['short_link'] is String ? json['short_link'] as String : '',
    soldItemsCount: json['sold_items_count'] is num ? json['sold_items_count'] as num : 0,
    tags: json['tags'] is List ? (json['tags'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    telegramClient: json['telegram_client'] is Map<String, dynamic>
            ? UserModelTelegramClient.fromJson(json['telegram_client'] as Map<String, dynamic>)
            : UserModelTelegramClient.fromJson(const {}),
    trophyPoints: json['trophy_points'] is num ? json['trophy_points'] as num : 0,
    userAllowAskDiscount: json['user_allow_ask_discount'] is bool ? json['user_allow_ask_discount'] as bool : false,
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
    userTitle: json['user_title'] is String ? json['user_title'] as String : '',
    username: json['username'] is String ? json['username'] as String : '',
    viewUrl: json['view_url'] is String ? json['view_url'] as String : '',
    visible: json['visible'] is bool ? json['visible'] as bool : false,
    warningPoints: json['warning_points'] is num ? json['warning_points'] as num : 0,
  );
}

class BalanceModel {
  final String balance;
  final num balanceId;
  final dynamic customTitle;
  final String fullTitle;
  final num merchantId;
  final String title;
  final String type;
  final num userId;

  const BalanceModel({
    required this.balance,
    required this.balanceId,
    required this.customTitle,
    required this.fullTitle,
    required this.merchantId,
    required this.title,
    required this.type,
    required this.userId,
  });

  factory BalanceModel.fromJson(Map<String, dynamic> json) => BalanceModel(
    balance: json['balance'] is String ? json['balance'] as String : '',
    balanceId: json['balance_id'] is num ? json['balance_id'] as num : 0,
    customTitle: json['custom_title'],
    fullTitle: json['fullTitle'] is String ? json['fullTitle'] as String : '',
    merchantId: json['merchant_id'] is num ? json['merchant_id'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
    type: json['type'] is String ? json['type'] as String : '',
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
  );
}

class ExtraModel {
  final String? proxy;
  final bool? closeItem;
  final String? region;
  final String? service;
  final String? system;
  final String? confirmationCode;
  final String? cookies;
  final bool? loginWithoutCookies;
  final bool? cookieLogin;
  final String? mfaFile;
  final num? dota2Mmr;
  final bool? eaGames;
  final bool? uplayGames;
  final bool? theQuarry;
  final bool? warframe;
  final bool? ark;
  final bool? arkAscended;
  final num? genshinCurrency;
  final num? honkaiCurrency;
  final num? zenlessCurrency;
  final String? password;
  final String? telegramClient;
  final String? telegramJson;
  final bool? checkChannels;
  final bool? checkSpam;
  final bool? checkHypixelBan;

  const ExtraModel({
    this.proxy,
    this.closeItem,
    this.region,
    this.service,
    this.system,
    this.confirmationCode,
    this.cookies,
    this.loginWithoutCookies,
    this.cookieLogin,
    this.mfaFile,
    this.dota2Mmr,
    this.eaGames,
    this.uplayGames,
    this.theQuarry,
    this.warframe,
    this.ark,
    this.arkAscended,
    this.genshinCurrency,
    this.honkaiCurrency,
    this.zenlessCurrency,
    this.password,
    this.telegramClient,
    this.telegramJson,
    this.checkChannels,
    this.checkSpam,
    this.checkHypixelBan,
  });

  factory ExtraModel.fromJson(Map<String, dynamic> json) => ExtraModel(
    proxy: json['proxy'] is String ? json['proxy'] as String : null,
    closeItem: json['close_item'] is bool ? json['close_item'] as bool : null,
    region: json['region'] is String ? json['region'] as String : null,
    service: json['service'] is String ? json['service'] as String : null,
    system: json['system'] is String ? json['system'] as String : null,
    confirmationCode: json['confirmationCode'] is String ? json['confirmationCode'] as String : null,
    cookies: json['cookies'] is String ? json['cookies'] as String : null,
    loginWithoutCookies: json['login_without_cookies'] is bool ? json['login_without_cookies'] as bool : null,
    cookieLogin: json['cookie_login'] is bool ? json['cookie_login'] as bool : null,
    mfaFile: json['mfa_file'] is String ? json['mfa_file'] as String : null,
    dota2Mmr: json['dota2_mmr'] is num ? json['dota2_mmr'] as num : null,
    eaGames: json['ea_games'] is bool ? json['ea_games'] as bool : null,
    uplayGames: json['uplay_games'] is bool ? json['uplay_games'] as bool : null,
    theQuarry: json['the_quarry'] is bool ? json['the_quarry'] as bool : null,
    warframe: json['warframe'] is bool ? json['warframe'] as bool : null,
    ark: json['ark'] is bool ? json['ark'] as bool : null,
    arkAscended: json['ark_ascended'] is bool ? json['ark_ascended'] as bool : null,
    genshinCurrency: json['genshin_currency'] is num ? json['genshin_currency'] as num : null,
    honkaiCurrency: json['honkai_currency'] is num ? json['honkai_currency'] as num : null,
    zenlessCurrency: json['zenless_currency'] is num ? json['zenless_currency'] as num : null,
    password: json['password'] is String ? json['password'] as String : null,
    telegramClient: json['telegramClient'] is String ? json['telegramClient'] as String : null,
    telegramJson: json['telegramJson'] is String ? json['telegramJson'] as String : null,
    checkChannels: json['checkChannels'] is bool ? json['checkChannels'] as bool : null,
    checkSpam: json['checkSpam'] is bool ? json['checkSpam'] as bool : null,
    checkHypixelBan: json['checkHypixelBan'] is bool ? json['checkHypixelBan'] as bool : null,
  );
}

class ConfirmationCodeModelCodeData {
  final String code;
  final num date;
  final String textPlain;

  const ConfirmationCodeModelCodeData({
    required this.code,
    required this.date,
    required this.textPlain,
  });

  factory ConfirmationCodeModelCodeData.fromJson(Map<String, dynamic> json) => ConfirmationCodeModelCodeData(
    code: json['code'] is String ? json['code'] as String : '',
    date: json['date'] is num ? json['date'] as num : 0,
    textPlain: json['textPlain'] is String ? json['textPlain'] as String : '',
  );
}

class ConfirmationCodeModel {
  final ItemModel item;
  final ConfirmationCodeModelCodeData codeData;

  const ConfirmationCodeModel({
    required this.item,
    required this.codeData,
  });

  factory ConfirmationCodeModel.fromJson(Map<String, dynamic> json) => ConfirmationCodeModel(
    item: json['item'] is Map<String, dynamic>
            ? ItemModel.fromJson(json['item'] as Map<String, dynamic>)
            : ItemModel.fromJson(const {}),
    codeData: json['codeData'] is Map<String, dynamic>
            ? ConfirmationCodeModelCodeData.fromJson(json['codeData'] as Map<String, dynamic>)
            : ConfirmationCodeModelCodeData.fromJson(const {}),
  );
}

class ItemListModel {
  final List<ItemFromListModel> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final String searchUrl;
  final List<ItemFromListModel> stickyItems;
  final RespSystemInfo systemInfo;

  const ItemListModel({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory ItemListModel.fromJson(Map<String, dynamic> json) => ItemListModel(
    items: json['items'] is List
            ? (json['items'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List
            ? (json['stickyItems'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ItemFromListModelBumpSettings {
  final bool? canBumpItem;
  final bool? canBumpItemGlobally;
  final String? shortErrorPhrase;
  final String? errorPhrase;

  const ItemFromListModelBumpSettings({
    this.canBumpItem,
    this.canBumpItemGlobally,
    this.shortErrorPhrase,
    this.errorPhrase,
  });

  factory ItemFromListModelBumpSettings.fromJson(Map<String, dynamic> json) => ItemFromListModelBumpSettings(
    canBumpItem: json['canBumpItem'] is bool ? json['canBumpItem'] as bool : null,
    canBumpItemGlobally: json['canBumpItemGlobally'] is bool ? json['canBumpItemGlobally'] as bool : null,
    shortErrorPhrase: json['shortErrorPhrase'] is String ? json['shortErrorPhrase'] as String : null,
    errorPhrase: json['errorPhrase'] is String ? json['errorPhrase'] as String : null,
  );
}

class ItemFromListModelSeller {
  final num? userId;
  final num? soldItemsCount;
  final num? activeItemCount;
  final String? restoreData;
  final String? username;
  final num? avatarDate;
  final num? isBanned;
  final num? displayStyleGroupId;
  final num? restorePercents;

  const ItemFromListModelSeller({
    this.userId,
    this.soldItemsCount,
    this.activeItemCount,
    this.restoreData,
    this.username,
    this.avatarDate,
    this.isBanned,
    this.displayStyleGroupId,
    this.restorePercents,
  });

  factory ItemFromListModelSeller.fromJson(Map<String, dynamic> json) => ItemFromListModelSeller(
    userId: json['user_id'] is num ? json['user_id'] as num : null,
    soldItemsCount: json['sold_items_count'] is num ? json['sold_items_count'] as num : null,
    activeItemCount: json['active_item_count'] is num ? json['active_item_count'] as num : null,
    restoreData: json['restore_data'] is String ? json['restore_data'] as String : null,
    username: json['username'] is String ? json['username'] as String : null,
    avatarDate: json['avatar_date'] is num ? json['avatar_date'] as num : null,
    isBanned: json['is_banned'] is num ? json['is_banned'] as num : null,
    displayStyleGroupId: json['display_style_group_id'] is num ? json['display_style_group_id'] as num : null,
    restorePercents: json['restore_percents'] is num ? json['restore_percents'] as num : null,
  );
}

class ItemFromListModel {
  final num? itemId;
  final String? itemState;
  final num? categoryId;
  final num? publishedDate;
  final String? title;
  final String? description;
  final num? price;
  final num? updateStatDate;
  final num? refreshedDate;
  final num? viewCount;
  final num? isSticky;
  final String? itemOrigin;
  final num? extendedGuarantee;
  final num? nsb;
  final num? allowAskDiscount;
  final String? titleEn;
  final String? descriptionEn;
  final String? itemDomain;
  final String? resaleItemOrigin;
  final num? isIgnored;
  final bool? guarantee;
  final bool? canViewLoginData;
  final bool? canUpdateItemStats;
  final bool? canViewEmailLoginData;
  final bool? showGetEmailCodeButton;
  final bool? canOpenItem;
  final bool? canCloseItem;
  final bool? canEditItem;
  final bool? canDeleteItem;
  final bool? canStickItem;
  final bool? canUnstickItem;
  final ItemFromListModelBumpSettings? bumpSettings;
  final bool? canBumpItem;
  final bool? canBuyItem;
  final num? rubPrice;
  final String? priceCurrency;
  final bool? canValidateAccount;
  final bool? canResellItemAfterPurchase;
  final bool? canViewAccountLink;
  final String? itemOriginPhrase;
  final List<String>? tags;
  final String? noteText;
  final String? descriptionHtml;
  final String? descriptionHtmlEn;
  final ItemFromListModelSeller? seller;

  const ItemFromListModel({
    this.itemId,
    this.itemState,
    this.categoryId,
    this.publishedDate,
    this.title,
    this.description,
    this.price,
    this.updateStatDate,
    this.refreshedDate,
    this.viewCount,
    this.isSticky,
    this.itemOrigin,
    this.extendedGuarantee,
    this.nsb,
    this.allowAskDiscount,
    this.titleEn,
    this.descriptionEn,
    this.itemDomain,
    this.resaleItemOrigin,
    this.isIgnored,
    this.guarantee,
    this.canViewLoginData,
    this.canUpdateItemStats,
    this.canViewEmailLoginData,
    this.showGetEmailCodeButton,
    this.canOpenItem,
    this.canCloseItem,
    this.canEditItem,
    this.canDeleteItem,
    this.canStickItem,
    this.canUnstickItem,
    this.bumpSettings,
    this.canBumpItem,
    this.canBuyItem,
    this.rubPrice,
    this.priceCurrency,
    this.canValidateAccount,
    this.canResellItemAfterPurchase,
    this.canViewAccountLink,
    this.itemOriginPhrase,
    this.tags,
    this.noteText,
    this.descriptionHtml,
    this.descriptionHtmlEn,
    this.seller,
  });

  factory ItemFromListModel.fromJson(Map<String, dynamic> json) => ItemFromListModel(
    itemId: json['item_id'] is num ? json['item_id'] as num : null,
    itemState: json['item_state'] is String ? json['item_state'] as String : null,
    categoryId: json['category_id'] is num ? json['category_id'] as num : null,
    publishedDate: json['published_date'] is num ? json['published_date'] as num : null,
    title: json['title'] is String ? json['title'] as String : null,
    description: json['description'] is String ? json['description'] as String : null,
    price: json['price'] is num ? json['price'] as num : null,
    updateStatDate: json['update_stat_date'] is num ? json['update_stat_date'] as num : null,
    refreshedDate: json['refreshed_date'] is num ? json['refreshed_date'] as num : null,
    viewCount: json['view_count'] is num ? json['view_count'] as num : null,
    isSticky: json['is_sticky'] is num ? json['is_sticky'] as num : null,
    itemOrigin: json['item_origin'] is String ? json['item_origin'] as String : null,
    extendedGuarantee: json['extended_guarantee'] is num ? json['extended_guarantee'] as num : null,
    nsb: json['nsb'] is num ? json['nsb'] as num : null,
    allowAskDiscount: json['allow_ask_discount'] is num ? json['allow_ask_discount'] as num : null,
    titleEn: json['title_en'] is String ? json['title_en'] as String : null,
    descriptionEn: json['description_en'] is String ? json['description_en'] as String : null,
    itemDomain: json['item_domain'] is String ? json['item_domain'] as String : null,
    resaleItemOrigin: json['resale_item_origin'] is String ? json['resale_item_origin'] as String : null,
    isIgnored: json['isIgnored'] is num ? json['isIgnored'] as num : null,
    guarantee: json['guarantee'] is bool ? json['guarantee'] as bool : null,
    canViewLoginData: json['canViewLoginData'] is bool ? json['canViewLoginData'] as bool : null,
    canUpdateItemStats: json['canUpdateItemStats'] is bool ? json['canUpdateItemStats'] as bool : null,
    canViewEmailLoginData: json['canViewEmailLoginData'] is bool ? json['canViewEmailLoginData'] as bool : null,
    showGetEmailCodeButton: json['showGetEmailCodeButton'] is bool ? json['showGetEmailCodeButton'] as bool : null,
    canOpenItem: json['canOpenItem'] is bool ? json['canOpenItem'] as bool : null,
    canCloseItem: json['canCloseItem'] is bool ? json['canCloseItem'] as bool : null,
    canEditItem: json['canEditItem'] is bool ? json['canEditItem'] as bool : null,
    canDeleteItem: json['canDeleteItem'] is bool ? json['canDeleteItem'] as bool : null,
    canStickItem: json['canStickItem'] is bool ? json['canStickItem'] as bool : null,
    canUnstickItem: json['canUnstickItem'] is bool ? json['canUnstickItem'] as bool : null,
    bumpSettings: json['bumpSettings'] is Map<String, dynamic>
            ? ItemFromListModelBumpSettings.fromJson(json['bumpSettings'] as Map<String, dynamic>)
            : null,
    canBumpItem: json['canBumpItem'] is bool ? json['canBumpItem'] as bool : null,
    canBuyItem: json['canBuyItem'] is bool ? json['canBuyItem'] as bool : null,
    rubPrice: json['rub_price'] is num ? json['rub_price'] as num : null,
    priceCurrency: json['price_currency'] is String ? json['price_currency'] as String : null,
    canValidateAccount: json['canValidateAccount'] is bool ? json['canValidateAccount'] as bool : null,
    canResellItemAfterPurchase: json['canResellItemAfterPurchase'] is bool ? json['canResellItemAfterPurchase'] as bool : null,
    canViewAccountLink: json['canViewAccountLink'] is bool ? json['canViewAccountLink'] as bool : null,
    itemOriginPhrase: json['itemOriginPhrase'] is String ? json['itemOriginPhrase'] as String : null,
    tags: json['tags'] is List ? (json['tags'] as List<dynamic>).cast<String>() : null,
    noteText: json['note_text'] is String ? json['note_text'] as String : null,
    descriptionHtml: json['description_html'] is String ? json['description_html'] as String : null,
    descriptionHtmlEn: json['description_html_en'] is String ? json['description_html_en'] as String : null,
    seller: json['seller'] is Map<String, dynamic>
            ? ItemFromListModelSeller.fromJson(json['seller'] as Map<String, dynamic>)
            : null,
  );
}

class ItemModelGuarantee {
  final num duration;
  final String class$;
  final String durationPhrase;
  final num endDate;
  final bool active;
  final bool cancelled;
  final num remainingTime;
  final String remainingTimePhrase;
  final String cancelledReason;
  final String cancelledReasonPhrase;

  const ItemModelGuarantee({
    required this.duration,
    required this.class$,
    required this.durationPhrase,
    required this.endDate,
    required this.active,
    required this.cancelled,
    required this.remainingTime,
    required this.remainingTimePhrase,
    required this.cancelledReason,
    required this.cancelledReasonPhrase,
  });

  factory ItemModelGuarantee.fromJson(Map<String, dynamic> json) => ItemModelGuarantee(
    duration: json['duration'] is num ? json['duration'] as num : 0,
    class$: json['class'] is String ? json['class'] as String : '',
    durationPhrase: json['durationPhrase'] is String ? json['durationPhrase'] as String : '',
    endDate: json['endDate'] is num ? json['endDate'] as num : 0,
    active: json['active'] is bool ? json['active'] as bool : false,
    cancelled: json['cancelled'] is bool ? json['cancelled'] as bool : false,
    remainingTime: json['remainingTime'] is num ? json['remainingTime'] as num : 0,
    remainingTimePhrase: json['remainingTimePhrase'] is String ? json['remainingTimePhrase'] as String : '',
    cancelledReason: json['cancelledReason'] is String ? json['cancelledReason'] as String : '',
    cancelledReasonPhrase: json['cancelledReasonPhrase'] is String ? json['cancelledReasonPhrase'] as String : '',
  );
}

class ItemModelLoginData {
  final String raw;
  final String encodedRaw;
  final String login;
  final String password;
  final String encodedPassword;
  final String oldPassword;
  final dynamic encodedOldPassword;

  const ItemModelLoginData({
    required this.raw,
    required this.encodedRaw,
    required this.login,
    required this.password,
    required this.encodedPassword,
    required this.oldPassword,
    required this.encodedOldPassword,
  });

  factory ItemModelLoginData.fromJson(Map<String, dynamic> json) => ItemModelLoginData(
    raw: json['raw'] is String ? json['raw'] as String : '',
    encodedRaw: json['encodedRaw'] is String ? json['encodedRaw'] as String : '',
    login: json['login'] is String ? json['login'] as String : '',
    password: json['password'] is String ? json['password'] as String : '',
    encodedPassword: json['encodedPassword'] is String ? json['encodedPassword'] as String : '',
    oldPassword: json['oldPassword'] is String ? json['oldPassword'] as String : '',
    encodedOldPassword: json['encodedOldPassword'],
  );
}

class ItemModelCopyFormatData {
  final String titleLink;
  final String loginData;
  final String full;

  const ItemModelCopyFormatData({
    required this.titleLink,
    required this.loginData,
    required this.full,
  });

  factory ItemModelCopyFormatData.fromJson(Map<String, dynamic> json) => ItemModelCopyFormatData(
    titleLink: json['title_link'] is String ? json['title_link'] as String : '',
    loginData: json['login_data'] is String ? json['login_data'] as String : '',
    full: json['full'] is String ? json['full'] as String : '',
  );
}

class ItemModelBuyer {
  final num userId;
  final num operationDate;
  final bool visitorIsBuyer;
  final String username;
  final num isBanned;
  final num displayStyleGroupId;
  final num displayIconGroupId;
  final String uniqUsernameCss;
  final String uniqBanner;
  final num userGroupId;

  const ItemModelBuyer({
    required this.userId,
    required this.operationDate,
    required this.visitorIsBuyer,
    required this.username,
    required this.isBanned,
    required this.displayStyleGroupId,
    required this.displayIconGroupId,
    required this.uniqUsernameCss,
    required this.uniqBanner,
    required this.userGroupId,
  });

  factory ItemModelBuyer.fromJson(Map<String, dynamic> json) => ItemModelBuyer(
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
    operationDate: json['operation_date'] is num ? json['operation_date'] as num : 0,
    visitorIsBuyer: json['visitorIsBuyer'] is bool ? json['visitorIsBuyer'] as bool : false,
    username: json['username'] is String ? json['username'] as String : '',
    isBanned: json['is_banned'] is num ? json['is_banned'] as num : 0,
    displayStyleGroupId: json['display_style_group_id'] is num ? json['display_style_group_id'] as num : 0,
    displayIconGroupId: json['display_icon_group_id'] is num ? json['display_icon_group_id'] as num : 0,
    uniqUsernameCss: json['uniq_username_css'] is String ? json['uniq_username_css'] as String : '',
    uniqBanner: json['uniq_banner'] is String ? json['uniq_banner'] as String : '',
    userGroupId: json['user_group_id'] is num ? json['user_group_id'] as num : 0,
  );
}

class ItemModelCustomFields {
  final String n4;
  final List<dynamic> allowSelfUnban;
  final String banReason;
  final String discord;
  final String github;
  final String jabber;
  final String lztUnbanAmount;
  final String steam;
  final String telegram;
  final String vk;

  const ItemModelCustomFields({
    required this.n4,
    required this.allowSelfUnban,
    required this.banReason,
    required this.discord,
    required this.github,
    required this.jabber,
    required this.lztUnbanAmount,
    required this.steam,
    required this.telegram,
    required this.vk,
  });

  factory ItemModelCustomFields.fromJson(Map<String, dynamic> json) => ItemModelCustomFields(
    n4: json['_4'] is String ? json['_4'] as String : '',
    allowSelfUnban: json['allowSelfUnban'] is List ? json['allowSelfUnban'] as List<dynamic> : const [],
    banReason: json['ban_reason'] is String ? json['ban_reason'] as String : '',
    discord: json['discord'] is String ? json['discord'] as String : '',
    github: json['github'] is String ? json['github'] as String : '',
    jabber: json['jabber'] is String ? json['jabber'] as String : '',
    lztUnbanAmount: json['lztUnbanAmount'] is String ? json['lztUnbanAmount'] as String : '',
    steam: json['steam'] is String ? json['steam'] as String : '',
    telegram: json['telegram'] is String ? json['telegram'] as String : '',
    vk: json['vk'] is String ? json['vk'] as String : '',
  );
}

class ItemModelBumpSettings {
  final bool canBumpItem;
  final bool canBumpItemGlobally;
  final dynamic shortErrorPhrase;
  final dynamic nextAllowedBumpDate;
  final dynamic errorPhrase;

  const ItemModelBumpSettings({
    required this.canBumpItem,
    required this.canBumpItemGlobally,
    required this.shortErrorPhrase,
    required this.nextAllowedBumpDate,
    required this.errorPhrase,
  });

  factory ItemModelBumpSettings.fromJson(Map<String, dynamic> json) => ItemModelBumpSettings(
    canBumpItem: json['canBumpItem'] is bool ? json['canBumpItem'] as bool : false,
    canBumpItemGlobally: json['canBumpItemGlobally'] is bool ? json['canBumpItemGlobally'] as bool : false,
    shortErrorPhrase: json['shortErrorPhrase'],
    nextAllowedBumpDate: json['nextAllowedBumpDate'],
    errorPhrase: json['errorPhrase'],
  );
}

class ItemModelSellerContacts {
  final String banReason;
  final String telegram;

  const ItemModelSellerContacts({
    required this.banReason,
    required this.telegram,
  });

  factory ItemModelSellerContacts.fromJson(Map<String, dynamic> json) => ItemModelSellerContacts(
    banReason: json['ban_reason'] is String ? json['ban_reason'] as String : '',
    telegram: json['telegram'] is String ? json['telegram'] as String : '',
  );
}

class ItemModelSeller {
  final num userId;
  final String username;
  final num avatarDate;
  final num isBanned;
  final num displayStyleGroupId;
  final num joinedDate;
  final num soldItemsCount;
  final num activeItemsCount;
  final String restoreData;
  final num effectiveLastActivity;
  final dynamic restorePercents;
  final bool isOnline;
  final ItemModelSellerContacts contacts;

  const ItemModelSeller({
    required this.userId,
    required this.username,
    required this.avatarDate,
    required this.isBanned,
    required this.displayStyleGroupId,
    required this.joinedDate,
    required this.soldItemsCount,
    required this.activeItemsCount,
    required this.restoreData,
    required this.effectiveLastActivity,
    required this.restorePercents,
    required this.isOnline,
    required this.contacts,
  });

  factory ItemModelSeller.fromJson(Map<String, dynamic> json) => ItemModelSeller(
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
    username: json['username'] is String ? json['username'] as String : '',
    avatarDate: json['avatar_date'] is num ? json['avatar_date'] as num : 0,
    isBanned: json['is_banned'] is num ? json['is_banned'] as num : 0,
    displayStyleGroupId: json['display_style_group_id'] is num ? json['display_style_group_id'] as num : 0,
    joinedDate: json['joined_date'] is num ? json['joined_date'] as num : 0,
    soldItemsCount: json['sold_items_count'] is num ? json['sold_items_count'] as num : 0,
    activeItemsCount: json['active_items_count'] is num ? json['active_items_count'] as num : 0,
    restoreData: json['restore_data'] is String ? json['restore_data'] as String : '',
    effectiveLastActivity: json['effective_last_activity'] is num ? json['effective_last_activity'] as num : 0,
    restorePercents: json['restore_percents'],
    isOnline: json['isOnline'] is bool ? json['isOnline'] as bool : false,
    contacts: json['contacts'] is Map<String, dynamic>
            ? ItemModelSellerContacts.fromJson(json['contacts'] as Map<String, dynamic>)
            : ItemModelSellerContacts.fromJson(const {}),
  );
}

class ItemModel {
  final num itemId;
  final String itemState;
  final num categoryId;
  final num publishedDate;
  final String title;
  final String description;
  final num price;
  final num updateStatDate;
  final num refreshedDate;
  final num editDate;
  final num pendingDeletionDate;
  final String login;
  final String tempEmail;
  final num viewCount;
  final num isSticky;
  final String information;
  final String itemOrigin;
  final num extendedGuarantee;
  final num nsb;
  final num allowAskDiscount;
  final String titleEn;
  final String descriptionEn;
  final String informationEn;
  final String emailType;
  final String emailProvider;
  final String itemDomain;
  final String resaleItemOrigin;
  final String noteText;
  final dynamic contentType;
  final dynamic contentId;
  final num deleteDate;
  final num deleteUserId;
  final String deleteUsername;
  final String deleteReason;
  final num userAllowAskDiscount;
  final num maxDiscountPercent;
  final String marketCustomTitle;
  final String feedbackData;
  final num buyerDisplayIconGroupId;
  final String buyerUniqBanner;
  final num buyerAvatarDate;
  final num buyerUserGroupId;
  final dynamic isFave;
  final dynamic inCart;
  final dynamic cartPrice;
  final bool canResellItem;
  final num priceWithSellerFee;
  final ItemModelGuarantee guarantee;
  final bool canViewLoginData;
  final bool canUpdateItemStats;
  final bool canReportItem;
  final bool canViewItemViews;
  final ItemModelLoginData loginData;
  final bool canViewEmailLoginData;
  final ItemModelCopyFormatData copyFormatData;
  final bool showGetEmailCodeButton;
  final dynamic getEmailCodeDisplayLogin;
  final ItemModelBuyer buyer;
  final bool isPersonalAccount;
  final num rubPrice;
  final String priceCurrency;
  final String priceWithSellerFeeLabel;
  final bool canValidateAccount;
  final bool canResellItemAfterPurchase;
  final bool isSmallExf;
  final num accountLastActivity;
  final bool canViewAccountLink;
  final List<Map<String, dynamic>> accountLinks;
  final String accountLink;
  final List<String> imagePreviewLinks;
  final bool canChangePassword;
  final bool canChangeEmailPassword;
  final bool uniqueKeyExists;
  final String itemOriginPhrase;
  final bool visitorIsAuthor;
  final bool canAskDiscount;
  final dynamic tags;
  final ItemModelCustomFields customFields;
  final List<dynamic> externalAuth;
  final bool isTrusted;
  final bool isBirthdayToday;
  final bool isIgnored;
  final num deposit;
  final List<Map<String, dynamic>> extraPrices;
  final bool canViewAccountLoginAndTempEmail;
  final ItemModelBumpSettings bumpSettings;
  final bool canCheckGuarantee;
  final bool canShareItem;
  final bool canCheckAiPrice;
  final num aiPrice;
  final num aiPriceCheckDate;
  final bool needToRequireVideoToViewLoginData;
  final bool canCheckAutoBuyPrice;
  final num autoBuyPrice;
  final num autoBuyPriceCheckDate;
  final String descriptionHtml;
  final String descriptionEnHtml;
  final String descriptionPlain;
  final String descriptionEnPlain;
  final ItemModelSeller seller;

  const ItemModel({
    required this.itemId,
    required this.itemState,
    required this.categoryId,
    required this.publishedDate,
    required this.title,
    required this.description,
    required this.price,
    required this.updateStatDate,
    required this.refreshedDate,
    required this.editDate,
    required this.pendingDeletionDate,
    required this.login,
    required this.tempEmail,
    required this.viewCount,
    required this.isSticky,
    required this.information,
    required this.itemOrigin,
    required this.extendedGuarantee,
    required this.nsb,
    required this.allowAskDiscount,
    required this.titleEn,
    required this.descriptionEn,
    required this.informationEn,
    required this.emailType,
    required this.emailProvider,
    required this.itemDomain,
    required this.resaleItemOrigin,
    required this.noteText,
    required this.contentType,
    required this.contentId,
    required this.deleteDate,
    required this.deleteUserId,
    required this.deleteUsername,
    required this.deleteReason,
    required this.userAllowAskDiscount,
    required this.maxDiscountPercent,
    required this.marketCustomTitle,
    required this.feedbackData,
    required this.buyerDisplayIconGroupId,
    required this.buyerUniqBanner,
    required this.buyerAvatarDate,
    required this.buyerUserGroupId,
    required this.isFave,
    required this.inCart,
    required this.cartPrice,
    required this.canResellItem,
    required this.priceWithSellerFee,
    required this.guarantee,
    required this.canViewLoginData,
    required this.canUpdateItemStats,
    required this.canReportItem,
    required this.canViewItemViews,
    required this.loginData,
    required this.canViewEmailLoginData,
    required this.copyFormatData,
    required this.showGetEmailCodeButton,
    required this.getEmailCodeDisplayLogin,
    required this.buyer,
    required this.isPersonalAccount,
    required this.rubPrice,
    required this.priceCurrency,
    required this.priceWithSellerFeeLabel,
    required this.canValidateAccount,
    required this.canResellItemAfterPurchase,
    required this.isSmallExf,
    required this.accountLastActivity,
    required this.canViewAccountLink,
    required this.accountLinks,
    required this.accountLink,
    required this.imagePreviewLinks,
    required this.canChangePassword,
    required this.canChangeEmailPassword,
    required this.uniqueKeyExists,
    required this.itemOriginPhrase,
    required this.visitorIsAuthor,
    required this.canAskDiscount,
    required this.tags,
    required this.customFields,
    required this.externalAuth,
    required this.isTrusted,
    required this.isBirthdayToday,
    required this.isIgnored,
    required this.deposit,
    required this.extraPrices,
    required this.canViewAccountLoginAndTempEmail,
    required this.bumpSettings,
    required this.canCheckGuarantee,
    required this.canShareItem,
    required this.canCheckAiPrice,
    required this.aiPrice,
    required this.aiPriceCheckDate,
    required this.needToRequireVideoToViewLoginData,
    required this.canCheckAutoBuyPrice,
    required this.autoBuyPrice,
    required this.autoBuyPriceCheckDate,
    required this.descriptionHtml,
    required this.descriptionEnHtml,
    required this.descriptionPlain,
    required this.descriptionEnPlain,
    required this.seller,
  });

  factory ItemModel.fromJson(Map<String, dynamic> json) => ItemModel(
    itemId: json['item_id'] is num ? json['item_id'] as num : 0,
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    categoryId: json['category_id'] is num ? json['category_id'] as num : 0,
    publishedDate: json['published_date'] is num ? json['published_date'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
    description: json['description'] is String ? json['description'] as String : '',
    price: json['price'] is num ? json['price'] as num : 0,
    updateStatDate: json['update_stat_date'] is num ? json['update_stat_date'] as num : 0,
    refreshedDate: json['refreshed_date'] is num ? json['refreshed_date'] as num : 0,
    editDate: json['edit_date'] is num ? json['edit_date'] as num : 0,
    pendingDeletionDate: json['pending_deletion_date'] is num ? json['pending_deletion_date'] as num : 0,
    login: json['login'] is String ? json['login'] as String : '',
    tempEmail: json['temp_email'] is String ? json['temp_email'] as String : '',
    viewCount: json['view_count'] is num ? json['view_count'] as num : 0,
    isSticky: json['is_sticky'] is num ? json['is_sticky'] as num : 0,
    information: json['information'] is String ? json['information'] as String : '',
    itemOrigin: json['item_origin'] is String ? json['item_origin'] as String : '',
    extendedGuarantee: json['extended_guarantee'] is num ? json['extended_guarantee'] as num : 0,
    nsb: json['nsb'] is num ? json['nsb'] as num : 0,
    allowAskDiscount: json['allow_ask_discount'] is num ? json['allow_ask_discount'] as num : 0,
    titleEn: json['title_en'] is String ? json['title_en'] as String : '',
    descriptionEn: json['description_en'] is String ? json['description_en'] as String : '',
    informationEn: json['information_en'] is String ? json['information_en'] as String : '',
    emailType: json['email_type'] is String ? json['email_type'] as String : '',
    emailProvider: json['email_provider'] is String ? json['email_provider'] as String : '',
    itemDomain: json['item_domain'] is String ? json['item_domain'] as String : '',
    resaleItemOrigin: json['resale_item_origin'] is String ? json['resale_item_origin'] as String : '',
    noteText: json['note_text'] is String ? json['note_text'] as String : '',
    contentType: json['content_type'],
    contentId: json['content_id'],
    deleteDate: json['delete_date'] is num ? json['delete_date'] as num : 0,
    deleteUserId: json['delete_user_id'] is num ? json['delete_user_id'] as num : 0,
    deleteUsername: json['delete_username'] is String ? json['delete_username'] as String : '',
    deleteReason: json['delete_reason'] is String ? json['delete_reason'] as String : '',
    userAllowAskDiscount: json['user_allow_ask_discount'] is num ? json['user_allow_ask_discount'] as num : 0,
    maxDiscountPercent: json['max_discount_percent'] is num ? json['max_discount_percent'] as num : 0,
    marketCustomTitle: json['market_custom_title'] is String ? json['market_custom_title'] as String : '',
    feedbackData: json['feedback_data'] is String ? json['feedback_data'] as String : '',
    buyerDisplayIconGroupId: json['buyer_display_icon_group_id'] is num ? json['buyer_display_icon_group_id'] as num : 0,
    buyerUniqBanner: json['buyer_uniq_banner'] is String ? json['buyer_uniq_banner'] as String : '',
    buyerAvatarDate: json['buyer_avatar_date'] is num ? json['buyer_avatar_date'] as num : 0,
    buyerUserGroupId: json['buyer_user_group_id'] is num ? json['buyer_user_group_id'] as num : 0,
    isFave: json['is_fave'],
    inCart: json['in_cart'],
    cartPrice: json['cart_price'],
    canResellItem: json['canResellItem'] is bool ? json['canResellItem'] as bool : false,
    priceWithSellerFee: json['priceWithSellerFee'] is num ? json['priceWithSellerFee'] as num : 0,
    guarantee: json['guarantee'] is Map<String, dynamic>
            ? ItemModelGuarantee.fromJson(json['guarantee'] as Map<String, dynamic>)
            : ItemModelGuarantee.fromJson(const {}),
    canViewLoginData: json['canViewLoginData'] is bool ? json['canViewLoginData'] as bool : false,
    canUpdateItemStats: json['canUpdateItemStats'] is bool ? json['canUpdateItemStats'] as bool : false,
    canReportItem: json['canReportItem'] is bool ? json['canReportItem'] as bool : false,
    canViewItemViews: json['canViewItemViews'] is bool ? json['canViewItemViews'] as bool : false,
    loginData: json['loginData'] is Map<String, dynamic>
            ? ItemModelLoginData.fromJson(json['loginData'] as Map<String, dynamic>)
            : ItemModelLoginData.fromJson(const {}),
    canViewEmailLoginData: json['canViewEmailLoginData'] is bool ? json['canViewEmailLoginData'] as bool : false,
    copyFormatData: json['copyFormatData'] is Map<String, dynamic>
            ? ItemModelCopyFormatData.fromJson(json['copyFormatData'] as Map<String, dynamic>)
            : ItemModelCopyFormatData.fromJson(const {}),
    showGetEmailCodeButton: json['showGetEmailCodeButton'] is bool ? json['showGetEmailCodeButton'] as bool : false,
    getEmailCodeDisplayLogin: json['getEmailCodeDisplayLogin'],
    buyer: json['buyer'] is Map<String, dynamic>
            ? ItemModelBuyer.fromJson(json['buyer'] as Map<String, dynamic>)
            : ItemModelBuyer.fromJson(const {}),
    isPersonalAccount: json['isPersonalAccount'] is bool ? json['isPersonalAccount'] as bool : false,
    rubPrice: json['rub_price'] is num ? json['rub_price'] as num : 0,
    priceCurrency: json['price_currency'] is String ? json['price_currency'] as String : '',
    priceWithSellerFeeLabel: json['priceWithSellerFeeLabel'] is String ? json['priceWithSellerFeeLabel'] as String : '',
    canValidateAccount: json['canValidateAccount'] is bool ? json['canValidateAccount'] as bool : false,
    canResellItemAfterPurchase: json['canResellItemAfterPurchase'] is bool ? json['canResellItemAfterPurchase'] as bool : false,
    isSmallExf: json['isSmallExf'] is bool ? json['isSmallExf'] as bool : false,
    accountLastActivity: json['account_last_activity'] is num ? json['account_last_activity'] as num : 0,
    canViewAccountLink: json['canViewAccountLink'] is bool ? json['canViewAccountLink'] as bool : false,
    accountLinks: json['accountLinks'] is List ? (json['accountLinks'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    accountLink: json['accountLink'] is String ? json['accountLink'] as String : '',
    imagePreviewLinks: json['imagePreviewLinks'] is List ? (json['imagePreviewLinks'] as List<dynamic>).cast<String>() : const [],
    canChangePassword: json['canChangePassword'] is bool ? json['canChangePassword'] as bool : false,
    canChangeEmailPassword: json['canChangeEmailPassword'] is bool ? json['canChangeEmailPassword'] as bool : false,
    uniqueKeyExists: json['uniqueKeyExists'] is bool ? json['uniqueKeyExists'] as bool : false,
    itemOriginPhrase: json['itemOriginPhrase'] is String ? json['itemOriginPhrase'] as String : '',
    visitorIsAuthor: json['visitorIsAuthor'] is bool ? json['visitorIsAuthor'] as bool : false,
    canAskDiscount: json['canAskDiscount'] is bool ? json['canAskDiscount'] as bool : false,
    tags: json['tags'],
    customFields: json['customFields'] is Map<String, dynamic>
            ? ItemModelCustomFields.fromJson(json['customFields'] as Map<String, dynamic>)
            : ItemModelCustomFields.fromJson(const {}),
    externalAuth: json['externalAuth'] is List ? json['externalAuth'] as List<dynamic> : const [],
    isTrusted: json['isTrusted'] is bool ? json['isTrusted'] as bool : false,
    isBirthdayToday: json['isBirthdayToday'] is bool ? json['isBirthdayToday'] as bool : false,
    isIgnored: json['isIgnored'] is bool ? json['isIgnored'] as bool : false,
    deposit: json['deposit'] is num ? json['deposit'] as num : 0,
    extraPrices: json['extraPrices'] is List ? (json['extraPrices'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    canViewAccountLoginAndTempEmail: json['canViewAccountLoginAndTempEmail'] is bool ? json['canViewAccountLoginAndTempEmail'] as bool : false,
    bumpSettings: json['bumpSettings'] is Map<String, dynamic>
            ? ItemModelBumpSettings.fromJson(json['bumpSettings'] as Map<String, dynamic>)
            : ItemModelBumpSettings.fromJson(const {}),
    canCheckGuarantee: json['canCheckGuarantee'] is bool ? json['canCheckGuarantee'] as bool : false,
    canShareItem: json['canShareItem'] is bool ? json['canShareItem'] as bool : false,
    canCheckAiPrice: json['canCheckAiPrice'] is bool ? json['canCheckAiPrice'] as bool : false,
    aiPrice: json['aiPrice'] is num ? json['aiPrice'] as num : 0,
    aiPriceCheckDate: json['aiPriceCheckDate'] is num ? json['aiPriceCheckDate'] as num : 0,
    needToRequireVideoToViewLoginData: json['needToRequireVideoToViewLoginData'] is bool ? json['needToRequireVideoToViewLoginData'] as bool : false,
    canCheckAutoBuyPrice: json['canCheckAutoBuyPrice'] is bool ? json['canCheckAutoBuyPrice'] as bool : false,
    autoBuyPrice: json['autoBuyPrice'] is num ? json['autoBuyPrice'] as num : 0,
    autoBuyPriceCheckDate: json['autoBuyPriceCheckDate'] is num ? json['autoBuyPriceCheckDate'] as num : 0,
    descriptionHtml: json['descriptionHtml'] is String ? json['descriptionHtml'] as String : '',
    descriptionEnHtml: json['descriptionEnHtml'] is String ? json['descriptionEnHtml'] as String : '',
    descriptionPlain: json['descriptionPlain'] is String ? json['descriptionPlain'] as String : '',
    descriptionEnPlain: json['descriptionEnPlain'] is String ? json['descriptionEnPlain'] as String : '',
    seller: json['seller'] is Map<String, dynamic>
            ? ItemModelSeller.fromJson(json['seller'] as Map<String, dynamic>)
            : ItemModelSeller.fromJson(const {}),
  );
}

class InvoiceModel {
  final String additionalData;
  final num amount;
  final String comment;
  final num expiresAt;
  final num invoiceDate;
  final num invoiceId;
  final bool isTest;
  final num merchantId;
  final num paidDate;
  final num payerUserId;
  final String paymentId;
  final num resendAttempts;
  final String status;
  final String url;
  final String urlCallback;
  final String urlSuccess;
  final num userId;

  const InvoiceModel({
    required this.additionalData,
    required this.amount,
    required this.comment,
    required this.expiresAt,
    required this.invoiceDate,
    required this.invoiceId,
    required this.isTest,
    required this.merchantId,
    required this.paidDate,
    required this.payerUserId,
    required this.paymentId,
    required this.resendAttempts,
    required this.status,
    required this.url,
    required this.urlCallback,
    required this.urlSuccess,
    required this.userId,
  });

  factory InvoiceModel.fromJson(Map<String, dynamic> json) => InvoiceModel(
    additionalData: json['additional_data'] is String ? json['additional_data'] as String : '',
    amount: json['amount'] is num ? json['amount'] as num : 0,
    comment: json['comment'] is String ? json['comment'] as String : '',
    expiresAt: json['expires_at'] is num ? json['expires_at'] as num : 0,
    invoiceDate: json['invoice_date'] is num ? json['invoice_date'] as num : 0,
    invoiceId: json['invoice_id'] is num ? json['invoice_id'] as num : 0,
    isTest: json['is_test'] is bool ? json['is_test'] as bool : false,
    merchantId: json['merchant_id'] is num ? json['merchant_id'] as num : 0,
    paidDate: json['paid_date'] is num ? json['paid_date'] as num : 0,
    payerUserId: json['payer_user_id'] is num ? json['payer_user_id'] as num : 0,
    paymentId: json['payment_id'] is String ? json['payment_id'] as String : '',
    resendAttempts: json['resend_attempts'] is num ? json['resend_attempts'] as num : 0,
    status: json['status'] is String ? json['status'] as String : '',
    url: json['url'] is String ? json['url'] as String : '',
    urlCallback: json['url_callback'] is String ? json['url_callback'] as String : '',
    urlSuccess: json['url_success'] is String ? json['url_success'] as String : '',
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
  );
}

class RespSystemInfo {
  final num visitorId;
  final num time;
  final num logId;

  const RespSystemInfo({
    required this.visitorId,
    required this.time,
    required this.logId,
  });

  factory RespSystemInfo.fromJson(Map<String, dynamic> json) => RespSystemInfo(
    visitorId: json['visitor_id'] is num ? json['visitor_id'] as num : 0,
    time: json['time'] is num ? json['time'] as num : 0,
    logId: json['log_id'] is num ? json['log_id'] as num : 0,
  );
}

// ─── AutoPaymentsApi Types ────────────────────────────────────────

class AutoPaymentsListResponse {
  final dynamic payments;
  final RespSystemInfo systemInfo;

  const AutoPaymentsListResponse({
    required this.payments,
    required this.systemInfo,
  });

  factory AutoPaymentsListResponse.fromJson(Map<String, dynamic> json) => AutoPaymentsListResponse(
    payments: json['payments'],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class AutoPaymentsCreateBody {
  /// Secret answer.
  final String? secretAnswer;
  /// Username of the payment receiver.
  final String usernameReceiver;
  final Day day;
  /// Amount to be transferred.
  final num amount;
  final Currency? currency;
  /// Payment description.
  final String? description;

  const AutoPaymentsCreateBody({
    this.secretAnswer,
    required this.usernameReceiver,
    required this.day,
    required this.amount,
    this.currency,
    this.description,
  });

  Map<String, Object?> toMap() {
    return {
      'secret_answer': secretAnswer,
      'username_receiver': usernameReceiver,
      'day': day.value,
      'amount': amount,
      'currency': currency?.value,
      'description': description,
    };
  }
}

class AutoPaymentsCreateResponse {
  final String status;
  final String message;
  final num autoPaymentId;
  final RespSystemInfo systemInfo;

  const AutoPaymentsCreateResponse({
    required this.status,
    required this.message,
    required this.autoPaymentId,
    required this.systemInfo,
  });

  factory AutoPaymentsCreateResponse.fromJson(Map<String, dynamic> json) => AutoPaymentsCreateResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    autoPaymentId: json['auto_payment_id'] is num ? json['auto_payment_id'] as num : 0,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class AutoPaymentsDeleteBody {
  /// Auto payment ID.
  final num autoPaymentId;

  const AutoPaymentsDeleteBody({
    required this.autoPaymentId,
  });

  Map<String, Object?> toMap() {
    return {
      'auto_payment_id': autoPaymentId,
    };
  }
}

class AutoPaymentsDeleteResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const AutoPaymentsDeleteResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory AutoPaymentsDeleteResponse.fromJson(Map<String, dynamic> json) => AutoPaymentsDeleteResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

// ─── BatchApi Types ────────────────────────────────────────

typedef BatchBatchBody = List<Map<String, dynamic>>;

class BatchBatchResponseJobsJobId {
  final String? jobResult;
  final String? jobError;

  const BatchBatchResponseJobsJobId({
    this.jobResult,
    this.jobError,
  });

  factory BatchBatchResponseJobsJobId.fromJson(Map<String, dynamic> json) => BatchBatchResponseJobsJobId(
    jobResult: json['_job_result'] is String ? json['_job_result'] as String : null,
    jobError: json['_job_error'] is String ? json['_job_error'] as String : null,
  );
}

class BatchBatchResponseJobs {
  final BatchBatchResponseJobsJobId jobId;

  const BatchBatchResponseJobs({
    required this.jobId,
  });

  factory BatchBatchResponseJobs.fromJson(Map<String, dynamic> json) => BatchBatchResponseJobs(
    jobId: json['job_id'] is Map<String, dynamic>
            ? BatchBatchResponseJobsJobId.fromJson(json['job_id'] as Map<String, dynamic>)
            : BatchBatchResponseJobsJobId.fromJson(const {}),
  );
}

class BatchBatchResponse {
  final BatchBatchResponseJobs jobs;
  final RespSystemInfo? systemInfo;

  const BatchBatchResponse({
    required this.jobs,
    this.systemInfo,
  });

  factory BatchBatchResponse.fromJson(Map<String, dynamic> json) => BatchBatchResponse(
    jobs: json['jobs'] is Map<String, dynamic>
            ? BatchBatchResponseJobs.fromJson(json['jobs'] as Map<String, dynamic>)
            : BatchBatchResponseJobs.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

// ─── CartApi Types ────────────────────────────────────────

class CartGetParams {
  final CategoryId? categoryId;
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;

  const CartGetParams({
    this.categoryId,
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
  });

  Map<String, Object?> toMap() {
    return {
      'category_id': categoryId?.value,
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
    };
  }
}

class CartGetResponse {
  final List<ItemFromListModel> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final String searchUrl;
  final List<ItemFromListModel> stickyItems;
  final RespSystemInfo systemInfo;

  const CartGetResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CartGetResponse.fromJson(Map<String, dynamic> json) => CartGetResponse(
    items: json['items'] is List
            ? (json['items'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List
            ? (json['stickyItems'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CartAddBody {
  /// Item id.
  final num itemId;

  const CartAddBody({
    required this.itemId,
  });

  Map<String, Object?> toMap() {
    return {
      'item_id': itemId,
    };
  }
}

class CartAddResponse {
  final bool success;
  final RespSystemInfo systemInfo;

  const CartAddResponse({
    required this.success,
    required this.systemInfo,
  });

  factory CartAddResponse.fromJson(Map<String, dynamic> json) => CartAddResponse(
    success: json['success'] is bool ? json['success'] as bool : false,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CartDeleteBody {
  /// Item id.
  final num? itemId;

  const CartDeleteBody({
    this.itemId,
  });

  Map<String, Object?> toMap() {
    return {
      'item_id': itemId,
    };
  }
}

class CartDeleteResponse {
  final bool success;
  final RespSystemInfo systemInfo;

  const CartDeleteResponse({
    required this.success,
    required this.systemInfo,
  });

  factory CartDeleteResponse.fromJson(Map<String, dynamic> json) => CartDeleteResponse(
    success: json['success'] is bool ? json['success'] as bool : false,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

// ─── CategoryApi Types ────────────────────────────────────────

class CategoryAllParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;

  const CategoryAllParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
    };
  }
}

class CategoryAllResponse {
  final List<ItemFromListModel> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final String searchUrl;
  final List<ItemFromListModel> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryAllResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryAllResponse.fromJson(Map<String, dynamic> json) => CategoryAllResponse(
    items: json['items'] is List
            ? (json['items'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List
            ? (json['stickyItems'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategorySteamParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email type.
  final List<String>? emailType;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  /// List of games.
  final List<num>? game;
  /// List of minimum hours played by game.
  final Map<String, num>? hoursPlayed;
  /// List of maximum hours played by game.
  final Map<String, num>? hoursPlayedMax;
  final Eg? eg;
  /// List of VAC bans by game.
  final List<num>? vac;
  /// Don't check game existence while checking for vac.
  final bool? vacSkipGameCheck;
  /// Default: "no".
  final Rt? rt;
  final TradeBan? tradeBan;
  final TradeLimit? tradeLimit;
  /// Number of days the account has been offline.
  final num? daybreak;
  final Limit? limit;
  final Mafile? mafile;
  /// How old is the account.
  final num? reg;
  final RegPeriod? regPeriod;
  /// Minimum level.
  final num? lmin;
  /// Maximum level.
  final num? lmax;
  /// Minimum rank in CS2 Matchmaking.
  final num? rmin;
  /// Maximum rank in CS2 Matchmaking.
  final num? rmax;
  /// Minimum rank in CS2 Wingman.
  final num? wingmanRmin;
  /// Maximum rank in CS2 Wingman.
  final num? wingmanRmax;
  /// Has no VAC ban.
  final bool? noVac;
  final MmBan? mmBan;
  /// Minimum balance.
  final num? balanceMin;
  /// Maximum balance.
  final num? balanceMax;
  /// Game ID to check inventory price.
  final num? invGame;
  /// Minimum inventory price for game.
  final num? invMin;
  /// Maximum inventory price for game.
  final num? invMax;
  /// Minimum number of friends.
  final num? friendsMin;
  /// Maximum number of friends.
  final num? friendsMax;
  /// Minimum number of games.
  final num? gmin;
  /// Maximum number of games.
  final num? gmax;
  /// Minimum number of wins.
  final num? winCountMin;
  /// Maximum number of wins.
  final num? winCountMax;
  /// List of medal IDs.
  final List<num>? medalId;
  /// Search for medals using "OR" instead of "AND".
  final bool? medalOperatorOr;
  /// Minimum number of medals.
  final num? medalMin;
  /// Maximum number of medals.
  final num? medalMax;
  /// List of gifts.
  final List<String>? gift;
  /// Minimum number of gifts.
  final num? giftMin;
  /// Maximum number of gifts.
  final num? giftMax;
  /// Minimum number of recently played hours.
  final num? recentlyHoursMin;
  /// Maximum number of recently played hours.
  final num? recentlyHoursMax;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  /// Minimum CS2 rank.
  final num? cs2ProfileRankMin;
  /// Maximum CS2 rank.
  final num? cs2ProfileRankMax;
  /// Minimum number of Dota 2 MMR.
  final num? solommrMin;
  /// Maximum number of Dota 2 MMR.
  final num? solommrMax;
  /// Minimum number of Dota 2 games.
  final num? d2GameCountMin;
  /// Maximum number of Dota 2 games.
  final num? d2GameCountMax;
  /// Minimum number of Dota 2 wins.
  final num? d2WinCountMin;
  /// Maximum number of Dota 2 wins.
  final num? d2WinCountMax;
  /// Minimum number of Dota 2 behavior.
  final num? d2BehaviorMin;
  /// Maximum number of Dota 2 behavior.
  final num? d2BehaviorMax;
  /// Minimum FACEIT level.
  final num? faceitLvlMin;
  /// Maximum FACEIT level.
  final num? faceitLvlMax;
  /// Minimum number of Steam points.
  final num? pointsMin;
  /// Maximum number of Steam points.
  final num? pointsMax;
  /// Minimum number of relevant games.
  final num? relevantGmin;
  /// Maximum number of relevant games.
  final num? relevantGmax;
  /// How old is last transaction.
  final num? lastTransDate;
  final LastTransDatePeriod? lastTransDatePeriod;
  /// How new is last transaction.
  final num? lastTransDateLater;
  final LastTransDatePeriodLater? lastTransDatePeriodLater;
  /// Has no transactions.
  final bool? noTrans;
  /// Has transactions.
  final bool? trans;
  /// Minimum amount spent on gifts.
  final num? giftsPurchaseMin;
  /// Maximum amount spent on gifts.
  final num? giftsPurchaseMax;
  /// Minimum amount of refunds.
  final num? refundsPurchaseMin;
  /// Minimum amount of refunds.
  final num? refundsPurchaseMax;
  /// Minimum spend amount on in-game purchases.
  final num? ingamePurchaseMin;
  /// Maximum spend amount on in-game purchases.
  final num? ingamePurchaseMax;
  /// Minimum spend amount on all purchases.
  final num? gamesPurchaseMin;
  /// Maximum spend amount on all purchases.
  final num? gamesPurchaseMax;
  /// Minimum spend amount on all purchases.
  final num? purchaseMin;
  /// Maximum spend amount on all purchases.
  final num? purchaseMax;
  final HasActivatedKeys? hasActivatedKeys;
  /// Minimum Premier ELO in CS2.
  final num? eloMin;
  /// Maximum Premier ELO in CS2.
  final num? eloMax;
  final Cs2MapRank? cs2MapRank;
  /// Minimum rank in CS2 on a certain map.
  final num? cs2MapRmin;
  /// Maximum rank in CS2 on a certain map.
  final num? cs2MapRmax;
  final HasFaceit? hasFaceit;
  /// Minimum FACEIT level.
  final num? faceitCsgoLvlMin;
  /// Maximum FACEIT level.
  final num? faceitCsgoLvlMax;
  /// Minimum number of Rust deaths.
  final num? rustDeathsMin;
  /// Maximum number of Rust deaths.
  final num? rustDeathsMax;
  /// Minimum number of Rust kills.
  final num? rustKillsMin;
  /// Maximum number of Rust kills.
  final num? rustKillsMax;
  /// How old is last match of Dota 2.
  final num? d2LastMatchDate;
  final D2LastMatchDatePeriod? d2LastMatchDatePeriod;
  /// Minimum number of available to collect trading cards.
  final num? cardsMin;
  /// Maximum number of available to collect trading cards.
  final num? cardsMax;
  /// Minimum number of available games with available to collect trading cards.
  final num? cardsGamesMin;
  /// Maximum number of available games with available to collect trading cards.
  final num? cardsGamesMax;
  /// Ignore inventory value if game has VAC ban.
  final bool? skipVacInv;

  const CategorySteamParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailType,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.itemDomain,
    this.game,
    this.hoursPlayed,
    this.hoursPlayedMax,
    this.eg,
    this.vac,
    this.vacSkipGameCheck,
    this.rt = Rt.no,
    this.tradeBan,
    this.tradeLimit,
    this.daybreak,
    this.limit,
    this.mafile,
    this.reg,
    this.regPeriod,
    this.lmin,
    this.lmax,
    this.rmin,
    this.rmax,
    this.wingmanRmin,
    this.wingmanRmax,
    this.noVac,
    this.mmBan,
    this.balanceMin,
    this.balanceMax,
    this.invGame,
    this.invMin,
    this.invMax,
    this.friendsMin,
    this.friendsMax,
    this.gmin,
    this.gmax,
    this.winCountMin,
    this.winCountMax,
    this.medalId,
    this.medalOperatorOr,
    this.medalMin,
    this.medalMax,
    this.gift,
    this.giftMin,
    this.giftMax,
    this.recentlyHoursMin,
    this.recentlyHoursMax,
    this.country,
    this.notCountry,
    this.cs2ProfileRankMin,
    this.cs2ProfileRankMax,
    this.solommrMin,
    this.solommrMax,
    this.d2GameCountMin,
    this.d2GameCountMax,
    this.d2WinCountMin,
    this.d2WinCountMax,
    this.d2BehaviorMin,
    this.d2BehaviorMax,
    this.faceitLvlMin,
    this.faceitLvlMax,
    this.pointsMin,
    this.pointsMax,
    this.relevantGmin,
    this.relevantGmax,
    this.lastTransDate,
    this.lastTransDatePeriod,
    this.lastTransDateLater,
    this.lastTransDatePeriodLater,
    this.noTrans,
    this.trans,
    this.giftsPurchaseMin,
    this.giftsPurchaseMax,
    this.refundsPurchaseMin,
    this.refundsPurchaseMax,
    this.ingamePurchaseMin,
    this.ingamePurchaseMax,
    this.gamesPurchaseMin,
    this.gamesPurchaseMax,
    this.purchaseMin,
    this.purchaseMax,
    this.hasActivatedKeys,
    this.eloMin,
    this.eloMax,
    this.cs2MapRank,
    this.cs2MapRmin,
    this.cs2MapRmax,
    this.hasFaceit,
    this.faceitCsgoLvlMin,
    this.faceitCsgoLvlMax,
    this.rustDeathsMin,
    this.rustDeathsMax,
    this.rustKillsMin,
    this.rustKillsMax,
    this.d2LastMatchDate,
    this.d2LastMatchDatePeriod,
    this.cardsMin,
    this.cardsMax,
    this.cardsGamesMin,
    this.cardsGamesMax,
    this.skipVacInv,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_type[]': emailType,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'item_domain': itemDomain,
      'game[]': game,
      'hours_played': hoursPlayed,
      'hours_played_max': hoursPlayedMax,
      'eg': eg?.value,
      'vac[]': vac,
      'vac_skip_game_check': vacSkipGameCheck,
      'rt': rt?.value,
      'trade_ban': tradeBan?.value,
      'trade_limit': tradeLimit?.value,
      'daybreak': daybreak,
      'limit': limit?.value,
      'mafile': mafile?.value,
      'reg': reg,
      'reg_period': regPeriod?.value,
      'lmin': lmin,
      'lmax': lmax,
      'rmin': rmin,
      'rmax': rmax,
      'wingman_rmin': wingmanRmin,
      'wingman_rmax': wingmanRmax,
      'no_vac': noVac,
      'mm_ban': mmBan?.value,
      'balance_min': balanceMin,
      'balance_max': balanceMax,
      'inv_game': invGame,
      'inv_min': invMin,
      'inv_max': invMax,
      'friends_min': friendsMin,
      'friends_max': friendsMax,
      'gmin': gmin,
      'gmax': gmax,
      'win_count_min': winCountMin,
      'win_count_max': winCountMax,
      'medal_id[]': medalId,
      'medal_operator_or': medalOperatorOr,
      'medal_min': medalMin,
      'medal_max': medalMax,
      'gift[]': gift,
      'gift_min': giftMin,
      'gift_max': giftMax,
      'recently_hours_min': recentlyHoursMin,
      'recently_hours_max': recentlyHoursMax,
      'country[]': country,
      'not_country[]': notCountry,
      'cs2_profile_rank_min': cs2ProfileRankMin,
      'cs2_profile_rank_max': cs2ProfileRankMax,
      'solommr_min': solommrMin,
      'solommr_max': solommrMax,
      'd2_game_count_min': d2GameCountMin,
      'd2_game_count_max': d2GameCountMax,
      'd2_win_count_min': d2WinCountMin,
      'd2_win_count_max': d2WinCountMax,
      'd2_behavior_min': d2BehaviorMin,
      'd2_behavior_max': d2BehaviorMax,
      'faceit_lvl_min': faceitLvlMin,
      'faceit_lvl_max': faceitLvlMax,
      'points_min': pointsMin,
      'points_max': pointsMax,
      'relevant_gmin': relevantGmin,
      'relevant_gmax': relevantGmax,
      'last_trans_date': lastTransDate,
      'last_trans_date_period': lastTransDatePeriod?.value,
      'last_trans_date_later': lastTransDateLater,
      'last_trans_date_period_later': lastTransDatePeriodLater?.value,
      'no_trans': noTrans,
      'trans': trans,
      'gifts_purchase_min': giftsPurchaseMin,
      'gifts_purchase_max': giftsPurchaseMax,
      'refunds_purchase_min': refundsPurchaseMin,
      'refunds_purchase_max': refundsPurchaseMax,
      'ingame_purchase_min': ingamePurchaseMin,
      'ingame_purchase_max': ingamePurchaseMax,
      'games_purchase_min': gamesPurchaseMin,
      'games_purchase_max': gamesPurchaseMax,
      'purchase_min': purchaseMin,
      'purchase_max': purchaseMax,
      'has_activated_keys': hasActivatedKeys?.value,
      'elo_min': eloMin,
      'elo_max': eloMax,
      'cs2_map_rank': cs2MapRank?.value,
      'cs2_map_rmin': cs2MapRmin,
      'cs2_map_rmax': cs2MapRmax,
      'has_faceit': hasFaceit?.value,
      'faceit_csgo_lvl_min': faceitCsgoLvlMin,
      'faceit_csgo_lvl_max': faceitCsgoLvlMax,
      'rust_deaths_min': rustDeathsMin,
      'rust_deaths_max': rustDeathsMax,
      'rust_kills_min': rustKillsMin,
      'rust_kills_max': rustKillsMax,
      'd2_last_match_date': d2LastMatchDate,
      'd2_last_match_date_period': d2LastMatchDatePeriod?.value,
      'cards_min': cardsMin,
      'cards_max': cardsMax,
      'cards_games_min': cardsGamesMin,
      'cards_games_max': cardsGamesMax,
      'skip_vac_inv': skipVacInv,
    };
  }
}

class CategorySteamResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategorySteamResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategorySteamResponse.fromJson(Map<String, dynamic> json) => CategorySteamResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryFortniteParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  /// Email type.
  final List<String>? emailType;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  final TempEmail? tempEmail;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Eg? eg;
  /// Minimum number of skins.
  final num? smin;
  /// Maximum number of skins.
  final num? smax;
  /// Minimum number of V-Bucks.
  final num? vbmin;
  /// Maximum number of V-Bucks.
  final num? vbmax;
  /// Skins.
  final List<String>? skin;
  /// Pickaxes.
  final List<String>? pickaxe;
  /// Gliders.
  final List<String>? glider;
  /// Dances.
  final List<String>? dance;
  final ChangeEmail? changeEmail;
  /// Platform.
  final List<String>? platform;
  /// Minimum number of shop skins.
  final num? skinsShopMin;
  /// Maximum number of shop skins.
  final num? skinsShopMax;
  /// Minimum number of shop pickaxes.
  final num? pickaxesShopMin;
  /// Maximum number of shop pickaxes.
  final num? pickaxesShopMax;
  /// Minimum number of shop dances.
  final num? dancesShopMin;
  /// Maximum number of shop dances.
  final num? dancesShopMax;
  /// Minimum number of shop gliders.
  final num? glidersShopMin;
  /// Maximum number of shop gliders.
  final num? glidersShopMax;
  /// Minimum total cost of all skins in the shop in V-Bucks.
  final num? skinsShopVbmin;
  /// Maximum total cost of all skins in the shop in V-Bucks.
  final num? skinsShopVbmax;
  /// Minimum total cost of all pickaxes in the shop in V-Bucks.
  final num? pickaxesShopVbmin;
  /// Maximum total cost of all pickaxes in the shop in V-Bucks.
  final num? pickaxesShopVbmax;
  /// Minimum total cost of all dances in the shop in V-Bucks.
  final num? dancesShopVbmin;
  /// Maximum total cost of all dances in the shop in V-Bucks.
  final num? dancesShopVbmax;
  /// Minimum total cost of all gliders in the shop in V-Bucks.
  final num? glidersShopVbmin;
  /// Maximum total cost of all gliders in the shop in V-Bucks.
  final num? glidersShopVbmax;
  final Bp? bp;
  /// Minimum level.
  final num? lmin;
  /// Maximum level.
  final num? lmax;
  /// Minimum level of Battle Pass.
  final num? bpLmin;
  /// Maximum level of Battle Pass.
  final num? bpLmax;
  /// How old is last transaction.
  final num? lastTransDate;
  final LastTransDatePeriod? lastTransDatePeriod;
  /// Has no transactions.
  final bool? noTrans;
  final XboxLinkable? xboxLinkable;
  final PsnLinkable? psnLinkable;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// Has Rocket League purchases.
  final bool? rlPurchases;
  /// How old is the account.
  final num? reg;
  final RegPeriod? regPeriod;
  /// Minimum number of available refund credits.
  final num? refundCreditsMin;
  /// Maximum number of available refund credits.
  final num? refundCreditsMax;
  /// Minimum number of pickaxes.
  final num? pickaxeMin;
  /// Maximum number of pickaxes.
  final num? pickaxeMax;
  /// Minimum number of dances.
  final num? dmin;
  /// Maximum number of dances.
  final num? dmax;
  /// Minimum number of gliders.
  final num? gmin;
  /// Maximum number of gliders.
  final num? gmax;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;

  const CategoryFortniteParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.emailType,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.tempEmail,
    this.itemDomain,
    this.eg,
    this.smin,
    this.smax,
    this.vbmin,
    this.vbmax,
    this.skin,
    this.pickaxe,
    this.glider,
    this.dance,
    this.changeEmail,
    this.platform,
    this.skinsShopMin,
    this.skinsShopMax,
    this.pickaxesShopMin,
    this.pickaxesShopMax,
    this.dancesShopMin,
    this.dancesShopMax,
    this.glidersShopMin,
    this.glidersShopMax,
    this.skinsShopVbmin,
    this.skinsShopVbmax,
    this.pickaxesShopVbmin,
    this.pickaxesShopVbmax,
    this.dancesShopVbmin,
    this.dancesShopVbmax,
    this.glidersShopVbmin,
    this.glidersShopVbmax,
    this.bp,
    this.lmin,
    this.lmax,
    this.bpLmin,
    this.bpLmax,
    this.lastTransDate,
    this.lastTransDatePeriod,
    this.noTrans,
    this.xboxLinkable,
    this.psnLinkable,
    this.daybreak,
    this.rlPurchases,
    this.reg,
    this.regPeriod,
    this.refundCreditsMin,
    this.refundCreditsMax,
    this.pickaxeMin,
    this.pickaxeMax,
    this.dmin,
    this.dmax,
    this.gmin,
    this.gmax,
    this.country,
    this.notCountry,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'email_type[]': emailType,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'temp_email': tempEmail?.value,
      'item_domain': itemDomain,
      'eg': eg?.value,
      'smin': smin,
      'smax': smax,
      'vbmin': vbmin,
      'vbmax': vbmax,
      'skin[]': skin,
      'pickaxe[]': pickaxe,
      'glider[]': glider,
      'dance[]': dance,
      'change_email': changeEmail?.value,
      'platform[]': platform,
      'skins_shop_min': skinsShopMin,
      'skins_shop_max': skinsShopMax,
      'pickaxes_shop_min': pickaxesShopMin,
      'pickaxes_shop_max': pickaxesShopMax,
      'dances_shop_min': dancesShopMin,
      'dances_shop_max': dancesShopMax,
      'gliders_shop_min': glidersShopMin,
      'gliders_shop_max': glidersShopMax,
      'skins_shop_vbmin': skinsShopVbmin,
      'skins_shop_vbmax': skinsShopVbmax,
      'pickaxes_shop_vbmin': pickaxesShopVbmin,
      'pickaxes_shop_vbmax': pickaxesShopVbmax,
      'dances_shop_vbmin': dancesShopVbmin,
      'dances_shop_vbmax': dancesShopVbmax,
      'gliders_shop_vbmin': glidersShopVbmin,
      'gliders_shop_vbmax': glidersShopVbmax,
      'bp': bp?.value,
      'lmin': lmin,
      'lmax': lmax,
      'bp_lmin': bpLmin,
      'bp_lmax': bpLmax,
      'last_trans_date': lastTransDate,
      'last_trans_date_period': lastTransDatePeriod?.value,
      'no_trans': noTrans,
      'xbox_linkable': xboxLinkable?.value,
      'psn_linkable': psnLinkable?.value,
      'daybreak': daybreak,
      'rl_purchases': rlPurchases,
      'reg': reg,
      'reg_period': regPeriod?.value,
      'refund_credits_min': refundCreditsMin,
      'refund_credits_max': refundCreditsMax,
      'pickaxe_min': pickaxeMin,
      'pickaxe_max': pickaxeMax,
      'dmin': dmin,
      'dmax': dmax,
      'gmin': gmin,
      'gmax': gmax,
      'country[]': country,
      'not_country[]': notCountry,
    };
  }
}

class CategoryFortniteResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryFortniteResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryFortniteResponse.fromJson(Map<String, dynamic> json) => CategoryFortniteResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryMihoyoParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Email type.
  final List<String>? emailType;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Email? email;
  final Ea? ea;
  /// Region.
  final List<String>? region;
  /// List of disallowed regions.
  final List<String>? notRegion;
  /// List of characters.
  final List<num>? genshinCharacter;
  /// List of minimum constellations on characters.
  final Map<String, num>? genshinCharacterConstellations;
  /// List of maximum constellations on characters.
  final Map<String, num>? genshinCharacterConstellationsMax;
  /// List of weapons.
  final List<num>? genshinWeapon;
  /// Minimum number of characters.
  final num? genshinCharMin;
  /// Maximum number of characters.
  final num? genshinCharMax;
  /// Minimum number of legendary characters.
  final num? genshinLegendaryMin;
  /// Maximum number of legendary characters.
  final num? genshinLegendaryMax;
  /// Minimum level.
  final num? genshinLevelMin;
  /// Maximum level.
  final num? genshinLevelMax;
  /// Minimum number of legendary weapon characters.
  final num? genshinLegendaryWeaponMin;
  /// Maximum number of legendary weapon characters.
  final num? genshinLegendaryWeaponMax;
  /// Minimum number of constellations on legendary characters.
  final num? constellationsMin;
  /// Maximum number of constellations on legendary characters.
  final num? constellationsMax;
  /// Minimum number of achievements.
  final num? genshinAchievementMin;
  /// Maximum number of achievements.
  final num? genshinAchievementMax;
  /// Minimum number of primogems.
  final num? genshinCurrencyMin;
  /// Maximum number of primogems.
  final num? genshinCurrencyMax;
  /// List of characters.
  final List<num>? honkaiCharacter;
  /// List of minimum eidolons on characters.
  final Map<String, num>? honkaiCharacterEidolons;
  /// List of maximum eidolons on characters.
  final Map<String, num>? honkaiCharacterEidolonsMax;
  /// List of weapons.
  final List<num>? honkaiWeapon;
  /// Minimum number of characters.
  final num? honkaiCharMin;
  /// Maximum number of characters.
  final num? honkaiCharMax;
  /// Minimum number of legendary characters.
  final num? honkaiLegendaryMin;
  /// Maximum number of legendary characters.
  final num? honkaiLegendaryMax;
  /// Minimum level.
  final num? honkaiLevelMin;
  /// Maximum level.
  final num? honkaiLevelMax;
  /// Minimum number of legendary weapon characters.
  final num? honkaiLegendaryWeaponMin;
  /// Maximum number of legendary weapon characters.
  final num? honkaiLegendaryWeaponMax;
  /// Minimum number of constellations on Honkai: Star Rail legendary characters.
  final num? eidolonsMin;
  /// Maximum number of legendary Honkai: Star Rail weapon characters.
  final num? eidolonsMax;
  /// Minimum number of achievements.
  final num? honkaiAchievementMin;
  /// Maximum number of achievements.
  final num? honkaiAchievementMax;
  /// Minimum number of Stellar Jade.
  final num? honkaiCurrencyMin;
  /// Maximum number of Stellar Jade.
  final num? honkaiCurrencyMax;
  /// List of Zenless Zone Zero characters.
  final List<num>? zenlessCharacter;
  /// List of minimum cinemas on characters.
  final Map<String, num>? zenlessCharacterCinemas;
  /// List of maximum cinemas on characters.
  final Map<String, num>? zenlessCharacterCinemasMax;
  /// List of Zenless Zone Zero weapons.
  final List<num>? zenlessWeapon;
  /// Minimum number of Zenless Zone Zero legendary characters.
  final num? zenlessLegendaryMin;
  /// Maximum number of Zenless Zone Zero legendary characters.
  final num? zenlessLegendaryMax;
  /// Minimum number of cinemas on Zenless Zone Zero characters.
  final num? cinemasMin;
  /// Maximum number of cinemas on Zenless Zone Zero characters.
  final num? cinemasMax;
  /// Minimum number of legendary Zenless Zone Zero weapon characters.
  final num? zenlessLegendaryWeaponMin;
  /// Maximum number of legendary Zenless Zone Zero weapon characters.
  final num? zenlessLegendaryWeaponMax;
  /// Minimum number of Zenless Zone Zero characters.
  final num? zenlessCharMin;
  /// Maximum number of Zenless Zone Zero characters.
  final num? zenlessCharMax;
  /// Minimum Zenless Zone Zero level.
  final num? zenlessLevelMin;
  /// Maximum Zenless Zone Zero level.
  final num? zenlessLevelMax;
  /// Minimum count of Zenless Zone Zero achievements.
  final num? zenlessAchievementMin;
  /// Maximum count of Zenless Zone Zero achievements.
  final num? zenlessAchievementMax;
  /// Minimum count of Zenless Zone Zero polychrome.
  final num? zenlessCurrencyMin;
  /// Maximum count of Zenless Zone Zero polychrome.
  final num? zenlessCurrencyMax;
  /// Number of days the account has been offline.
  final num? daybreak;

  const CategoryMihoyoParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.emailType,
    this.parseSameItemIds,
    this.itemDomain,
    this.email,
    this.ea,
    this.region,
    this.notRegion,
    this.genshinCharacter,
    this.genshinCharacterConstellations,
    this.genshinCharacterConstellationsMax,
    this.genshinWeapon,
    this.genshinCharMin,
    this.genshinCharMax,
    this.genshinLegendaryMin,
    this.genshinLegendaryMax,
    this.genshinLevelMin,
    this.genshinLevelMax,
    this.genshinLegendaryWeaponMin,
    this.genshinLegendaryWeaponMax,
    this.constellationsMin,
    this.constellationsMax,
    this.genshinAchievementMin,
    this.genshinAchievementMax,
    this.genshinCurrencyMin,
    this.genshinCurrencyMax,
    this.honkaiCharacter,
    this.honkaiCharacterEidolons,
    this.honkaiCharacterEidolonsMax,
    this.honkaiWeapon,
    this.honkaiCharMin,
    this.honkaiCharMax,
    this.honkaiLegendaryMin,
    this.honkaiLegendaryMax,
    this.honkaiLevelMin,
    this.honkaiLevelMax,
    this.honkaiLegendaryWeaponMin,
    this.honkaiLegendaryWeaponMax,
    this.eidolonsMin,
    this.eidolonsMax,
    this.honkaiAchievementMin,
    this.honkaiAchievementMax,
    this.honkaiCurrencyMin,
    this.honkaiCurrencyMax,
    this.zenlessCharacter,
    this.zenlessCharacterCinemas,
    this.zenlessCharacterCinemasMax,
    this.zenlessWeapon,
    this.zenlessLegendaryMin,
    this.zenlessLegendaryMax,
    this.cinemasMin,
    this.cinemasMax,
    this.zenlessLegendaryWeaponMin,
    this.zenlessLegendaryWeaponMax,
    this.zenlessCharMin,
    this.zenlessCharMax,
    this.zenlessLevelMin,
    this.zenlessLevelMax,
    this.zenlessAchievementMin,
    this.zenlessAchievementMax,
    this.zenlessCurrencyMin,
    this.zenlessCurrencyMax,
    this.daybreak,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'email_type[]': emailType,
      'parse_same_item_ids': parseSameItemIds,
      'item_domain': itemDomain,
      'email': email?.value,
      'ea': ea?.value,
      'region': region,
      'not_region': notRegion,
      'genshin_character[]': genshinCharacter,
      'genshin_character_constellations': genshinCharacterConstellations,
      'genshin_character_constellations_max': genshinCharacterConstellationsMax,
      'genshin_weapon[]': genshinWeapon,
      'genshin_char_min': genshinCharMin,
      'genshin_char_max': genshinCharMax,
      'genshin_legendary_min': genshinLegendaryMin,
      'genshin_legendary_max': genshinLegendaryMax,
      'genshin_level_min': genshinLevelMin,
      'genshin_level_max': genshinLevelMax,
      'genshin_legendary_weapon_min': genshinLegendaryWeaponMin,
      'genshin_legendary_weapon_max': genshinLegendaryWeaponMax,
      'constellations_min': constellationsMin,
      'constellations_max': constellationsMax,
      'genshin_achievement_min': genshinAchievementMin,
      'genshin_achievement_max': genshinAchievementMax,
      'genshin_currency_min': genshinCurrencyMin,
      'genshin_currency_max': genshinCurrencyMax,
      'honkai_character[]': honkaiCharacter,
      'honkai_character_eidolons': honkaiCharacterEidolons,
      'honkai_character_eidolons_max': honkaiCharacterEidolonsMax,
      'honkai_weapon[]': honkaiWeapon,
      'honkai_char_min': honkaiCharMin,
      'honkai_char_max': honkaiCharMax,
      'honkai_legendary_min': honkaiLegendaryMin,
      'honkai_legendary_max': honkaiLegendaryMax,
      'honkai_level_min': honkaiLevelMin,
      'honkai_level_max': honkaiLevelMax,
      'honkai_legendary_weapon_min': honkaiLegendaryWeaponMin,
      'honkai_legendary_weapon_max': honkaiLegendaryWeaponMax,
      'eidolons_min': eidolonsMin,
      'eidolons_max': eidolonsMax,
      'honkai_achievement_min': honkaiAchievementMin,
      'honkai_achievement_max': honkaiAchievementMax,
      'honkai_currency_min': honkaiCurrencyMin,
      'honkai_currency_max': honkaiCurrencyMax,
      'zenless_character[]': zenlessCharacter,
      'zenless_character_cinemas': zenlessCharacterCinemas,
      'zenless_character_cinemas_max': zenlessCharacterCinemasMax,
      'zenless_weapon[]': zenlessWeapon,
      'zenless_legendary_min': zenlessLegendaryMin,
      'zenless_legendary_max': zenlessLegendaryMax,
      'cinemas_min': cinemasMin,
      'cinemas_max': cinemasMax,
      'zenless_legendary_weapon_min': zenlessLegendaryWeaponMin,
      'zenless_legendary_weapon_max': zenlessLegendaryWeaponMax,
      'zenless_char_min': zenlessCharMin,
      'zenless_char_max': zenlessCharMax,
      'zenless_level_min': zenlessLevelMin,
      'zenless_level_max': zenlessLevelMax,
      'zenless_achievement_min': zenlessAchievementMin,
      'zenless_achievement_max': zenlessAchievementMax,
      'zenless_currency_min': zenlessCurrencyMin,
      'zenless_currency_max': zenlessCurrencyMax,
      'daybreak': daybreak,
    };
  }
}

class CategoryMihoyoResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryMihoyoResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryMihoyoResponse.fromJson(Map<String, dynamic> json) => CategoryMihoyoResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryRiotParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  /// Minimum valorant rank.
  final num? rmin;
  /// Maximum valorant rank.
  final num? rmax;
  /// Last minimum valorant rank.
  final num? lastRmin;
  /// Last maximum valorant rank.
  final num? lastRmax;
  /// Previous minimum rank.
  final num? previousRmin;
  /// Previous maximum rank.
  final num? previousRmax;
  /// List of weapon skins.
  final List<String>? weaponSkin;
  /// List of buddies.
  final List<String>? buddy;
  /// List of agents.
  final List<String>? agent;
  /// List of champions.
  final List<String>? champion;
  /// List of LoL skins.
  final List<String>? skin;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// Minimum level in Valorant.
  final num? valorantLevelMin;
  /// Maximum level in Valorant.
  final num? valorantLevelMax;
  /// Minimum level in LoL.
  final num? lolLevelMin;
  /// Maximum level in LoL.
  final num? lolLevelMax;
  /// Minimum inventory value.
  final num? invMin;
  /// Maximum inventory value.
  final num? invMax;
  /// Minimum number of Valorant points.
  final num? vpMin;
  /// Maximum number of Valorant points.
  final num? vpMax;
  /// Minimum number of skins.
  final num? valorantSmin;
  /// Maximum number of skins.
  final num? valorantSmax;
  /// List of allowed rank types.
  final List<String>? valorantRankType;
  /// Minimum amount of agents.
  final num? amin;
  /// Maximum amount of agents.
  final num? amax;
  /// List of allowed regions in Valorant.
  final List<String>? valorantRegion;
  /// List of disallowed regions in Valorant.
  final List<String>? valorantNotRegion;
  /// List of allowed regions in LoL.
  final List<String>? lolRegion;
  /// List of disallowed regions in LoL.
  final List<String>? lolNotRegion;
  /// Has any knife.
  final bool? knife;
  /// Minimum number of skins in LoL.
  final num? lolSmin;
  /// Maximum number of skins in LoL.
  final num? lolSmax;
  /// Minimum number of champions.
  final num? championMin;
  /// Maximum number of champions.
  final num? championMax;
  /// Minimum win-rate.
  final num? winRateMin;
  /// Maximum win-rate.
  final num? winRateMax;
  /// Minimum wallet blue balance.
  final num? blueMin;
  /// Maximum wallet blue balance.
  final num? blueMax;
  /// Minimum wallet orange balance.
  final num? orangeMin;
  /// Maximum wallet orange balance.
  final num? orangeMax;
  /// Minimum wallet mythic balance.
  final num? mythicMin;
  /// Maximum wallet mythic balance.
  final num? mythicMax;
  /// Minimum wallet riot balance.
  final num? riotMin;
  /// Maximum wallet riot balance.
  final num? riotMax;
  final Email? email;
  final Tel? tel;
  /// Minimum knifes in Valorant.
  final num? valorantKnifeMin;
  /// Maximum knifes in Valorant.
  final num? valorantKnifeMax;
  /// Minimum number of Valorant Radiant Points.
  final num? rpMin;
  /// Maximum number of Valorant Radiant Points.
  final num? rpMax;
  /// Minimum number of Valorant free agents.
  final num? faMin;
  /// Maximum number of Valorant free agents.
  final num? faMax;
  /// List of allowed ranks in LoL.
  final List<String>? lolRank;

  const CategoryRiotParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.rmin,
    this.rmax,
    this.lastRmin,
    this.lastRmax,
    this.previousRmin,
    this.previousRmax,
    this.weaponSkin,
    this.buddy,
    this.agent,
    this.champion,
    this.skin,
    this.country,
    this.notCountry,
    this.daybreak,
    this.valorantLevelMin,
    this.valorantLevelMax,
    this.lolLevelMin,
    this.lolLevelMax,
    this.invMin,
    this.invMax,
    this.vpMin,
    this.vpMax,
    this.valorantSmin,
    this.valorantSmax,
    this.valorantRankType,
    this.amin,
    this.amax,
    this.valorantRegion,
    this.valorantNotRegion,
    this.lolRegion,
    this.lolNotRegion,
    this.knife,
    this.lolSmin,
    this.lolSmax,
    this.championMin,
    this.championMax,
    this.winRateMin,
    this.winRateMax,
    this.blueMin,
    this.blueMax,
    this.orangeMin,
    this.orangeMax,
    this.mythicMin,
    this.mythicMax,
    this.riotMin,
    this.riotMax,
    this.email,
    this.tel,
    this.valorantKnifeMin,
    this.valorantKnifeMax,
    this.rpMin,
    this.rpMax,
    this.faMin,
    this.faMax,
    this.lolRank,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'rmin': rmin,
      'rmax': rmax,
      'last_rmin': lastRmin,
      'last_rmax': lastRmax,
      'previous_rmin': previousRmin,
      'previous_rmax': previousRmax,
      'weaponSkin[]': weaponSkin,
      'buddy[]': buddy,
      'agent[]': agent,
      'champion[]': champion,
      'skin[]': skin,
      'country[]': country,
      'not_country[]': notCountry,
      'daybreak': daybreak,
      'valorant_level_min': valorantLevelMin,
      'valorant_level_max': valorantLevelMax,
      'lol_level_min': lolLevelMin,
      'lol_level_max': lolLevelMax,
      'inv_min': invMin,
      'inv_max': invMax,
      'vp_min': vpMin,
      'vp_max': vpMax,
      'valorant_smin': valorantSmin,
      'valorant_smax': valorantSmax,
      'valorant_rank_type[]': valorantRankType,
      'amin': amin,
      'amax': amax,
      'valorant_region[]': valorantRegion,
      'valorant_not_region[]': valorantNotRegion,
      'lol_region[]': lolRegion,
      'lol_not_region[]': lolNotRegion,
      'knife': knife,
      'lol_smin': lolSmin,
      'lol_smax': lolSmax,
      'champion_min': championMin,
      'champion_max': championMax,
      'win_rate_min': winRateMin,
      'win_rate_max': winRateMax,
      'blue_min': blueMin,
      'blue_max': blueMax,
      'orange_min': orangeMin,
      'orange_max': orangeMax,
      'mythic_min': mythicMin,
      'mythic_max': mythicMax,
      'riot_min': riotMin,
      'riot_max': riotMax,
      'email': email?.value,
      'tel': tel?.value,
      'valorant_knife_min': valorantKnifeMin,
      'valorant_knife_max': valorantKnifeMax,
      'rp_min': rpMin,
      'rp_max': rpMax,
      'fa_min': faMin,
      'fa_max': faMax,
      'lol_rank[]': lolRank,
    };
  }
}

class CategoryRiotResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryRiotResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryRiotResponse.fromJson(Map<String, dynamic> json) => CategoryRiotResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryTelegramParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  final Spam? spam;
  final Password? password;
  final Premium? premium;
  /// When premium subscription will be active.
  final num? premiumExpiration;
  final PremiumExpirationPeriod? premiumExpirationPeriod;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// Minimum number of channels.
  final num? minChannels;
  /// Maximum number of channels.
  final num? maxChannels;
  /// Minimum number of chats.
  final num? minChats;
  /// Maximum number of chats.
  final num? maxChats;
  /// Minimum number of conversations.
  final num? minConversations;
  /// Maximum number of conversations.
  final num? maxConversations;
  /// Minimum number of channels, where account is administrator/owner.
  final num? minAdmin;
  /// Maximum number of channels, where account is administrator/owner.
  final num? maxAdmin;
  /// Minimum number of subscribers in channel, where account is administrator/owner.
  final num? minAdminSub;
  /// Maximum number of subscribers in channel, where account is administrator/owner.
  final num? maxAdminSub;
  /// Minimum number of digits in ID.
  final num? digMin;
  /// Maximum number of digits in ID.
  final num? digMax;
  /// Minimum number of contacts.
  final num? minContacts;
  /// Maximum number of contacts.
  final num? maxContacts;
  /// Minimum number of Telegram Stars.
  final num? minStars;
  /// Maximum number of Telegram Stars.
  final num? maxStars;
  /// Birthday was X time before.
  final num? birthday;
  final BirthdayPeriod? birthdayPeriod;
  /// Birthday was X time after.
  final num? birthdayAfter;
  final BirthdayAfterPeriod? birthdayAfterPeriod;
  /// Minimum ID of account, will be rounded down till nearest 10k. Available if your balance is higher than 100000 RUB.
  final num? minId;
  /// Maximum ID of account, will be rounded down till nearest 10k. Available if your balance is higher than 100000 RUB.
  final num? maxId;
  /// Allow geo spam block in search with spam=no.
  final bool? allowGeoSpamblock;
  /// Minimum number of Telegram gifts on account.
  final num? minGifts;
  /// Maximum number of Telegram gifts on account.
  final num? maxGifts;
  /// Minimum number of Telegram NFT gifts on account.
  final num? minNftGifts;
  /// Maximum number of Telegram NFT gifts on account.
  final num? maxNftGifts;
  /// Minimum value of all Stars gifts.
  final num? minGiftsStars;
  /// Maximum value of all Stars gifts.
  final num? maxGiftsStars;
  /// Minimum value of all Stars gifts after convert.
  final num? minGiftsConvertStars;
  /// Maximum value of all Stars gifts after convert.
  final num? maxGiftsConvertStars;
  /// List of allowed DC ID.
  final List<num>? dcId;
  /// List of disallowed DC ID.
  final List<num>? notDcId;
  final Email? email;
  /// Minimum number of bots.
  final num? minBots;
  /// Maximum number of bots.
  final num? maxBots;
  /// Minimum active users in bot.
  final num? minBotActiveUsers;
  /// Maximum active users in bot.
  final num? maxBotActiveUsers;

  const CategoryTelegramParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.spam,
    this.password,
    this.premium,
    this.premiumExpiration,
    this.premiumExpirationPeriod,
    this.country,
    this.notCountry,
    this.daybreak,
    this.minChannels,
    this.maxChannels,
    this.minChats,
    this.maxChats,
    this.minConversations,
    this.maxConversations,
    this.minAdmin,
    this.maxAdmin,
    this.minAdminSub,
    this.maxAdminSub,
    this.digMin,
    this.digMax,
    this.minContacts,
    this.maxContacts,
    this.minStars,
    this.maxStars,
    this.birthday,
    this.birthdayPeriod,
    this.birthdayAfter,
    this.birthdayAfterPeriod,
    this.minId,
    this.maxId,
    this.allowGeoSpamblock,
    this.minGifts,
    this.maxGifts,
    this.minNftGifts,
    this.maxNftGifts,
    this.minGiftsStars,
    this.maxGiftsStars,
    this.minGiftsConvertStars,
    this.maxGiftsConvertStars,
    this.dcId,
    this.notDcId,
    this.email,
    this.minBots,
    this.maxBots,
    this.minBotActiveUsers,
    this.maxBotActiveUsers,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'spam': spam?.value,
      'password': password?.value,
      'premium': premium?.value,
      'premium_expiration': premiumExpiration,
      'premium_expiration_period': premiumExpirationPeriod?.value,
      'country[]': country,
      'not_country[]': notCountry,
      'daybreak': daybreak,
      'min_channels': minChannels,
      'max_channels': maxChannels,
      'min_chats': minChats,
      'max_chats': maxChats,
      'min_conversations': minConversations,
      'max_conversations': maxConversations,
      'min_admin': minAdmin,
      'max_admin': maxAdmin,
      'min_admin_sub': minAdminSub,
      'max_admin_sub': maxAdminSub,
      'dig_min': digMin,
      'dig_max': digMax,
      'min_contacts': minContacts,
      'max_contacts': maxContacts,
      'min_stars': minStars,
      'max_stars': maxStars,
      'birthday': birthday,
      'birthday_period': birthdayPeriod?.value,
      'birthday_after': birthdayAfter,
      'birthday_after_period': birthdayAfterPeriod?.value,
      'min_id': minId,
      'max_id': maxId,
      'allow_geo_spamblock': allowGeoSpamblock,
      'min_gifts': minGifts,
      'max_gifts': maxGifts,
      'min_nft_gifts': minNftGifts,
      'max_nft_gifts': maxNftGifts,
      'min_gifts_stars': minGiftsStars,
      'max_gifts_stars': maxGiftsStars,
      'min_gifts_convert_stars': minGiftsConvertStars,
      'max_gifts_convert_stars': maxGiftsConvertStars,
      'dc_id[]': dcId,
      'not_dc_id[]': notDcId,
      'email': email?.value,
      'min_bots': minBots,
      'max_bots': maxBots,
      'min_bot_active_users': minBotActiveUsers,
      'max_bot_active_users': maxBotActiveUsers,
    };
  }
}

class CategoryTelegramResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryTelegramResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryTelegramResponse.fromJson(Map<String, dynamic> json) => CategoryTelegramResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategorySupercellParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Eg? eg;
  final Tel? tel;
  /// Minimum Brawl Stars level.
  final num? brawlLevelMin;
  /// Maximum Brawl Stars level.
  final num? brawlLevelMax;
  /// Minimum number of Brawl Stars trophies.
  final num? brawlCupMin;
  /// Maximum number of Brawl Stars trophies.
  final num? brawlCupMax;
  /// Minimum number of Brawl Stars wins.
  final num? brawlWinsMin;
  /// Maximum number of Brawl Stars wins.
  final num? brawlWinsMax;
  final BrawlPass? brawlPass;
  /// List of brawlers.
  final List<String>? brawler;
  /// Minimum number of brawlers.
  final num? brawlersMin;
  /// Maximum number of brawlers.
  final num? brawlersMax;
  /// Minimum number of legendary brawlers.
  final num? legendaryBrawlersMin;
  /// Maximum number of legendary brawlers.
  final num? legendaryBrawlersMax;
  /// Minimum Clash Royale level.
  final num? royaleLevelMin;
  /// Maximum Clash Royale level.
  final num? royaleLevelMax;
  /// Minimum number of Clash Royale trophies.
  final num? royaleCupMin;
  /// Maximum number of Clash Royale trophies.
  final num? royaleCupMax;
  /// Minimum number of Clash Royale wins.
  final num? royaleWinsMin;
  /// Maximum number of Clash Royale wins.
  final num? royaleWinsMax;
  /// Minimum King level in Clash Royale.
  final num? kingLevelMin;
  /// Maximum King level in Clash Royale.
  final num? kingLevelMax;
  final RoyalePass? royalePass;
  /// Minimum Clash of Clans level.
  final num? clashLevelMin;
  /// Maximum Clash of Clans level.
  final num? clashLevelMax;
  /// Minimum number of Clash of Clans trophies.
  final num? clashCupMin;
  /// Maximum number of Clash of Clans trophies.
  final num? clashCupMax;
  /// Minimum number of Clash of Clans wins.
  final num? clashWinsMin;
  /// Maximum number of Clash of Clans wins.
  final num? clashWinsMax;
  final ClashPass? clashPass;
  /// Minimum total heroes level count in Clash of Clans.
  final num? totalHeroesLevelMin;
  /// Maximum total heroes level count in Clash of Clans.
  final num? totalHeroesLevelMax;
  /// Minimum total troops level count in Clash of Clans.
  final num? totalTroopsLevelMin;
  /// Maximum total troops level count in Clash of Clans.
  final num? totalTroopsLevelMax;
  /// Minimum total spells level count in Clash of Clans.
  final num? totalSpellsLevelMin;
  /// Maximum total spells level count in Clash of Clans.
  final num? totalSpellsLevelMax;
  /// Minimum total builder village heroes level count in Clash of Clans.
  final num? totalBuilderHeroesLevelMin;
  /// Maximum total builder village heroes level count in Clash of Clans.
  final num? totalBuilderHeroesLevelMax;
  /// Minimum total builder village troops level count in Clash of Clans.
  final num? totalBuilderTroopsLevelMin;
  /// Maximum total builder village troops level count in Clash of Clans.
  final num? totalBuilderTroopsLevelMax;
  /// Minimum level of town hall.
  final num? townHallLevelMin;
  /// Maximum level of town hall.
  final num? townHallLevelMax;
  /// Minimum level of builder hall.
  final num? builderHallLevelMin;
  /// Maximum level of builder hall.
  final num? builderHallLevelMax;
  /// Minimum number of builder hall cups.
  final num? builderHallCupMin;
  /// Maximum number of builder hall cups.
  final num? builderHallCupMax;
  /// Minimum account creation year (e.g. 2023).
  final num? creationYearMin;
  /// Maximum account creation year (e.g. 2024).
  final num? creationYearMax;

  const CategorySupercellParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.daybreak,
    this.emailType,
    this.itemDomain,
    this.eg,
    this.tel,
    this.brawlLevelMin,
    this.brawlLevelMax,
    this.brawlCupMin,
    this.brawlCupMax,
    this.brawlWinsMin,
    this.brawlWinsMax,
    this.brawlPass,
    this.brawler,
    this.brawlersMin,
    this.brawlersMax,
    this.legendaryBrawlersMin,
    this.legendaryBrawlersMax,
    this.royaleLevelMin,
    this.royaleLevelMax,
    this.royaleCupMin,
    this.royaleCupMax,
    this.royaleWinsMin,
    this.royaleWinsMax,
    this.kingLevelMin,
    this.kingLevelMax,
    this.royalePass,
    this.clashLevelMin,
    this.clashLevelMax,
    this.clashCupMin,
    this.clashCupMax,
    this.clashWinsMin,
    this.clashWinsMax,
    this.clashPass,
    this.totalHeroesLevelMin,
    this.totalHeroesLevelMax,
    this.totalTroopsLevelMin,
    this.totalTroopsLevelMax,
    this.totalSpellsLevelMin,
    this.totalSpellsLevelMax,
    this.totalBuilderHeroesLevelMin,
    this.totalBuilderHeroesLevelMax,
    this.totalBuilderTroopsLevelMin,
    this.totalBuilderTroopsLevelMax,
    this.townHallLevelMin,
    this.townHallLevelMax,
    this.builderHallLevelMin,
    this.builderHallLevelMax,
    this.builderHallCupMin,
    this.builderHallCupMax,
    this.creationYearMin,
    this.creationYearMax,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'daybreak': daybreak,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'eg': eg?.value,
      'tel': tel?.value,
      'brawl_level_min': brawlLevelMin,
      'brawl_level_max': brawlLevelMax,
      'brawl_cup_min': brawlCupMin,
      'brawl_cup_max': brawlCupMax,
      'brawl_wins_min': brawlWinsMin,
      'brawl_wins_max': brawlWinsMax,
      'brawl_pass': brawlPass?.value,
      'brawler[]': brawler,
      'brawlers_min': brawlersMin,
      'brawlers_max': brawlersMax,
      'legendary_brawlers_min': legendaryBrawlersMin,
      'legendary_brawlers_max': legendaryBrawlersMax,
      'royale_level_min': royaleLevelMin,
      'royale_level_max': royaleLevelMax,
      'royale_cup_min': royaleCupMin,
      'royale_cup_max': royaleCupMax,
      'royale_wins_min': royaleWinsMin,
      'royale_wins_max': royaleWinsMax,
      'king_level_min': kingLevelMin,
      'king_level_max': kingLevelMax,
      'royale_pass': royalePass?.value,
      'clash_level_min': clashLevelMin,
      'clash_level_max': clashLevelMax,
      'clash_cup_min': clashCupMin,
      'clash_cup_max': clashCupMax,
      'clash_wins_min': clashWinsMin,
      'clash_wins_max': clashWinsMax,
      'clash_pass': clashPass?.value,
      'total_heroes_level_min': totalHeroesLevelMin,
      'total_heroes_level_max': totalHeroesLevelMax,
      'total_troops_level_min': totalTroopsLevelMin,
      'total_troops_level_max': totalTroopsLevelMax,
      'total_spells_level_min': totalSpellsLevelMin,
      'total_spells_level_max': totalSpellsLevelMax,
      'total_builder_heroes_level_min': totalBuilderHeroesLevelMin,
      'total_builder_heroes_level_max': totalBuilderHeroesLevelMax,
      'total_builder_troops_level_min': totalBuilderTroopsLevelMin,
      'total_builder_troops_level_max': totalBuilderTroopsLevelMax,
      'town_hall_level_min': townHallLevelMin,
      'town_hall_level_max': townHallLevelMax,
      'builder_hall_level_min': builderHallLevelMin,
      'builder_hall_level_max': builderHallLevelMax,
      'builder_hall_cup_min': builderHallCupMin,
      'builder_hall_cup_max': builderHallCupMax,
      'creation_year_min': creationYearMin,
      'creation_year_max': creationYearMax,
    };
  }
}

class CategorySupercellResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategorySupercellResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategorySupercellResponse.fromJson(Map<String, dynamic> json) => CategorySupercellResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryEaParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  /// List of games.
  final List<String>? game;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  /// Minimum count of games.
  final num? gmin;
  /// Maximum count of games.
  final num? gmax;
  /// Minimum rank points in Apex Legends.
  final num? alRankMin;
  /// Maximum rank points in Apex Legends.
  final num? alRankMax;
  /// Minimum level in Apex Legends.
  final num? alLevelMin;
  /// Maximum level in Apex Legends.
  final num? alLevelMax;
  final HasBan? hasBan;
  final XboxConnected? xboxConnected;
  final SteamConnected? steamConnected;
  final PsnConnected? psnConnected;
  final Subscription? subscription;
  /// Length of subscription.
  final num? subscriptionLength;
  final SubscriptionPeriod? subscriptionPeriod;
  /// List of minimum hours played by game.
  final Map<String, num>? hoursPlayed;
  /// List of maximum hours played by game.
  final Map<String, num>? hoursPlayedMax;
  final Transactions? transactions;

  const CategoryEaParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.game,
    this.country,
    this.notCountry,
    this.gmin,
    this.gmax,
    this.alRankMin,
    this.alRankMax,
    this.alLevelMin,
    this.alLevelMax,
    this.hasBan,
    this.xboxConnected,
    this.steamConnected,
    this.psnConnected,
    this.subscription,
    this.subscriptionLength,
    this.subscriptionPeriod,
    this.hoursPlayed,
    this.hoursPlayedMax,
    this.transactions,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'game[]': game,
      'country[]': country,
      'not_country[]': notCountry,
      'gmin': gmin,
      'gmax': gmax,
      'al_rank_min': alRankMin,
      'al_rank_max': alRankMax,
      'al_level_min': alLevelMin,
      'al_level_max': alLevelMax,
      'has_ban': hasBan?.value,
      'xbox_connected': xboxConnected?.value,
      'steam_connected': steamConnected?.value,
      'psn_connected': psnConnected?.value,
      'subscription': subscription?.value,
      'subscription_length': subscriptionLength,
      'subscription_period': subscriptionPeriod?.value,
      'hours_played': hoursPlayed,
      'hours_played_max': hoursPlayedMax,
      'transactions': transactions?.value,
    };
  }
}

class CategoryEaResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryEaResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryEaResponse.fromJson(Map<String, dynamic> json) => CategoryEaResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryWotParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Tel? tel;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// Minimum number of battles.
  final num? battlesMin;
  /// Maximum number of battles.
  final num? battlesMax;
  /// Minimum number of gold.
  final num? goldMin;
  /// Maximum number of gold.
  final num? goldMax;
  /// Minimum number of silver.
  final num? silverMin;
  /// Maximum number of silver.
  final num? silverMax;
  /// Minimum number of top tanks.
  final num? topMin;
  /// Maximum number of top tanks.
  final num? topMax;
  /// Minimum number of premium tanks.
  final num? premMin;
  /// Maximum number of premium tanks.
  final num? premMax;
  /// Minimum number of top premium tanks.
  final num? topPremMin;
  /// Maximum number of top premium tanks.
  final num? topPremMax;
  /// Minimum number of wins.
  final num? winPmin;
  /// Maximum number of wins.
  final num? winPmax;
  /// List of tanks.
  final List<num>? tank;
  /// Region.
  final List<String>? region;
  /// Exclude region.
  final List<String>? notRegion;
  final Premium? premium;
  /// When premium subscription will be active.
  final num? premiumExpiration;
  final PremiumExpirationPeriod? premiumExpirationPeriod;
  final Clan? clan;
  /// List of allowed clan role.
  final List<String>? clanRole;
  /// List of disallowed clan role.
  final List<String>? notClanRole;
  /// Minimum number of gold in clan treasure.
  final num? clanGoldMin;
  /// Maximum number of gold in clan treasure.
  final num? clanGoldMax;
  /// Minimum number of credits in clan treasure.
  final num? clanCreditsMin;
  /// Maximum number of credits in clan treasure.
  final num? clanCreditsMax;
  /// Minimum number of crystal in clan treasure.
  final num? clanCrystalMin;
  /// Maximum number of crystal in clan treasure.
  final num? clanCrystalMax;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;

  const CategoryWotParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.tel,
    this.daybreak,
    this.battlesMin,
    this.battlesMax,
    this.goldMin,
    this.goldMax,
    this.silverMin,
    this.silverMax,
    this.topMin,
    this.topMax,
    this.premMin,
    this.premMax,
    this.topPremMin,
    this.topPremMax,
    this.winPmin,
    this.winPmax,
    this.tank,
    this.region,
    this.notRegion,
    this.premium,
    this.premiumExpiration,
    this.premiumExpirationPeriod,
    this.clan,
    this.clanRole,
    this.notClanRole,
    this.clanGoldMin,
    this.clanGoldMax,
    this.clanCreditsMin,
    this.clanCreditsMax,
    this.clanCrystalMin,
    this.clanCrystalMax,
    this.country,
    this.notCountry,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'tel': tel?.value,
      'daybreak': daybreak,
      'battles_min': battlesMin,
      'battles_max': battlesMax,
      'gold_min': goldMin,
      'gold_max': goldMax,
      'silver_min': silverMin,
      'silver_max': silverMax,
      'top_min': topMin,
      'top_max': topMax,
      'prem_min': premMin,
      'prem_max': premMax,
      'top_prem_min': topPremMin,
      'top_prem_max': topPremMax,
      'win_pmin': winPmin,
      'win_pmax': winPmax,
      'tank[]': tank,
      'region[]': region,
      'not_region[]': notRegion,
      'premium': premium?.value,
      'premium_expiration': premiumExpiration,
      'premium_expiration_period': premiumExpirationPeriod?.value,
      'clan': clan?.value,
      'clan_role[]': clanRole,
      'not_clan_role[]': notClanRole,
      'clan_gold_min': clanGoldMin,
      'clan_gold_max': clanGoldMax,
      'clan_credits_min': clanCreditsMin,
      'clan_credits_max': clanCreditsMax,
      'clan_crystal_min': clanCrystalMin,
      'clan_crystal_max': clanCrystalMax,
      'country[]': country,
      'not_country[]': notCountry,
    };
  }
}

class CategoryWotResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryWotResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryWotResponse.fromJson(Map<String, dynamic> json) => CategoryWotResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryWotBlitzParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Tel? tel;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// Minimum number of battles.
  final num? battlesMin;
  /// Maximum number of battles.
  final num? battlesMax;
  /// Minimum number of gold.
  final num? goldMin;
  /// Maximum number of gold.
  final num? goldMax;
  /// Minimum number of silver.
  final num? silverMin;
  /// Maximum number of silver.
  final num? silverMax;
  /// Minimum number of top tanks.
  final num? topMin;
  /// Maximum number of top tanks.
  final num? topMax;
  /// Minimum number of premium tanks.
  final num? premMin;
  /// Maximum number of premium tanks.
  final num? premMax;
  /// Minimum number of top premium tanks.
  final num? topPremMin;
  /// Maximum number of top premium tanks.
  final num? topPremMax;
  /// Minimum number of wins.
  final num? winPmin;
  /// Maximum number of wins.
  final num? winPmax;
  /// List of tanks.
  final List<num>? tank;
  /// Region.
  final List<String>? region;
  /// Exclude region.
  final List<String>? notRegion;
  final Premium? premium;
  /// When premium subscription will be active.
  final num? premiumExpiration;
  final PremiumExpirationPeriod? premiumExpirationPeriod;
  final Clan? clan;
  /// List of allowed clan role.
  final List<String>? clanRole;
  /// List of disallowed clan role.
  final List<String>? notClanRole;
  /// Minimum number of gold in clan treasure.
  final num? clanGoldMin;
  /// Maximum number of gold in clan treasure.
  final num? clanGoldMax;
  /// Minimum number of credits in clan treasure.
  final num? clanCreditsMin;
  /// Maximum number of credits in clan treasure.
  final num? clanCreditsMax;
  /// Minimum number of crystal in clan treasure.
  final num? clanCrystalMin;
  /// Maximum number of crystal in clan treasure.
  final num? clanCrystalMax;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;

  const CategoryWotBlitzParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.tel,
    this.daybreak,
    this.battlesMin,
    this.battlesMax,
    this.goldMin,
    this.goldMax,
    this.silverMin,
    this.silverMax,
    this.topMin,
    this.topMax,
    this.premMin,
    this.premMax,
    this.topPremMin,
    this.topPremMax,
    this.winPmin,
    this.winPmax,
    this.tank,
    this.region,
    this.notRegion,
    this.premium,
    this.premiumExpiration,
    this.premiumExpirationPeriod,
    this.clan,
    this.clanRole,
    this.notClanRole,
    this.clanGoldMin,
    this.clanGoldMax,
    this.clanCreditsMin,
    this.clanCreditsMax,
    this.clanCrystalMin,
    this.clanCrystalMax,
    this.country,
    this.notCountry,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'tel': tel?.value,
      'daybreak': daybreak,
      'battles_min': battlesMin,
      'battles_max': battlesMax,
      'gold_min': goldMin,
      'gold_max': goldMax,
      'silver_min': silverMin,
      'silver_max': silverMax,
      'top_min': topMin,
      'top_max': topMax,
      'prem_min': premMin,
      'prem_max': premMax,
      'top_prem_min': topPremMin,
      'top_prem_max': topPremMax,
      'win_pmin': winPmin,
      'win_pmax': winPmax,
      'tank[]': tank,
      'region[]': region,
      'not_region[]': notRegion,
      'premium': premium?.value,
      'premium_expiration': premiumExpiration,
      'premium_expiration_period': premiumExpirationPeriod?.value,
      'clan': clan?.value,
      'clan_role[]': clanRole,
      'not_clan_role[]': notClanRole,
      'clan_gold_min': clanGoldMin,
      'clan_gold_max': clanGoldMax,
      'clan_credits_min': clanCreditsMin,
      'clan_credits_max': clanCreditsMax,
      'clan_crystal_min': clanCrystalMin,
      'clan_crystal_max': clanCrystalMax,
      'country[]': country,
      'not_country[]': notCountry,
    };
  }
}

class CategoryWotBlitzResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryWotBlitzResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryWotBlitzResponse.fromJson(Map<String, dynamic> json) => CategoryWotBlitzResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryGiftsParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  final Subscription? subscription;
  /// Length of subscription.
  final num? subscriptionLength;
  final SubscriptionPeriod? subscriptionPeriod;

  const CategoryGiftsParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.subscription,
    this.subscriptionLength,
    this.subscriptionPeriod,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'subscription': subscription?.value,
      'subscription_length': subscriptionLength,
      'subscription_period': subscriptionPeriod?.value,
    };
  }
}

class CategoryGiftsResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryGiftsResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryGiftsResponse.fromJson(Map<String, dynamic> json) => CategoryGiftsResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryEpicGamesParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Eg? eg;
  /// List of games.
  final List<String>? game;
  final ChangeEmail? changeEmail;
  /// Has Rocket League purchases.
  final bool? rlPurchases;
  /// Minimum epic wallet balance.
  final num? balanceMin;
  /// Maximum epic wallet balance.
  final num? balanceMax;
  /// Minimum rewards balance.
  final num? rewardsBalanceMin;
  /// Maximum rewards balance.
  final num? rewardsBalanceMax;
  /// Minimum number of games.
  final num? gmin;
  /// Maximum number of games.
  final num? gmax;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// List of minimum hours played by game.
  final Map<String, num>? hoursPlayed;
  /// List of maximum hours played by game.
  final Map<String, num>? hoursPlayedMax;

  const CategoryEpicGamesParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.eg,
    this.game,
    this.changeEmail,
    this.rlPurchases,
    this.balanceMin,
    this.balanceMax,
    this.rewardsBalanceMin,
    this.rewardsBalanceMax,
    this.gmin,
    this.gmax,
    this.country,
    this.notCountry,
    this.daybreak,
    this.hoursPlayed,
    this.hoursPlayedMax,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'eg': eg?.value,
      'game[]': game,
      'change_email': changeEmail?.value,
      'rl_purchases': rlPurchases,
      'balance_min': balanceMin,
      'balance_max': balanceMax,
      'rewards_balance_min': rewardsBalanceMin,
      'rewards_balance_max': rewardsBalanceMax,
      'gmin': gmin,
      'gmax': gmax,
      'country[]': country,
      'not_country[]': notCountry,
      'daybreak': daybreak,
      'hours_played': hoursPlayed,
      'hours_played_max': hoursPlayedMax,
    };
  }
}

class CategoryEpicGamesResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryEpicGamesResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryEpicGamesResponse.fromJson(Map<String, dynamic> json) => CategoryEpicGamesResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryEscapeFromTarkovParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Region? region;
  /// List of versions.
  final List<String>? version;
  /// How old is the account.
  final num? reg;
  final RegPeriod? regPeriod;
  /// Minimum level.
  final num? levelMin;
  /// Maximum level.
  final num? levelMax;
  final Pve? pve;
  final Side? side;

  const CategoryEscapeFromTarkovParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.region,
    this.version,
    this.reg,
    this.regPeriod,
    this.levelMin,
    this.levelMax,
    this.pve,
    this.side,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'region': region?.value,
      'version[]': version,
      'reg': reg,
      'reg_period': regPeriod?.value,
      'level_min': levelMin,
      'level_max': levelMax,
      'pve': pve?.value,
      'side': side?.value,
    };
  }
}

class CategoryEscapeFromTarkovResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryEscapeFromTarkovResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryEscapeFromTarkovResponse.fromJson(Map<String, dynamic> json) => CategoryEscapeFromTarkovResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategorySocialClubParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// Minimum number of Social Club level.
  final num? levelMin;
  /// Maximum number of Social Club level.
  final num? levelMax;
  /// Minimum number of GTA V cash.
  final num? cashMin;
  /// Maximum number of GTA V cash.
  final num? cashMax;
  /// Minimum number of GTA V bank cash.
  final num? bankCashMin;
  /// Maximum number of GTA V bank cash.
  final num? bankCashMax;
  /// List of games.
  final List<String>? game;

  const CategorySocialClubParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.daybreak,
    this.levelMin,
    this.levelMax,
    this.cashMin,
    this.cashMax,
    this.bankCashMin,
    this.bankCashMax,
    this.game,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'daybreak': daybreak,
      'level_min': levelMin,
      'level_max': levelMax,
      'cash_min': cashMin,
      'cash_max': cashMax,
      'bank_cash_min': bankCashMin,
      'bank_cash_max': bankCashMax,
      'game[]': game,
    };
  }
}

class CategorySocialClubResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategorySocialClubResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategorySocialClubResponse.fromJson(Map<String, dynamic> json) => CategorySocialClubResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryUplayParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  /// List of games.
  final List<String>? game;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// Minimum count of games.
  final num? gmin;
  /// Maximum count of games.
  final num? gmax;
  final Subscription? subscription;
  /// Length of subscription.
  final num? subscriptionLength;
  final SubscriptionPeriod? subscriptionPeriod;
  /// Minimum level in Tom Clancy's Rainbow Six Siege.
  final num? r6LevelMin;
  /// Maximum level in Tom Clancy's Rainbow Six Siege.
  final num? r6LevelMax;
  /// Minimum rank points in Tom Clancy's Rainbow Six Siege.
  final num? r6RankMin;
  /// Maximum rank points in Tom Clancy's Rainbow Six Siege.
  final num? r6RankMax;
  /// Minimum count of operators in Tom Clancy's Rainbow Six Siege.
  final num? r6OperatorsMin;
  /// Maximum count of operators in Tom Clancy's Rainbow Six Siege.
  final num? r6OperatorsMax;
  final R6Ban? r6Ban;
  /// Minimum number of skins in Tom Clancy's Rainbow Six Siege.
  final num? r6Smin;
  /// Maximum number of skins in Tom Clancy's Rainbow Six Siege.
  final num? r6Smax;
  /// List of weapon skins in Tom Clancy's Rainbow Six Siege.
  final List<String>? r6Skin;
  /// List of operators in Tom Clancy's Rainbow Six Siege.
  final List<String>? r6Operator;
  final XboxConnected? xboxConnected;
  final PsnConnected? psnConnected;
  final SteamConnected? steamConnected;
  /// Minimum balance.
  final num? balanceMin;
  /// Maximum balance.
  final num? balanceMax;
  final Transactions? transactions;
  /// How old is the account.
  final num? reg;
  final RegPeriod? regPeriod;

  const CategoryUplayParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.game,
    this.country,
    this.notCountry,
    this.daybreak,
    this.gmin,
    this.gmax,
    this.subscription,
    this.subscriptionLength,
    this.subscriptionPeriod,
    this.r6LevelMin,
    this.r6LevelMax,
    this.r6RankMin,
    this.r6RankMax,
    this.r6OperatorsMin,
    this.r6OperatorsMax,
    this.r6Ban,
    this.r6Smin,
    this.r6Smax,
    this.r6Skin,
    this.r6Operator,
    this.xboxConnected,
    this.psnConnected,
    this.steamConnected,
    this.balanceMin,
    this.balanceMax,
    this.transactions,
    this.reg,
    this.regPeriod,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'game[]': game,
      'country[]': country,
      'not_country[]': notCountry,
      'daybreak': daybreak,
      'gmin': gmin,
      'gmax': gmax,
      'subscription': subscription?.value,
      'subscription_length': subscriptionLength,
      'subscription_period': subscriptionPeriod?.value,
      'r6_level_min': r6LevelMin,
      'r6_level_max': r6LevelMax,
      'r6_rank_min': r6RankMin,
      'r6_rank_max': r6RankMax,
      'r6_operators_min': r6OperatorsMin,
      'r6_operators_max': r6OperatorsMax,
      'r6_ban': r6Ban?.value,
      'r6_smin': r6Smin,
      'r6_smax': r6Smax,
      'r6_skin[]': r6Skin,
      'r6_operator[]': r6Operator,
      'xbox_connected': xboxConnected?.value,
      'psn_connected': psnConnected?.value,
      'steam_connected': steamConnected?.value,
      'balance_min': balanceMin,
      'balance_max': balanceMax,
      'transactions': transactions?.value,
      'reg': reg,
      'reg_period': regPeriod?.value,
    };
  }
}

class CategoryUplayResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryUplayResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryUplayResponse.fromJson(Map<String, dynamic> json) => CategoryUplayResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryDiscordParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Tel? tel;
  final Nitro? nitro;
  /// Nitro type.
  final List<String>? nitroType;
  /// Length of nitro.
  final num? nitroLength;
  final NitroPeriod? nitroPeriod;
  /// Minimum number of boosts.
  final num? boostsMin;
  /// Maximum number of boosts.
  final num? boostsMax;
  final Billing? billing;
  final Gifts? gifts;
  final Transactions? transactions;
  /// List of badges.
  final List<String>? badge;
  /// List of account conditions.
  final List<String>? condition;
  /// Minimum number of chats.
  final num? chatMin;
  /// Maximum number of chats.
  final num? chatMax;
  /// Minimum number of subscribers in server, where account is administrator/owner.
  final num? minAdminMembers;
  /// Maximum number of subscribers in server, where account is administrator/owner.
  final num? maxAdminMembers;
  /// Minimum number of servers, where account is administrator/owner.
  final num? minAdmin;
  /// Maximum number of servers, where account is administrator/owner.
  final num? maxAdmin;
  /// How old is the account.
  final num? reg;
  final RegPeriod? regPeriod;
  /// List of languages.
  final List<String>? language;
  /// List of languages that won't be included.
  final List<String>? notLanguage;
  final Clans? clans;
  /// Minimum number of clans, where account is administrator.
  final num? minAdminClans;
  /// Maximum number of clans, where account is administrator.
  final num? maxAdminClans;
  /// Minimum number of clans, where account is owner.
  final num? minOwnerClans;
  /// Maximum number of clans, where account is owner.
  final num? maxOwnerClans;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  /// Minimum count of servers.
  final num? minServers;
  /// Maximum count of servers.
  final num? maxServers;
  final N2fa? n2fa;
  /// Minimum number of Nitro full credits.
  final num? minFullCredits;
  /// Maximum number of Nitro full credits.
  final num? maxFullCredits;
  /// Minimum number of Nitro basic credits.
  final num? minBasicCredits;
  /// Maximum number of Nitro basic credits.
  final num? maxBasicCredits;
  /// Minimum number of Discord Orbs.
  final num? minOrbs;
  /// Maximum number of Discord Orbs.
  final num? maxOrbs;

  const CategoryDiscordParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.tel,
    this.nitro,
    this.nitroType,
    this.nitroLength,
    this.nitroPeriod,
    this.boostsMin,
    this.boostsMax,
    this.billing,
    this.gifts,
    this.transactions,
    this.badge,
    this.condition,
    this.chatMin,
    this.chatMax,
    this.minAdminMembers,
    this.maxAdminMembers,
    this.minAdmin,
    this.maxAdmin,
    this.reg,
    this.regPeriod,
    this.language,
    this.notLanguage,
    this.clans,
    this.minAdminClans,
    this.maxAdminClans,
    this.minOwnerClans,
    this.maxOwnerClans,
    this.country,
    this.notCountry,
    this.minServers,
    this.maxServers,
    this.n2fa,
    this.minFullCredits,
    this.maxFullCredits,
    this.minBasicCredits,
    this.maxBasicCredits,
    this.minOrbs,
    this.maxOrbs,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'tel': tel?.value,
      'nitro': nitro?.value,
      'nitro_type[]': nitroType,
      'nitro_length': nitroLength,
      'nitro_period': nitroPeriod?.value,
      'boosts_min': boostsMin,
      'boosts_max': boostsMax,
      'billing': billing?.value,
      'gifts': gifts?.value,
      'transactions': transactions?.value,
      'badge[]': badge,
      'condition[]': condition,
      'chat_min': chatMin,
      'chat_max': chatMax,
      'min_admin_members': minAdminMembers,
      'max_admin_members': maxAdminMembers,
      'min_admin': minAdmin,
      'max_admin': maxAdmin,
      'reg': reg,
      'reg_period': regPeriod?.value,
      'language[]': language,
      'not_language[]': notLanguage,
      'clans': clans?.value,
      'min_admin_clans': minAdminClans,
      'max_admin_clans': maxAdminClans,
      'min_owner_clans': minOwnerClans,
      'max_owner_clans': maxOwnerClans,
      'country[]': country,
      'not_country[]': notCountry,
      'min_servers': minServers,
      'max_servers': maxServers,
      '2fa': n2fa?.value,
      'min_full_credits': minFullCredits,
      'max_full_credits': maxFullCredits,
      'min_basic_credits': minBasicCredits,
      'max_basic_credits': maxBasicCredits,
      'min_orbs': minOrbs,
      'max_orbs': maxOrbs,
    };
  }
}

class CategoryDiscordResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryDiscordResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryDiscordResponse.fromJson(Map<String, dynamic> json) => CategoryDiscordResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryTikTokParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Tel? tel;
  /// How old is the account.
  final num? reg;
  final RegPeriod? regPeriod;
  /// Minimum number of followers.
  final num? followersMin;
  /// Maximum number of followers.
  final num? followersMax;
  /// Minimum number of posts.
  final num? postMin;
  /// Maximum number of posts.
  final num? postMax;
  /// Minimum number of likes.
  final num? likeMin;
  /// Maximum number of likes.
  final num? likeMax;
  /// Minimum number of coins.
  final num? coinsMin;
  /// Maximum number of coins.
  final num? coinsMax;
  final CookieLogin? cookieLogin;
  final Verified? verified;
  final Email? email;

  const CategoryTikTokParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.tel,
    this.reg,
    this.regPeriod,
    this.followersMin,
    this.followersMax,
    this.postMin,
    this.postMax,
    this.likeMin,
    this.likeMax,
    this.coinsMin,
    this.coinsMax,
    this.cookieLogin,
    this.verified,
    this.email,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'tel': tel?.value,
      'reg': reg,
      'reg_period': regPeriod?.value,
      'followers_min': followersMin,
      'followers_max': followersMax,
      'post_min': postMin,
      'post_max': postMax,
      'like_min': likeMin,
      'like_max': likeMax,
      'coins_min': coinsMin,
      'coins_max': coinsMax,
      'cookie_login': cookieLogin?.value,
      'verified': verified?.value,
      'email': email?.value,
    };
  }
}

class CategoryTikTokResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryTikTokResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryTikTokResponse.fromJson(Map<String, dynamic> json) => CategoryTikTokResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryInstagramParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Tel? tel;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  final Cookies? cookies;
  final LoginWithoutCookies? loginWithoutCookies;
  /// Minimum number of followers.
  final num? followersMin;
  /// Maximum number of followers.
  final num? followersMax;
  /// Minimum number of posts.
  final num? postMin;
  /// Maximum number of posts.
  final num? postMax;
  /// How old is the account.
  final num? reg;
  final RegPeriod? regPeriod;

  const CategoryInstagramParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.tel,
    this.country,
    this.notCountry,
    this.cookies,
    this.loginWithoutCookies,
    this.followersMin,
    this.followersMax,
    this.postMin,
    this.postMax,
    this.reg,
    this.regPeriod,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'tel': tel?.value,
      'country[]': country,
      'not_country[]': notCountry,
      'cookies': cookies?.value,
      'login_without_cookies': loginWithoutCookies?.value,
      'followers_min': followersMin,
      'followers_max': followersMax,
      'post_min': postMin,
      'post_max': postMax,
      'reg': reg,
      'reg_period': regPeriod?.value,
    };
  }
}

class CategoryInstagramResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryInstagramResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryInstagramResponse.fromJson(Map<String, dynamic> json) => CategoryInstagramResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryBattleNetParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  final Eg? eg;
  /// List of games.
  final List<num>? game;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  final Tel? tel;
  final EditBtag? editBtag;
  final ChangeableFn? changeableFn;
  final RealId? realId;
  final ParentControl? parentControl;
  final NoBans? noBans;
  /// Minimum balance.
  final num? balanceMin;
  /// Maximum balance.
  final num? balanceMax;

  const CategoryBattleNetParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.eg,
    this.game,
    this.daybreak,
    this.country,
    this.notCountry,
    this.tel,
    this.editBtag,
    this.changeableFn,
    this.realId,
    this.parentControl,
    this.noBans,
    this.balanceMin,
    this.balanceMax,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'eg': eg?.value,
      'game[]': game,
      'daybreak': daybreak,
      'country[]': country,
      'not_country[]': notCountry,
      'tel': tel?.value,
      'edit_btag': editBtag?.value,
      'changeable_fn': changeableFn?.value,
      'real_id': realId?.value,
      'parent_control': parentControl?.value,
      'no_bans': noBans?.value,
      'balance_min': balanceMin,
      'balance_max': balanceMax,
    };
  }
}

class CategoryBattleNetResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryBattleNetResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryBattleNetResponse.fromJson(Map<String, dynamic> json) => CategoryBattleNetResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryChatGPTParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Email type.
  final List<String>? emailType;
  /// Domain of native/autoreg email.
  final String? itemDomain;
  /// List of allowed subscriptions.
  final List<String>? subscription;
  /// Length of subscription.
  final num? subscriptionLength;
  final SubscriptionPeriod? subscriptionPeriod;
  final Autorenewal? autorenewal;
  final Tel? tel;
  final Transactions? transactions;
  /// How old is the account.
  final num? reg;
  final RegPeriod? regPeriod;
  /// List of allowed tiers.
  final List<String>? openaiTier;
  /// Minimum OpenAI credit balance.
  final num? openaiBalanceMin;
  /// Maximum OpenAI credit balance.
  final num? openaiBalanceMax;

  const CategoryChatGPTParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.emailType,
    this.itemDomain,
    this.subscription,
    this.subscriptionLength,
    this.subscriptionPeriod,
    this.autorenewal,
    this.tel,
    this.transactions,
    this.reg,
    this.regPeriod,
    this.openaiTier,
    this.openaiBalanceMin,
    this.openaiBalanceMax,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email_type[]': emailType,
      'item_domain': itemDomain,
      'subscription[]': subscription,
      'subscription_length': subscriptionLength,
      'subscription_period': subscriptionPeriod?.value,
      'autorenewal': autorenewal?.value,
      'tel': tel?.value,
      'transactions': transactions?.value,
      'reg': reg,
      'reg_period': regPeriod?.value,
      'openai_tier[]': openaiTier,
      'openai_balance_min': openaiBalanceMin,
      'openai_balance_max': openaiBalanceMax,
    };
  }
}

class CategoryChatGPTResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryChatGPTResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryChatGPTResponse.fromJson(Map<String, dynamic> json) => CategoryChatGPTResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryVpnParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// List of allowed VPN services.
  final List<String>? service;
  /// Length of subscription.
  final num? subscriptionLength;
  final SubscriptionPeriod? subscriptionPeriod;
  final Autorenewal? autorenewal;

  const CategoryVpnParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.service,
    this.subscriptionLength,
    this.subscriptionPeriod,
    this.autorenewal,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'service[]': service,
      'subscription_length': subscriptionLength,
      'subscription_period': subscriptionPeriod?.value,
      'autorenewal': autorenewal?.value,
    };
  }
}

class CategoryVpnResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryVpnResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryVpnResponse.fromJson(Map<String, dynamic> json) => CategoryVpnResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryRobloxParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  final Email? email;
  /// Minimum robux.
  final num? robuxMin;
  /// Maximum robux.
  final num? robuxMax;
  /// Minimum friends.
  final num? friendsMin;
  /// Maximum friends.
  final num? friendsMax;
  /// Minimum number of followers.
  final num? followersMin;
  /// Maximum number of followers.
  final num? followersMax;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  /// How old is the account.
  final num? reg;
  final RegPeriod? regPeriod;
  final Subscription? subscription;
  /// Length of subscription.
  final num? subscriptionLength;
  final SubscriptionPeriod? subscriptionPeriod;
  final Autorenewal? autorenewal;
  final XboxConnected? xboxConnected;
  final PsnConnected? psnConnected;
  final Verified? verified;
  final AgeVerified? ageVerified;
  /// Minimum amount of incoming robux.
  final num? incomingRobuxTotalMin;
  /// Maximum amount of incoming robux.
  final num? incomingRobuxTotalMax;
  /// Minimum limited items value.
  final num? limitedPriceMin;
  /// Maximum limited items value.
  final num? limitedPriceMax;
  /// Minimum total Robux cost of all game passes in popular Roblox games..
  final num? gamepassMin;
  /// Maximum total Robux cost of all game passes in popular Roblox games..
  final num? gamepassMax;
  final GameDonations? gameDonations;
  /// Minimum inventory value.
  final num? invMin;
  /// Maximum inventory value.
  final num? invMax;
  /// Minimum UGC limited items value.
  final num? ugcLimitedPriceMin;
  /// Maximum UGC limited items value.
  final num? ugcLimitedPriceMax;
  /// Minimum credit balance.
  final num? creditBalanceMin;
  /// Maximum credit balance.
  final num? creditBalanceMax;
  /// Minimum offsale items count.
  final num? offsaleMin;
  /// Maximum offsale items count.
  final num? offsaleMax;
  final Voice? voice;
  /// List of allowed age groups.
  final List<String>? ageGroup;
  /// List of disallowed age groups.
  final List<String>? notAgeGroup;

  const CategoryRobloxParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.email,
    this.robuxMin,
    this.robuxMax,
    this.friendsMin,
    this.friendsMax,
    this.followersMin,
    this.followersMax,
    this.country,
    this.notCountry,
    this.reg,
    this.regPeriod,
    this.subscription,
    this.subscriptionLength,
    this.subscriptionPeriod,
    this.autorenewal,
    this.xboxConnected,
    this.psnConnected,
    this.verified,
    this.ageVerified,
    this.incomingRobuxTotalMin,
    this.incomingRobuxTotalMax,
    this.limitedPriceMin,
    this.limitedPriceMax,
    this.gamepassMin,
    this.gamepassMax,
    this.gameDonations,
    this.invMin,
    this.invMax,
    this.ugcLimitedPriceMin,
    this.ugcLimitedPriceMax,
    this.creditBalanceMin,
    this.creditBalanceMax,
    this.offsaleMin,
    this.offsaleMax,
    this.voice,
    this.ageGroup,
    this.notAgeGroup,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'email': email?.value,
      'robux_min': robuxMin,
      'robux_max': robuxMax,
      'friends_min': friendsMin,
      'friends_max': friendsMax,
      'followers_min': followersMin,
      'followers_max': followersMax,
      'country': country,
      'not_country': notCountry,
      'reg': reg,
      'reg_period': regPeriod?.value,
      'subscription': subscription?.value,
      'subscription_length': subscriptionLength,
      'subscription_period': subscriptionPeriod?.value,
      'autorenewal': autorenewal?.value,
      'xbox_connected': xboxConnected?.value,
      'psn_connected': psnConnected?.value,
      'verified': verified?.value,
      'age_verified': ageVerified?.value,
      'incoming_robux_total_min': incomingRobuxTotalMin,
      'incoming_robux_total_max': incomingRobuxTotalMax,
      'limited_price_min': limitedPriceMin,
      'limited_price_max': limitedPriceMax,
      'gamepass_min': gamepassMin,
      'gamepass_max': gamepassMax,
      'game_donations': gameDonations?.value,
      'inv_min': invMin,
      'inv_max': invMax,
      'ugc_limited_price_min': ugcLimitedPriceMin,
      'ugc_limited_price_max': ugcLimitedPriceMax,
      'credit_balance_min': creditBalanceMin,
      'credit_balance_max': creditBalanceMax,
      'offsale_min': offsaleMin,
      'offsale_max': offsaleMax,
      'voice': voice?.value,
      'age_group[]': ageGroup,
      'not_age_group[]': notAgeGroup,
    };
  }
}

class CategoryRobloxResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryRobloxResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryRobloxResponse.fromJson(Map<String, dynamic> json) => CategoryRobloxResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryWarfaceParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// Minimum rank.
  final num? rankMin;
  /// Maximum rank.
  final num? rankMax;
  /// Minimum bonus rank.
  final num? bonusRankMin;
  /// Maximum bonus rank.
  final num? bonusRankMax;
  final Tel? tel;
  /// Number of days the account has been offline.
  final num? daybreak;
  /// Minimum amount of Kredits.
  final num? kreditsMin;
  /// Maximum amount of Kredits.
  final num? kreditsMax;
  /// Minimum total donated Kredits.
  final num? totalKreditsMin;
  /// Maximum total donated Kredits.
  final num? totalKreditsMax;

  const CategoryWarfaceParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.rankMin,
    this.rankMax,
    this.bonusRankMin,
    this.bonusRankMax,
    this.tel,
    this.daybreak,
    this.kreditsMin,
    this.kreditsMax,
    this.totalKreditsMin,
    this.totalKreditsMax,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'rank_min': rankMin,
      'rank_max': rankMax,
      'bonus_rank_min': bonusRankMin,
      'bonus_rank_max': bonusRankMax,
      'tel': tel?.value,
      'daybreak': daybreak,
      'kredits_min': kreditsMin,
      'kredits_max': kreditsMax,
      'total_kredits_min': totalKreditsMin,
      'total_kredits_max': totalKreditsMax,
    };
  }
}

class CategoryWarfaceResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryWarfaceResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryWarfaceResponse.fromJson(Map<String, dynamic> json) => CategoryWarfaceResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryMinecraftParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  final Subscription? subscription;
  /// Length of subscription.
  final num? subscriptionLength;
  final SubscriptionPeriod? subscriptionPeriod;
  final Autorenewal? autorenewal;
  final Java? java;
  final Bedrock? bedrock;
  final Dungeons? dungeons;
  final Legends? legends;
  final ChangeNickname? changeNickname;
  /// List of capes.
  final List<String>? capes;
  /// Minimum number of capes.
  final num? capesMin;
  /// Maximum number of capes.
  final num? capesMax;
  /// List of allowed countries.
  final List<String>? country;
  /// List of disallowed countries.
  final List<String>? notCountry;
  final HypixelBan? hypixelBan;
  final HypixelSkyblockApiEnabled? hypixelSkyblockApiEnabled;
  /// Rank on hypixel.
  final List<String>? rankHypixel;
  /// Minimum number of level hypixel.
  final num? levelHypixelMin;
  /// Maximum number of level hypixel.
  final num? levelHypixelMax;
  /// Minimum number of achievement hypixel.
  final num? achievementHypixelMin;
  /// Maximum number of achievement hypixel.
  final num? achievementHypixelMax;
  /// Minimum level on Hypixel SkyBlock.
  final num? levelHypixelSkyblockMin;
  /// Maximum level on Hypixel SkyBlock.
  final num? levelHypixelSkyblockMax;
  /// Minimum net worth on Hypixel SkyBlock.
  final num? netWorthHypixelSkyblockMin;
  /// Maximum net worth on Hypixel SkyBlock.
  final num? netWorthHypixelSkyblockMax;
  /// How old is the account.
  final num? reg;
  final RegPeriod? regPeriod;
  /// How old is the last login account.
  final num? lastLoginHypixel;
  final LastLoginHypixelPeriod? lastLoginHypixelPeriod;
  final CanChangeDetails? canChangeDetails;
  /// Minimum number of characters in nickname.
  final num? nicknameLengthMin;
  /// Maximum number of characters in nickname.
  final num? nicknameLengthMax;
  final HypixelBanParsed? hypixelBanParsed;
  /// Minimum number of Minecoins.
  final num? minecoinsMin;
  /// Maximum number of Minecoins.
  final num? minecoinsMax;

  const CategoryMinecraftParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.subscription,
    this.subscriptionLength,
    this.subscriptionPeriod,
    this.autorenewal,
    this.java,
    this.bedrock,
    this.dungeons,
    this.legends,
    this.changeNickname,
    this.capes,
    this.capesMin,
    this.capesMax,
    this.country,
    this.notCountry,
    this.hypixelBan,
    this.hypixelSkyblockApiEnabled,
    this.rankHypixel,
    this.levelHypixelMin,
    this.levelHypixelMax,
    this.achievementHypixelMin,
    this.achievementHypixelMax,
    this.levelHypixelSkyblockMin,
    this.levelHypixelSkyblockMax,
    this.netWorthHypixelSkyblockMin,
    this.netWorthHypixelSkyblockMax,
    this.reg,
    this.regPeriod,
    this.lastLoginHypixel,
    this.lastLoginHypixelPeriod,
    this.canChangeDetails,
    this.nicknameLengthMin,
    this.nicknameLengthMax,
    this.hypixelBanParsed,
    this.minecoinsMin,
    this.minecoinsMax,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'subscription': subscription?.value,
      'subscription_length': subscriptionLength,
      'subscription_period': subscriptionPeriod?.value,
      'autorenewal': autorenewal?.value,
      'java': java?.value,
      'bedrock': bedrock?.value,
      'dungeons': dungeons?.value,
      'legends': legends?.value,
      'change_nickname': changeNickname?.value,
      'capes[]': capes,
      'capes_min': capesMin,
      'capes_max': capesMax,
      'country[]': country,
      'not_country[]': notCountry,
      'hypixel_ban': hypixelBan?.value,
      'hypixel_skyblock_api_enabled': hypixelSkyblockApiEnabled?.value,
      'rank_hypixel[]': rankHypixel,
      'level_hypixel_min': levelHypixelMin,
      'level_hypixel_max': levelHypixelMax,
      'achievement_hypixel_min': achievementHypixelMin,
      'achievement_hypixel_max': achievementHypixelMax,
      'level_hypixel_skyblock_min': levelHypixelSkyblockMin,
      'level_hypixel_skyblock_max': levelHypixelSkyblockMax,
      'net_worth_hypixel_skyblock_min': netWorthHypixelSkyblockMin,
      'net_worth_hypixel_skyblock_max': netWorthHypixelSkyblockMax,
      'reg': reg,
      'reg_period': regPeriod?.value,
      'last_login_hypixel': lastLoginHypixel,
      'last_login_hypixel_period': lastLoginHypixelPeriod?.value,
      'can_change_details': canChangeDetails?.value,
      'nickname_length_min': nicknameLengthMin,
      'nickname_length_max': nicknameLengthMax,
      'hypixel_ban_parsed': hypixelBanParsed?.value,
      'minecoins_min': minecoinsMin,
      'minecoins_max': minecoinsMax,
    };
  }
}

class CategoryMinecraftResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryMinecraftResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryMinecraftResponse.fromJson(Map<String, dynamic> json) => CategoryMinecraftResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryHytaleParams {
  /// The number of the page to display results from.
  final num? page;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// The word or words contained in the account title.
  final String? title;
  final OrderBy? orderBy;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? tagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notTagId;
  /// List of tag ids (Tag list is available via $1).
  final List<num>? publicTagId;
  /// List of tag ids that won't be included (Tag list is available via $1).
  final List<num>? notPublicTagId;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  /// Search accounts of user.
  final num? userId;
  /// Not sold before.
  final bool? nsb;
  /// Sold before.
  final bool? sb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Sold by me before.
  final bool? sbByMe;
  final Currency? currency;
  /// Has email login data.
  final bool? emailLoginData;
  /// Email provider.
  final List<String>? emailProvider;
  final NotEmailProvider? notEmailProvider;
  /// Parse same item ids.
  final bool? parseSameItemIds;
  /// List of allowed editions.
  final List<String>? edition;
  /// Minimum number of profiles with game.
  final num? profilesMin;
  /// Maximum number of profiles with game.
  final num? profilesMax;

  const CategoryHytaleParams({
    this.page,
    this.pmin,
    this.pmax,
    this.title,
    this.orderBy,
    this.tagId,
    this.notTagId,
    this.publicTagId,
    this.notPublicTagId,
    this.origin,
    this.notOrigin,
    this.userId,
    this.nsb,
    this.sb,
    this.nsbByMe,
    this.sbByMe,
    this.currency,
    this.emailLoginData,
    this.emailProvider,
    this.notEmailProvider,
    this.parseSameItemIds,
    this.edition,
    this.profilesMin,
    this.profilesMax,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'pmin': pmin,
      'pmax': pmax,
      'title': title,
      'order_by': orderBy?.value,
      'tag_id[]': tagId,
      'not_tag_id[]': notTagId,
      'public_tag_id[]': publicTagId,
      'not_public_tag_id[]': notPublicTagId,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'user_id': userId,
      'nsb': nsb,
      'sb': sb,
      'nsb_by_me': nsbByMe,
      'sb_by_me': sbByMe,
      'currency': currency?.value,
      'email_login_data': emailLoginData,
      'email_provider[]': emailProvider,
      'not_email_provider[]': notEmailProvider?.value,
      'parse_same_item_ids': parseSameItemIds,
      'edition[]': edition,
      'profiles_min': profilesMin,
      'profiles_max': profilesMax,
    };
  }
}

class CategoryHytaleResponse {
  final List<Map<String, dynamic>> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final bool wasCached;
  final num cacheTTL;
  final num lastModified;
  final num serverTime;
  final String searchUrl;
  final List<dynamic> stickyItems;
  final RespSystemInfo systemInfo;

  const CategoryHytaleResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.wasCached,
    required this.cacheTTL,
    required this.lastModified,
    required this.serverTime,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory CategoryHytaleResponse.fromJson(Map<String, dynamic> json) => CategoryHytaleResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    wasCached: json['wasCached'] is bool ? json['wasCached'] as bool : false,
    cacheTTL: json['cacheTTL'] is num ? json['cacheTTL'] as num : 0,
    lastModified: json['lastModified'] is num ? json['lastModified'] as num : 0,
    serverTime: json['serverTime'] is num ? json['serverTime'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List ? json['stickyItems'] as List<dynamic> : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryListParams {
  /// Display top queries for per category.
  final bool? topQueries;

  const CategoryListParams({
    this.topQueries,
  });

  Map<String, Object?> toMap() {
    return {
      'top_queries': topQueries,
    };
  }
}

class CategoryListResponseCategoryLinks {
  final String permalink;
  final String detail;
  final String subCategories;
  final String subForums;

  const CategoryListResponseCategoryLinks({
    required this.permalink,
    required this.detail,
    required this.subCategories,
    required this.subForums,
  });

  factory CategoryListResponseCategoryLinks.fromJson(Map<String, dynamic> json) => CategoryListResponseCategoryLinks(
    permalink: json['permalink'] is String ? json['permalink'] as String : '',
    detail: json['detail'] is String ? json['detail'] as String : '',
    subCategories: json['sub-categories'] is String ? json['sub-categories'] as String : '',
    subForums: json['sub-forums'] is String ? json['sub-forums'] as String : '',
  );
}

class CategoryListResponseCategory {
  final num categoryId;
  final String categoryTitle;
  final String categoryDescription;
  final CategoryListResponseCategoryLinks links;

  const CategoryListResponseCategory({
    required this.categoryId,
    required this.categoryTitle,
    required this.categoryDescription,
    required this.links,
  });

  factory CategoryListResponseCategory.fromJson(Map<String, dynamic> json) => CategoryListResponseCategory(
    categoryId: json['category_id'] is num ? json['category_id'] as num : 0,
    categoryTitle: json['category_title'] is String ? json['category_title'] as String : '',
    categoryDescription: json['category_description'] is String ? json['category_description'] as String : '',
    links: json['links'] is Map<String, dynamic>
            ? CategoryListResponseCategoryLinks.fromJson(json['links'] as Map<String, dynamic>)
            : CategoryListResponseCategoryLinks.fromJson(const {}),
  );
}

class CategoryListResponse {
  final CategoryListResponseCategory category;
  final RespSystemInfo systemInfo;

  const CategoryListResponse({
    required this.category,
    required this.systemInfo,
  });

  factory CategoryListResponse.fromJson(Map<String, dynamic> json) => CategoryListResponse(
    category: json['category'] is Map<String, dynamic>
            ? CategoryListResponseCategory.fromJson(json['category'] as Map<String, dynamic>)
            : CategoryListResponseCategory.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CategoryParamsResponseCategory {
  final num categoryId;
  final num subCategoryId;
  final num categoryOrder;
  final String categoryTitle;
  final String categoryName;
  final String categoryUrl;
  final String categoryDescriptionHtml;
  final String categoryLoginUrl;
  final num addItemAvailable;
  final num massUploadItemAvailable;
  final num hasGuarantee;
  final num hasAccountLink;
  final num requireTempEmail;
  final String recoveryLink;
  final num checkButtonEnabled;
  final num checkerEnabled;
  final num supportPersonalProxy;
  final num supportEmailLoginData;
  final num requireEmailLoginData;
  final num displayInList;
  final String categoryDescriptionHtmlEn;
  final String categoryH1HtmlEn;
  final num accountPriceMin;
  final num requireVideoRecording;
  final String topQueries;
  final num requireEldForNativeAccs;
  final num canBeResold;
  final num supportTempEmail;
  final String cookies;
  final String loginType;
  final num guestHidden;
  final num availableTempEmail;
  final num resaleDurationLimitDays;
  final num buyWithoutValidation;
  final num maxInvalidUploadTries;

  const CategoryParamsResponseCategory({
    required this.categoryId,
    required this.subCategoryId,
    required this.categoryOrder,
    required this.categoryTitle,
    required this.categoryName,
    required this.categoryUrl,
    required this.categoryDescriptionHtml,
    required this.categoryLoginUrl,
    required this.addItemAvailable,
    required this.massUploadItemAvailable,
    required this.hasGuarantee,
    required this.hasAccountLink,
    required this.requireTempEmail,
    required this.recoveryLink,
    required this.checkButtonEnabled,
    required this.checkerEnabled,
    required this.supportPersonalProxy,
    required this.supportEmailLoginData,
    required this.requireEmailLoginData,
    required this.displayInList,
    required this.categoryDescriptionHtmlEn,
    required this.categoryH1HtmlEn,
    required this.accountPriceMin,
    required this.requireVideoRecording,
    required this.topQueries,
    required this.requireEldForNativeAccs,
    required this.canBeResold,
    required this.supportTempEmail,
    required this.cookies,
    required this.loginType,
    required this.guestHidden,
    required this.availableTempEmail,
    required this.resaleDurationLimitDays,
    required this.buyWithoutValidation,
    required this.maxInvalidUploadTries,
  });

  factory CategoryParamsResponseCategory.fromJson(Map<String, dynamic> json) => CategoryParamsResponseCategory(
    categoryId: json['category_id'] is num ? json['category_id'] as num : 0,
    subCategoryId: json['sub_category_id'] is num ? json['sub_category_id'] as num : 0,
    categoryOrder: json['category_order'] is num ? json['category_order'] as num : 0,
    categoryTitle: json['category_title'] is String ? json['category_title'] as String : '',
    categoryName: json['category_name'] is String ? json['category_name'] as String : '',
    categoryUrl: json['category_url'] is String ? json['category_url'] as String : '',
    categoryDescriptionHtml: json['category_description_html'] is String ? json['category_description_html'] as String : '',
    categoryLoginUrl: json['category_login_url'] is String ? json['category_login_url'] as String : '',
    addItemAvailable: json['add_item_available'] is num ? json['add_item_available'] as num : 0,
    massUploadItemAvailable: json['mass_upload_item_available'] is num ? json['mass_upload_item_available'] as num : 0,
    hasGuarantee: json['has_guarantee'] is num ? json['has_guarantee'] as num : 0,
    hasAccountLink: json['has_account_link'] is num ? json['has_account_link'] as num : 0,
    requireTempEmail: json['require_temp_email'] is num ? json['require_temp_email'] as num : 0,
    recoveryLink: json['recovery_link'] is String ? json['recovery_link'] as String : '',
    checkButtonEnabled: json['check_button_enabled'] is num ? json['check_button_enabled'] as num : 0,
    checkerEnabled: json['checker_enabled'] is num ? json['checker_enabled'] as num : 0,
    supportPersonalProxy: json['support_personal_proxy'] is num ? json['support_personal_proxy'] as num : 0,
    supportEmailLoginData: json['support_email_login_data'] is num ? json['support_email_login_data'] as num : 0,
    requireEmailLoginData: json['require_email_login_data'] is num ? json['require_email_login_data'] as num : 0,
    displayInList: json['display_in_list'] is num ? json['display_in_list'] as num : 0,
    categoryDescriptionHtmlEn: json['category_description_html_en'] is String ? json['category_description_html_en'] as String : '',
    categoryH1HtmlEn: json['category_h1_html_en'] is String ? json['category_h1_html_en'] as String : '',
    accountPriceMin: json['account_price_min'] is num ? json['account_price_min'] as num : 0,
    requireVideoRecording: json['require_video_recording'] is num ? json['require_video_recording'] as num : 0,
    topQueries: json['top_queries'] is String ? json['top_queries'] as String : '',
    requireEldForNativeAccs: json['require_eld_for_native_accs'] is num ? json['require_eld_for_native_accs'] as num : 0,
    canBeResold: json['can_be_resold'] is num ? json['can_be_resold'] as num : 0,
    supportTempEmail: json['support_temp_email'] is num ? json['support_temp_email'] as num : 0,
    cookies: json['cookies'] is String ? json['cookies'] as String : '',
    loginType: json['login_type'] is String ? json['login_type'] as String : '',
    guestHidden: json['guest_hidden'] is num ? json['guest_hidden'] as num : 0,
    availableTempEmail: json['available_temp_email'] is num ? json['available_temp_email'] as num : 0,
    resaleDurationLimitDays: json['resale_duration_limit_days'] is num ? json['resale_duration_limit_days'] as num : 0,
    buyWithoutValidation: json['buy_without_validation'] is num ? json['buy_without_validation'] as num : 0,
    maxInvalidUploadTries: json['max_invalid_upload_tries'] is num ? json['max_invalid_upload_tries'] as num : 0,
  );
}

class CategoryParamsResponse {
  final CategoryParamsResponseCategory? category;
  final List<Map<String, dynamic>>? params;
  final dynamic baseParams;
  final RespSystemInfo? systemInfo;

  const CategoryParamsResponse({
    this.category,
    this.params,
    this.baseParams,
    this.systemInfo,
  });

  factory CategoryParamsResponse.fromJson(Map<String, dynamic> json) => CategoryParamsResponse(
    category: json['category'] is Map<String, dynamic>
            ? CategoryParamsResponseCategory.fromJson(json['category'] as Map<String, dynamic>)
            : null,
    params: json['params'] is List ? (json['params'] as List<dynamic>).cast<Map<String, dynamic>>() : null,
    baseParams: json['base_params'],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class CategoryGamesResponse {
  final List<Map<String, dynamic>>? games;
  final RespSystemInfo? systemInfo;

  const CategoryGamesResponse({
    this.games,
    this.systemInfo,
  });

  factory CategoryGamesResponse.fromJson(Map<String, dynamic> json) => CategoryGamesResponse(
    games: json['games'] is List ? (json['games'] as List<dynamic>).cast<Map<String, dynamic>>() : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

// ─── CustomDiscountsApi Types ────────────────────────────────────────

class CustomDiscountsGetResponse {
  final List<DiscountModel> discounts;
  final num total;
  final RespSystemInfo systemInfo;

  const CustomDiscountsGetResponse({
    required this.discounts,
    required this.total,
    required this.systemInfo,
  });

  factory CustomDiscountsGetResponse.fromJson(Map<String, dynamic> json) => CustomDiscountsGetResponse(
    discounts: json['discounts'] is List
            ? (json['discounts'] as List<dynamic>)
                .map((e) => DiscountModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    total: json['total'] is num ? json['total'] as num : 0,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CustomDiscountsCreateBody {
  /// User ID.
  final num userId;
  final CategoryId categoryId;
  /// Discount percent to apply.
  final num discountPercent;
  /// Minimum accounts price for which the discount applies.
  final num minPrice;
  /// Maximum accounts price for which the discount applies.
  final num? maxPrice;
  final Currency? currency;

  const CustomDiscountsCreateBody({
    required this.userId,
    required this.categoryId,
    required this.discountPercent,
    required this.minPrice,
    this.maxPrice,
    this.currency,
  });

  Map<String, Object?> toMap() {
    return {
      'user_id': userId,
      'category_id': categoryId.value,
      'discount_percent': discountPercent,
      'min_price': minPrice,
      'max_price': maxPrice,
      'currency': currency?.value,
    };
  }
}

class CustomDiscountsCreateResponse {
  final DiscountModel discount;
  final num total;
  final RespSystemInfo systemInfo;

  const CustomDiscountsCreateResponse({
    required this.discount,
    required this.total,
    required this.systemInfo,
  });

  factory CustomDiscountsCreateResponse.fromJson(Map<String, dynamic> json) => CustomDiscountsCreateResponse(
    discount: json['discount'] is Map<String, dynamic>
            ? DiscountModel.fromJson(json['discount'] as Map<String, dynamic>)
            : DiscountModel.fromJson(const {}),
    total: json['total'] is num ? json['total'] as num : 0,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CustomDiscountsEditBody {
  /// ID of the discount to edit.
  final num discountId;
  /// Discount percent to apply.
  final num? discountPercent;
  /// Minimum price for which the discount applies.
  final num? minPrice;
  /// Maximum price for which the discount applies.
  final num? maxPrice;

  const CustomDiscountsEditBody({
    required this.discountId,
    this.discountPercent,
    this.minPrice,
    this.maxPrice,
  });

  Map<String, Object?> toMap() {
    return {
      'discount_id': discountId,
      'discount_percent': discountPercent,
      'min_price': minPrice,
      'max_price': maxPrice,
    };
  }
}

class CustomDiscountsEditResponse {
  final List<DiscountModel> discounts;
  final num total;
  final RespSystemInfo systemInfo;

  const CustomDiscountsEditResponse({
    required this.discounts,
    required this.total,
    required this.systemInfo,
  });

  factory CustomDiscountsEditResponse.fromJson(Map<String, dynamic> json) => CustomDiscountsEditResponse(
    discounts: json['discounts'] is List
            ? (json['discounts'] as List<dynamic>)
                .map((e) => DiscountModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    total: json['total'] is num ? json['total'] as num : 0,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class CustomDiscountsDeleteBody {
  /// ID of the discount to delete.
  final num discountId;

  const CustomDiscountsDeleteBody({
    required this.discountId,
  });

  Map<String, Object?> toMap() {
    return {
      'discount_id': discountId,
    };
  }
}

class CustomDiscountsDeleteResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const CustomDiscountsDeleteResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory CustomDiscountsDeleteResponse.fromJson(Map<String, dynamic> json) => CustomDiscountsDeleteResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

// ─── ImapApi Types ────────────────────────────────────────

class ImapCreateBody {
  /// Domain to delete IMAP configuration for.
  final String domain;
  /// IMAP server address.
  final String imapServer;
  /// IMAP server port.
  final num port;
  /// Whether to use a secure connection.
  final bool secure;

  const ImapCreateBody({
    required this.domain,
    required this.imapServer,
    required this.port,
    required this.secure,
  });

  Map<String, Object?> toMap() {
    return {
      'domain': domain,
      'imap_server': imapServer,
      'port': port,
      'secure': secure,
    };
  }
}

class ImapCreateResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ImapCreateResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ImapCreateResponse.fromJson(Map<String, dynamic> json) => ImapCreateResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ImapDeleteBody {
  /// Domain to delete IMAP configuration for.
  final String domain;

  const ImapDeleteBody({
    required this.domain,
  });

  Map<String, Object?> toMap() {
    return {
      'domain': domain,
    };
  }
}

class ImapDeleteResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ImapDeleteResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ImapDeleteResponse.fromJson(Map<String, dynamic> json) => ImapDeleteResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

// ─── ListApi Types ────────────────────────────────────────

class ListUserParams {
  /// User id.
  final num? userId;
  final CategoryId? categoryId;
  /// The number of the page to display results from.
  final num? page;
  final Show? show;
  /// Delete reason. (Only if $1 is set to $1).
  final String? deleteReason;
  /// The word or words contained in the account title.
  final String? title;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// Login.
  final String? login;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  final OrderBy? orderBy;
  /// Sold before.
  final bool? sb;
  /// Sold by me before.
  final bool? sbByMe;
  /// Not sold before.
  final bool? nsb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Username of buyer. (If $1 is $1).
  final String? username;
  /// Start date for filtering by publication date.
  final String? publishedStartDate;
  /// End date for filtering by publication date.
  final String? publishedEndDate;
  /// Enable filtering by publication date.
  final bool? filterByPublishedDate;
  /// Start date for filtering by buyer operation date.
  final String? paidStartDate;
  /// End date for filtering by buyer operation date.
  final String? paidEndDate;
  /// Enable filtering by buyer operation date.
  final bool? filterByBuyerOperationDate;
  /// Start date for filtering by deletion date.
  final String? deleteStartDate;
  /// End date for filtering by deletion date.
  final String? deleteEndDate;
  /// Enable filtering by deletion date.
  final bool? filterByDeleteDate;

  const ListUserParams({
    this.userId,
    this.categoryId,
    this.page,
    this.show,
    this.deleteReason,
    this.title,
    this.pmin,
    this.pmax,
    this.login,
    this.origin,
    this.notOrigin,
    this.orderBy,
    this.sb,
    this.sbByMe,
    this.nsb,
    this.nsbByMe,
    this.username,
    this.publishedStartDate,
    this.publishedEndDate,
    this.filterByPublishedDate,
    this.paidStartDate,
    this.paidEndDate,
    this.filterByBuyerOperationDate,
    this.deleteStartDate,
    this.deleteEndDate,
    this.filterByDeleteDate,
  });

  Map<String, Object?> toMap() {
    return {
      'user_id': userId,
      'category_id': categoryId?.value,
      'page': page,
      'show': show?.value,
      'delete_reason': deleteReason,
      'title': title,
      'pmin': pmin,
      'pmax': pmax,
      'login': login,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'order_by': orderBy?.value,
      'sb': sb,
      'sb_by_me': sbByMe,
      'nsb': nsb,
      'nsb_by_me': nsbByMe,
      'username': username,
      'published_startDate': publishedStartDate,
      'published_endDate': publishedEndDate,
      'filter_by_published_date': filterByPublishedDate,
      'paid_startDate': paidStartDate,
      'paid_endDate': paidEndDate,
      'filter_by_buyer_operation_date': filterByBuyerOperationDate,
      'delete_startDate': deleteStartDate,
      'delete_endDate': deleteEndDate,
      'filter_by_delete_date': filterByDeleteDate,
    };
  }
}

class ListUserResponse {
  final List<ItemFromListModel> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final String searchUrl;
  final List<ItemFromListModel> stickyItems;
  final RespSystemInfo systemInfo;

  const ListUserResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory ListUserResponse.fromJson(Map<String, dynamic> json) => ListUserResponse(
    items: json['items'] is List
            ? (json['items'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List
            ? (json['stickyItems'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ListOrdersParams {
  /// User id.
  final num? userId;
  final CategoryId? categoryId;
  /// The number of the page to display results from.
  final num? page;
  final Show? show;
  /// The word or words contained in the account title.
  final String? title;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  /// Login.
  final String? login;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  final OrderBy? orderBy;
  /// Sold before.
  final bool? sb;
  /// Sold by me before.
  final bool? sbByMe;
  /// Not sold before.
  final bool? nsb;
  /// Not sold by me before.
  final bool? nsbByMe;

  const ListOrdersParams({
    this.userId,
    this.categoryId,
    this.page,
    this.show,
    this.title,
    this.pmin,
    this.pmax,
    this.login,
    this.origin,
    this.notOrigin,
    this.orderBy,
    this.sb,
    this.sbByMe,
    this.nsb,
    this.nsbByMe,
  });

  Map<String, Object?> toMap() {
    return {
      'user_id': userId,
      'category_id': categoryId?.value,
      'page': page,
      'show': show?.value,
      'title': title,
      'pmin': pmin,
      'pmax': pmax,
      'login': login,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'order_by': orderBy?.value,
      'sb': sb,
      'sb_by_me': sbByMe,
      'nsb': nsb,
      'nsb_by_me': nsbByMe,
    };
  }
}

class ListOrdersResponse {
  final List<ItemFromListModel> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final String searchUrl;
  final List<ItemFromListModel> stickyItems;
  final RespSystemInfo systemInfo;

  const ListOrdersResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory ListOrdersResponse.fromJson(Map<String, dynamic> json) => ListOrdersResponse(
    items: json['items'] is List
            ? (json['items'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List
            ? (json['stickyItems'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ListStatesParams {
  /// User ID.
  final Object? userId;

  const ListStatesParams({
    this.userId,
  });

  Map<String, Object?> toMap() {
    return {
      'user_id': userId,
    };
  }
}

class ListStatesResponseUserItemStatesStickied {
  final String itemState;
  final num itemCount;
  final String title;
  final num stickyLimit;

  const ListStatesResponseUserItemStatesStickied({
    required this.itemState,
    required this.itemCount,
    required this.title,
    required this.stickyLimit,
  });

  factory ListStatesResponseUserItemStatesStickied.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesStickied(
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
    stickyLimit: json['stickyLimit'] is num ? json['stickyLimit'] as num : 0,
  );
}

class ListStatesResponseUserItemStatesDiscountRequest {
  final String itemState;
  final num itemCount;
  final String title;

  const ListStatesResponseUserItemStatesDiscountRequest({
    required this.itemState,
    required this.itemCount,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesDiscountRequest.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesDiscountRequest(
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesInBuyersFavorites {
  final String itemState;
  final num itemCount;
  final String title;

  const ListStatesResponseUserItemStatesInBuyersFavorites({
    required this.itemState,
    required this.itemCount,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesInBuyersFavorites.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesInBuyersFavorites(
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesActive {
  final num itemCount;
  final String itemState;
  final String title;

  const ListStatesResponseUserItemStatesActive({
    required this.itemCount,
    required this.itemState,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesActive.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesActive(
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesPaid {
  final num itemCount;
  final String itemState;
  final String title;

  const ListStatesResponseUserItemStatesPaid({
    required this.itemCount,
    required this.itemState,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesPaid.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesPaid(
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesClosed {
  final String itemState;
  final num itemCount;
  final String title;

  const ListStatesResponseUserItemStatesClosed({
    required this.itemState,
    required this.itemCount,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesClosed.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesClosed(
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesDeleted {
  final num itemCount;
  final String itemState;
  final String title;

  const ListStatesResponseUserItemStatesDeleted({
    required this.itemCount,
    required this.itemState,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesDeleted.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesDeleted(
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesAwaiting {
  final num itemCount;
  final String itemState;
  final String title;

  const ListStatesResponseUserItemStatesAwaiting({
    required this.itemCount,
    required this.itemState,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesAwaiting.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesAwaiting(
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesPreActive {
  final String itemState;
  final num itemCount;
  final String title;

  const ListStatesResponseUserItemStatesPreActive({
    required this.itemState,
    required this.itemCount,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesPreActive.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesPreActive(
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesPreUpload {
  final String itemState;
  final num itemCount;
  final String title;

  const ListStatesResponseUserItemStatesPreUpload({
    required this.itemState,
    required this.itemCount,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesPreUpload.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesPreUpload(
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesPendingDeletion {
  final String itemState;
  final num itemCount;
  final String title;

  const ListStatesResponseUserItemStatesPendingDeletion({
    required this.itemState,
    required this.itemCount,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesPendingDeletion.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesPendingDeletion(
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesClosedInactive {
  final num itemCount;
  final String itemState;
  final String title;

  const ListStatesResponseUserItemStatesClosedInactive({
    required this.itemCount,
    required this.itemState,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesClosedInactive.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesClosedInactive(
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStatesAutoBump {
  final String itemState;
  final num itemCount;
  final String title;

  const ListStatesResponseUserItemStatesAutoBump({
    required this.itemState,
    required this.itemCount,
    required this.title,
  });

  factory ListStatesResponseUserItemStatesAutoBump.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStatesAutoBump(
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    itemCount: json['item_count'] is num ? json['item_count'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
  );
}

class ListStatesResponseUserItemStates {
  final ListStatesResponseUserItemStatesStickied stickied;
  final ListStatesResponseUserItemStatesDiscountRequest discountRequest;
  final ListStatesResponseUserItemStatesInBuyersFavorites inBuyersFavorites;
  final ListStatesResponseUserItemStatesActive active;
  final ListStatesResponseUserItemStatesPaid paid;
  final ListStatesResponseUserItemStatesClosed closed;
  final ListStatesResponseUserItemStatesDeleted deleted;
  final ListStatesResponseUserItemStatesAwaiting awaiting;
  final ListStatesResponseUserItemStatesPreActive preActive;
  final ListStatesResponseUserItemStatesPreUpload preUpload;
  final ListStatesResponseUserItemStatesPendingDeletion pendingDeletion;
  final ListStatesResponseUserItemStatesClosedInactive closedInactive;
  final ListStatesResponseUserItemStatesAutoBump autoBump;

  const ListStatesResponseUserItemStates({
    required this.stickied,
    required this.discountRequest,
    required this.inBuyersFavorites,
    required this.active,
    required this.paid,
    required this.closed,
    required this.deleted,
    required this.awaiting,
    required this.preActive,
    required this.preUpload,
    required this.pendingDeletion,
    required this.closedInactive,
    required this.autoBump,
  });

  factory ListStatesResponseUserItemStates.fromJson(Map<String, dynamic> json) => ListStatesResponseUserItemStates(
    stickied: json['stickied'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesStickied.fromJson(json['stickied'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesStickied.fromJson(const {}),
    discountRequest: json['discount_request'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesDiscountRequest.fromJson(json['discount_request'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesDiscountRequest.fromJson(const {}),
    inBuyersFavorites: json['in_buyers_favorites'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesInBuyersFavorites.fromJson(json['in_buyers_favorites'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesInBuyersFavorites.fromJson(const {}),
    active: json['active'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesActive.fromJson(json['active'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesActive.fromJson(const {}),
    paid: json['paid'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesPaid.fromJson(json['paid'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesPaid.fromJson(const {}),
    closed: json['closed'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesClosed.fromJson(json['closed'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesClosed.fromJson(const {}),
    deleted: json['deleted'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesDeleted.fromJson(json['deleted'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesDeleted.fromJson(const {}),
    awaiting: json['awaiting'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesAwaiting.fromJson(json['awaiting'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesAwaiting.fromJson(const {}),
    preActive: json['pre_active'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesPreActive.fromJson(json['pre_active'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesPreActive.fromJson(const {}),
    preUpload: json['pre_upload'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesPreUpload.fromJson(json['pre_upload'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesPreUpload.fromJson(const {}),
    pendingDeletion: json['pending_deletion'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesPendingDeletion.fromJson(json['pending_deletion'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesPendingDeletion.fromJson(const {}),
    closedInactive: json['closed_inactive'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesClosedInactive.fromJson(json['closed_inactive'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesClosedInactive.fromJson(const {}),
    autoBump: json['auto_bump'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStatesAutoBump.fromJson(json['auto_bump'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStatesAutoBump.fromJson(const {}),
  );
}

class ListStatesResponse {
  final ListStatesResponseUserItemStates userItemStates;
  final RespSystemInfo systemInfo;

  const ListStatesResponse({
    required this.userItemStates,
    required this.systemInfo,
  });

  factory ListStatesResponse.fromJson(Map<String, dynamic> json) => ListStatesResponse(
    userItemStates: json['userItemStates'] is Map<String, dynamic>
            ? ListStatesResponseUserItemStates.fromJson(json['userItemStates'] as Map<String, dynamic>)
            : ListStatesResponseUserItemStates.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ListDownloadParams {
  final Format? format;
  /// Custom format string for download. (Required if $1 is set to $1).
  final String? customFormat;
  final CategoryId? categoryId;
  /// The number of the page to display results from.
  final num? page;
  final Show? show;
  /// Delete reason. (Only if $1 is set to $1).
  final String? deleteReason;
  /// The word or words contained in the account title.
  final String? title;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  final OrderBy? orderBy;
  /// Sold before.
  final bool? sb;
  /// Sold by me before.
  final bool? sbByMe;
  /// Not sold before.
  final bool? nsb;
  /// Not sold by me before.
  final bool? nsbByMe;
  /// Username of buyer. (If $1 is $1).
  final String? username;
  /// Start date for filtering by publication date.
  final String? publishedStartDate;
  /// End date for filtering by publication date.
  final String? publishedEndDate;
  /// Enable filtering by publication date.
  final bool? filterByPublishedDate;
  /// Start date for filtering by buyer operation date.
  final String? paidStartDate;
  /// End date for filtering by buyer operation date.
  final String? paidEndDate;
  /// Enable filtering by buyer operation date.
  final bool? filterByBuyerOperationDate;
  /// Start date for filtering by deletion date.
  final String? deleteStartDate;
  /// End date for filtering by deletion date.
  final String? deleteEndDate;
  /// Enable filtering by deletion date.
  final bool? filterByDeleteDate;

  const ListDownloadParams({
    this.format,
    this.customFormat,
    this.categoryId,
    this.page,
    this.show,
    this.deleteReason,
    this.title,
    this.pmin,
    this.pmax,
    this.origin,
    this.notOrigin,
    this.orderBy,
    this.sb,
    this.sbByMe,
    this.nsb,
    this.nsbByMe,
    this.username,
    this.publishedStartDate,
    this.publishedEndDate,
    this.filterByPublishedDate,
    this.paidStartDate,
    this.paidEndDate,
    this.filterByBuyerOperationDate,
    this.deleteStartDate,
    this.deleteEndDate,
    this.filterByDeleteDate,
  });

  Map<String, Object?> toMap() {
    return {
      'format': format?.value,
      'custom_format': customFormat,
      'category_id': categoryId?.value,
      'page': page,
      'show': show?.value,
      'delete_reason': deleteReason,
      'title': title,
      'pmin': pmin,
      'pmax': pmax,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'order_by': orderBy?.value,
      'sb': sb,
      'sb_by_me': sbByMe,
      'nsb': nsb,
      'nsb_by_me': nsbByMe,
      'username': username,
      'published_startDate': publishedStartDate,
      'published_endDate': publishedEndDate,
      'filter_by_published_date': filterByPublishedDate,
      'paid_startDate': paidStartDate,
      'paid_endDate': paidEndDate,
      'filter_by_buyer_operation_date': filterByBuyerOperationDate,
      'delete_startDate': deleteStartDate,
      'delete_endDate': deleteEndDate,
      'filter_by_delete_date': filterByDeleteDate,
    };
  }
}

class ListFavoritesParams {
  /// The number of the page to display results from.
  final num? page;
  final Show? show;
  /// The word or words contained in the account title.
  final String? title;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  final OrderBy? orderBy;
  /// Sold before.
  final bool? sb;
  /// Sold by me before.
  final bool? sbByMe;
  /// Not sold before.
  final bool? nsb;
  /// Not sold by me before.
  final bool? nsbByMe;

  const ListFavoritesParams({
    this.page,
    this.show,
    this.title,
    this.pmin,
    this.pmax,
    this.origin,
    this.notOrigin,
    this.orderBy,
    this.sb,
    this.sbByMe,
    this.nsb,
    this.nsbByMe,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'show': show?.value,
      'title': title,
      'pmin': pmin,
      'pmax': pmax,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'order_by': orderBy?.value,
      'sb': sb,
      'sb_by_me': sbByMe,
      'nsb': nsb,
      'nsb_by_me': nsbByMe,
    };
  }
}

class ListFavoritesResponse {
  final List<ItemFromListModel> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final String searchUrl;
  final List<ItemFromListModel> stickyItems;
  final RespSystemInfo systemInfo;

  const ListFavoritesResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory ListFavoritesResponse.fromJson(Map<String, dynamic> json) => ListFavoritesResponse(
    items: json['items'] is List
            ? (json['items'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List
            ? (json['stickyItems'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ListViewedParams {
  /// The number of the page to display results from.
  final num? page;
  final Show? show;
  /// The word or words contained in the account title.
  final String? title;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  final Origin? origin;
  /// List of account origins that won't be included.
  final List<String>? notOrigin;
  final OrderBy? orderBy;
  /// Sold before.
  final bool? sb;
  /// Sold by me before.
  final bool? sbByMe;
  /// Not sold before.
  final bool? nsb;
  /// Not sold by me before.
  final bool? nsbByMe;

  const ListViewedParams({
    this.page,
    this.show,
    this.title,
    this.pmin,
    this.pmax,
    this.origin,
    this.notOrigin,
    this.orderBy,
    this.sb,
    this.sbByMe,
    this.nsb,
    this.nsbByMe,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'show': show?.value,
      'title': title,
      'pmin': pmin,
      'pmax': pmax,
      'origin[]': origin?.value,
      'not_origin[]': notOrigin,
      'order_by': orderBy?.value,
      'sb': sb,
      'sb_by_me': sbByMe,
      'nsb': nsb,
      'nsb_by_me': nsbByMe,
    };
  }
}

class ListViewedResponse {
  final List<ItemFromListModel> items;
  final num totalItems;
  final dynamic totalItemsPrice;
  final bool hasNextPage;
  final num perPage;
  final num page;
  final String searchUrl;
  final List<ItemFromListModel> stickyItems;
  final RespSystemInfo systemInfo;

  const ListViewedResponse({
    required this.items,
    required this.totalItems,
    required this.totalItemsPrice,
    required this.hasNextPage,
    required this.perPage,
    required this.page,
    required this.searchUrl,
    required this.stickyItems,
    required this.systemInfo,
  });

  factory ListViewedResponse.fromJson(Map<String, dynamic> json) => ListViewedResponse(
    items: json['items'] is List
            ? (json['items'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    totalItems: json['totalItems'] is num ? json['totalItems'] as num : 0,
    totalItemsPrice: json['totalItemsPrice'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    searchUrl: json['searchUrl'] is String ? json['searchUrl'] as String : '',
    stickyItems: json['stickyItems'] is List
            ? (json['stickyItems'] as List<dynamic>)
                .map((e) => ItemFromListModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

// ─── ManagingApi Types ────────────────────────────────────────

class ManagingGetParams {
  /// Parse same item ids.
  final bool? parseSameItemIds;

  const ManagingGetParams({
    this.parseSameItemIds,
  });

  Map<String, Object?> toMap() {
    return {
      'parse_same_item_ids': parseSameItemIds,
    };
  }
}

class ManagingGetResponse {
  final ItemModel item;
  final bool canStickItem;
  final bool canUnstickItem;
  final bool canBuyItem;
  final String cannotBuyItemError;
  final bool canCloseItem;
  final bool canOpenItem;
  final bool canReportItem;
  final bool canEditItem;
  final bool canDeleteItem;
  final bool canCancelConfirmedBuy;
  final bool canViewItemHistory;
  final bool faveCount;
  final bool isVisibleItem;
  final bool canViewLoginData;
  final bool showToFavouritesButton;
  final String itemLink;
  final bool canChangeOwner;
  final List<num> sameItemsIds;
  final num sameItemsCount;
  final RespSystemInfo systemInfo;

  const ManagingGetResponse({
    required this.item,
    required this.canStickItem,
    required this.canUnstickItem,
    required this.canBuyItem,
    required this.cannotBuyItemError,
    required this.canCloseItem,
    required this.canOpenItem,
    required this.canReportItem,
    required this.canEditItem,
    required this.canDeleteItem,
    required this.canCancelConfirmedBuy,
    required this.canViewItemHistory,
    required this.faveCount,
    required this.isVisibleItem,
    required this.canViewLoginData,
    required this.showToFavouritesButton,
    required this.itemLink,
    required this.canChangeOwner,
    required this.sameItemsIds,
    required this.sameItemsCount,
    required this.systemInfo,
  });

  factory ManagingGetResponse.fromJson(Map<String, dynamic> json) => ManagingGetResponse(
    item: json['item'] is Map<String, dynamic>
            ? ItemModel.fromJson(json['item'] as Map<String, dynamic>)
            : ItemModel.fromJson(const {}),
    canStickItem: json['canStickItem'] is bool ? json['canStickItem'] as bool : false,
    canUnstickItem: json['canUnstickItem'] is bool ? json['canUnstickItem'] as bool : false,
    canBuyItem: json['canBuyItem'] is bool ? json['canBuyItem'] as bool : false,
    cannotBuyItemError: json['cannotBuyItemError'] is String ? json['cannotBuyItemError'] as String : '',
    canCloseItem: json['canCloseItem'] is bool ? json['canCloseItem'] as bool : false,
    canOpenItem: json['canOpenItem'] is bool ? json['canOpenItem'] as bool : false,
    canReportItem: json['canReportItem'] is bool ? json['canReportItem'] as bool : false,
    canEditItem: json['canEditItem'] is bool ? json['canEditItem'] as bool : false,
    canDeleteItem: json['canDeleteItem'] is bool ? json['canDeleteItem'] as bool : false,
    canCancelConfirmedBuy: json['canCancelConfirmedBuy'] is bool ? json['canCancelConfirmedBuy'] as bool : false,
    canViewItemHistory: json['canViewItemHistory'] is bool ? json['canViewItemHistory'] as bool : false,
    faveCount: json['faveCount'] is bool ? json['faveCount'] as bool : false,
    isVisibleItem: json['isVisibleItem'] is bool ? json['isVisibleItem'] as bool : false,
    canViewLoginData: json['canViewLoginData'] is bool ? json['canViewLoginData'] as bool : false,
    showToFavouritesButton: json['showToFavouritesButton'] is bool ? json['showToFavouritesButton'] as bool : false,
    itemLink: json['itemLink'] is String ? json['itemLink'] as String : '',
    canChangeOwner: json['canChangeOwner'] is bool ? json['canChangeOwner'] as bool : false,
    sameItemsIds: json['sameItemsIds'] is List ? (json['sameItemsIds'] as List<dynamic>).cast<num>() : const [],
    sameItemsCount: json['sameItemsCount'] is num ? json['sameItemsCount'] as num : 0,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingDeleteBody {
  /// Delete reason.
  final String reason;

  const ManagingDeleteBody({
    required this.reason,
  });

  Map<String, Object?> toMap() {
    return {
      'reason': reason,
    };
  }
}

class ManagingDeleteResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const ManagingDeleteResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory ManagingDeleteResponse.fromJson(Map<String, dynamic> json) => ManagingDeleteResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingCreateClaimBody {
  /// Item id.
  final num itemId;
  /// You should describe what's happened. - describe the situation in a nutshell. If you wish, you can describe the situation in more detail using the "Spoiler" function. - attach screenshots of correspondence. You must upload to the site [Imgur](https://imgur.com/upload) - other evidence; - notify the respondent about the complaint you created, familiarize him with hidden content Describe the situation in as much detail as possible.
  final String postBody;

  const ManagingCreateClaimBody({
    required this.itemId,
    required this.postBody,
  });

  Map<String, Object?> toMap() {
    return {
      'item_id': itemId,
      'post_body': postBody,
    };
  }
}

class ManagingCreateClaimResponseThreadFirstPostLinks {
  final String permalink;
  final String detail;
  final String thread;
  final String poster;
  final String likes;
  final String report;
  final String attachments;
  final String posterAvatar;

  const ManagingCreateClaimResponseThreadFirstPostLinks({
    required this.permalink,
    required this.detail,
    required this.thread,
    required this.poster,
    required this.likes,
    required this.report,
    required this.attachments,
    required this.posterAvatar,
  });

  factory ManagingCreateClaimResponseThreadFirstPostLinks.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponseThreadFirstPostLinks(
    permalink: json['permalink'] is String ? json['permalink'] as String : '',
    detail: json['detail'] is String ? json['detail'] as String : '',
    thread: json['thread'] is String ? json['thread'] as String : '',
    poster: json['poster'] is String ? json['poster'] as String : '',
    likes: json['likes'] is String ? json['likes'] as String : '',
    report: json['report'] is String ? json['report'] as String : '',
    attachments: json['attachments'] is String ? json['attachments'] as String : '',
    posterAvatar: json['poster_avatar'] is String ? json['poster_avatar'] as String : '',
  );
}

class ManagingCreateClaimResponseThreadFirstPostPermissions {
  final bool view;
  final bool edit;
  final bool delete;
  final bool reply;
  final bool like;
  final bool report;
  final bool uploadAttachment;

  const ManagingCreateClaimResponseThreadFirstPostPermissions({
    required this.view,
    required this.edit,
    required this.delete,
    required this.reply,
    required this.like,
    required this.report,
    required this.uploadAttachment,
  });

  factory ManagingCreateClaimResponseThreadFirstPostPermissions.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponseThreadFirstPostPermissions(
    view: json['view'] is bool ? json['view'] as bool : false,
    edit: json['edit'] is bool ? json['edit'] as bool : false,
    delete: json['delete'] is bool ? json['delete'] as bool : false,
    reply: json['reply'] is bool ? json['reply'] as bool : false,
    like: json['like'] is bool ? json['like'] as bool : false,
    report: json['report'] is bool ? json['report'] as bool : false,
    uploadAttachment: json['upload_attachment'] is bool ? json['upload_attachment'] as bool : false,
  );
}

class ManagingCreateClaimResponseThreadFirstPost {
  final num postId;
  final num threadId;
  final num posterUserId;
  final String posterUsername;
  final num postCreateDate;
  final String postBody;
  final String postBodyHtml;
  final String postBodyPlainText;
  final String signature;
  final String signatureHtml;
  final String signaturePlainText;
  final num postLikeCount;
  final num postAttachmentCount;
  final List<Map<String, dynamic>> likeUsers;
  final bool userIsIgnored;
  final bool postIsPublished;
  final bool postIsDeleted;
  final num postUpdateDate;
  final bool postIsFirstPost;
  final ManagingCreateClaimResponseThreadFirstPostLinks links;
  final ManagingCreateClaimResponseThreadFirstPostPermissions permissions;

  const ManagingCreateClaimResponseThreadFirstPost({
    required this.postId,
    required this.threadId,
    required this.posterUserId,
    required this.posterUsername,
    required this.postCreateDate,
    required this.postBody,
    required this.postBodyHtml,
    required this.postBodyPlainText,
    required this.signature,
    required this.signatureHtml,
    required this.signaturePlainText,
    required this.postLikeCount,
    required this.postAttachmentCount,
    required this.likeUsers,
    required this.userIsIgnored,
    required this.postIsPublished,
    required this.postIsDeleted,
    required this.postUpdateDate,
    required this.postIsFirstPost,
    required this.links,
    required this.permissions,
  });

  factory ManagingCreateClaimResponseThreadFirstPost.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponseThreadFirstPost(
    postId: json['post_id'] is num ? json['post_id'] as num : 0,
    threadId: json['thread_id'] is num ? json['thread_id'] as num : 0,
    posterUserId: json['poster_user_id'] is num ? json['poster_user_id'] as num : 0,
    posterUsername: json['poster_username'] is String ? json['poster_username'] as String : '',
    postCreateDate: json['post_create_date'] is num ? json['post_create_date'] as num : 0,
    postBody: json['post_body'] is String ? json['post_body'] as String : '',
    postBodyHtml: json['post_body_html'] is String ? json['post_body_html'] as String : '',
    postBodyPlainText: json['post_body_plain_text'] is String ? json['post_body_plain_text'] as String : '',
    signature: json['signature'] is String ? json['signature'] as String : '',
    signatureHtml: json['signature_html'] is String ? json['signature_html'] as String : '',
    signaturePlainText: json['signature_plain_text'] is String ? json['signature_plain_text'] as String : '',
    postLikeCount: json['post_like_count'] is num ? json['post_like_count'] as num : 0,
    postAttachmentCount: json['post_attachment_count'] is num ? json['post_attachment_count'] as num : 0,
    likeUsers: json['like_users'] is List ? (json['like_users'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    userIsIgnored: json['user_is_ignored'] is bool ? json['user_is_ignored'] as bool : false,
    postIsPublished: json['post_is_published'] is bool ? json['post_is_published'] as bool : false,
    postIsDeleted: json['post_is_deleted'] is bool ? json['post_is_deleted'] as bool : false,
    postUpdateDate: json['post_update_date'] is num ? json['post_update_date'] as num : 0,
    postIsFirstPost: json['post_is_first_post'] is bool ? json['post_is_first_post'] as bool : false,
    links: json['links'] is Map<String, dynamic>
            ? ManagingCreateClaimResponseThreadFirstPostLinks.fromJson(json['links'] as Map<String, dynamic>)
            : ManagingCreateClaimResponseThreadFirstPostLinks.fromJson(const {}),
    permissions: json['permissions'] is Map<String, dynamic>
            ? ManagingCreateClaimResponseThreadFirstPostPermissions.fromJson(json['permissions'] as Map<String, dynamic>)
            : ManagingCreateClaimResponseThreadFirstPostPermissions.fromJson(const {}),
  );
}

class ManagingCreateClaimResponseThreadLinks {
  final String permalink;
  final String detail;
  final String followers;
  final String forum;
  final String posts;
  final String firstPoster;
  final String firstPosterAvatar;
  final String firstPost;
  final String lastPoster;
  final String lastPost;

  const ManagingCreateClaimResponseThreadLinks({
    required this.permalink,
    required this.detail,
    required this.followers,
    required this.forum,
    required this.posts,
    required this.firstPoster,
    required this.firstPosterAvatar,
    required this.firstPost,
    required this.lastPoster,
    required this.lastPost,
  });

  factory ManagingCreateClaimResponseThreadLinks.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponseThreadLinks(
    permalink: json['permalink'] is String ? json['permalink'] as String : '',
    detail: json['detail'] is String ? json['detail'] as String : '',
    followers: json['followers'] is String ? json['followers'] as String : '',
    forum: json['forum'] is String ? json['forum'] as String : '',
    posts: json['posts'] is String ? json['posts'] as String : '',
    firstPoster: json['first_poster'] is String ? json['first_poster'] as String : '',
    firstPosterAvatar: json['first_poster_avatar'] is String ? json['first_poster_avatar'] as String : '',
    firstPost: json['first_post'] is String ? json['first_post'] as String : '',
    lastPoster: json['last_poster'] is String ? json['last_poster'] as String : '',
    lastPost: json['last_post'] is String ? json['last_post'] as String : '',
  );
}

class ManagingCreateClaimResponseThreadPermissions {
  final bool view;
  final bool delete;
  final bool follow;
  final bool post;
  final bool uploadAttachment;
  final bool edit;

  const ManagingCreateClaimResponseThreadPermissions({
    required this.view,
    required this.delete,
    required this.follow,
    required this.post,
    required this.uploadAttachment,
    required this.edit,
  });

  factory ManagingCreateClaimResponseThreadPermissions.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponseThreadPermissions(
    view: json['view'] is bool ? json['view'] as bool : false,
    delete: json['delete'] is bool ? json['delete'] as bool : false,
    follow: json['follow'] is bool ? json['follow'] as bool : false,
    post: json['post'] is bool ? json['post'] as bool : false,
    uploadAttachment: json['upload_attachment'] is bool ? json['upload_attachment'] as bool : false,
    edit: json['edit'] is bool ? json['edit'] as bool : false,
  );
}

class ManagingCreateClaimResponseThreadForumLinks {
  final String permalink;
  final String detail;
  final String subCategories;
  final String subForums;
  final String threads;
  final String followers;

  const ManagingCreateClaimResponseThreadForumLinks({
    required this.permalink,
    required this.detail,
    required this.subCategories,
    required this.subForums,
    required this.threads,
    required this.followers,
  });

  factory ManagingCreateClaimResponseThreadForumLinks.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponseThreadForumLinks(
    permalink: json['permalink'] is String ? json['permalink'] as String : '',
    detail: json['detail'] is String ? json['detail'] as String : '',
    subCategories: json['sub-categories'] is String ? json['sub-categories'] as String : '',
    subForums: json['sub-forums'] is String ? json['sub-forums'] as String : '',
    threads: json['threads'] is String ? json['threads'] as String : '',
    followers: json['followers'] is String ? json['followers'] as String : '',
  );
}

class ManagingCreateClaimResponseThreadForumPermissions {
  final bool view;
  final bool edit;
  final bool delete;
  final bool createThread;
  final bool uploadAttachment;
  final bool tagThread;
  final bool follow;

  const ManagingCreateClaimResponseThreadForumPermissions({
    required this.view,
    required this.edit,
    required this.delete,
    required this.createThread,
    required this.uploadAttachment,
    required this.tagThread,
    required this.follow,
  });

  factory ManagingCreateClaimResponseThreadForumPermissions.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponseThreadForumPermissions(
    view: json['view'] is bool ? json['view'] as bool : false,
    edit: json['edit'] is bool ? json['edit'] as bool : false,
    delete: json['delete'] is bool ? json['delete'] as bool : false,
    createThread: json['create_thread'] is bool ? json['create_thread'] as bool : false,
    uploadAttachment: json['upload_attachment'] is bool ? json['upload_attachment'] as bool : false,
    tagThread: json['tag_thread'] is bool ? json['tag_thread'] as bool : false,
    follow: json['follow'] is bool ? json['follow'] as bool : false,
  );
}

class ManagingCreateClaimResponseThreadForum {
  final num forumId;
  final String forumTitle;
  final String forumDescription;
  final num forumThreadCount;
  final num forumPostCount;
  final List<Map<String, dynamic>> forumPrefixes;
  final num threadDefaultPrefixId;
  final bool threadPrefixIsRequired;
  final ManagingCreateClaimResponseThreadForumLinks links;
  final ManagingCreateClaimResponseThreadForumPermissions permissions;
  final bool forumIsFollowed;

  const ManagingCreateClaimResponseThreadForum({
    required this.forumId,
    required this.forumTitle,
    required this.forumDescription,
    required this.forumThreadCount,
    required this.forumPostCount,
    required this.forumPrefixes,
    required this.threadDefaultPrefixId,
    required this.threadPrefixIsRequired,
    required this.links,
    required this.permissions,
    required this.forumIsFollowed,
  });

  factory ManagingCreateClaimResponseThreadForum.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponseThreadForum(
    forumId: json['forum_id'] is num ? json['forum_id'] as num : 0,
    forumTitle: json['forum_title'] is String ? json['forum_title'] as String : '',
    forumDescription: json['forum_description'] is String ? json['forum_description'] as String : '',
    forumThreadCount: json['forum_thread_count'] is num ? json['forum_thread_count'] as num : 0,
    forumPostCount: json['forum_post_count'] is num ? json['forum_post_count'] as num : 0,
    forumPrefixes: json['forum_prefixes'] is List ? (json['forum_prefixes'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    threadDefaultPrefixId: json['thread_default_prefix_id'] is num ? json['thread_default_prefix_id'] as num : 0,
    threadPrefixIsRequired: json['thread_prefix_is_required'] is bool ? json['thread_prefix_is_required'] as bool : false,
    links: json['links'] is Map<String, dynamic>
            ? ManagingCreateClaimResponseThreadForumLinks.fromJson(json['links'] as Map<String, dynamic>)
            : ManagingCreateClaimResponseThreadForumLinks.fromJson(const {}),
    permissions: json['permissions'] is Map<String, dynamic>
            ? ManagingCreateClaimResponseThreadForumPermissions.fromJson(json['permissions'] as Map<String, dynamic>)
            : ManagingCreateClaimResponseThreadForumPermissions.fromJson(const {}),
    forumIsFollowed: json['forum_is_followed'] is bool ? json['forum_is_followed'] as bool : false,
  );
}

class ManagingCreateClaimResponseThread {
  final num threadId;
  final num forumId;
  final String threadTitle;
  final num threadViewCount;
  final num creatorUserId;
  final String creatorUsername;
  final num threadCreateDate;
  final num threadUpdateDate;
  final bool userIsIgnored;
  final num threadPostCount;
  final bool threadIsPublished;
  final bool threadIsDeleted;
  final bool threadIsSticky;
  final bool threadIsFollowed;
  final ManagingCreateClaimResponseThreadFirstPost firstPost;
  final List<dynamic> threadPrefixes;
  final List<dynamic> threadTags;
  final ManagingCreateClaimResponseThreadLinks links;
  final ManagingCreateClaimResponseThreadPermissions permissions;
  final ManagingCreateClaimResponseThreadForum forum;

  const ManagingCreateClaimResponseThread({
    required this.threadId,
    required this.forumId,
    required this.threadTitle,
    required this.threadViewCount,
    required this.creatorUserId,
    required this.creatorUsername,
    required this.threadCreateDate,
    required this.threadUpdateDate,
    required this.userIsIgnored,
    required this.threadPostCount,
    required this.threadIsPublished,
    required this.threadIsDeleted,
    required this.threadIsSticky,
    required this.threadIsFollowed,
    required this.firstPost,
    required this.threadPrefixes,
    required this.threadTags,
    required this.links,
    required this.permissions,
    required this.forum,
  });

  factory ManagingCreateClaimResponseThread.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponseThread(
    threadId: json['thread_id'] is num ? json['thread_id'] as num : 0,
    forumId: json['forum_id'] is num ? json['forum_id'] as num : 0,
    threadTitle: json['thread_title'] is String ? json['thread_title'] as String : '',
    threadViewCount: json['thread_view_count'] is num ? json['thread_view_count'] as num : 0,
    creatorUserId: json['creator_user_id'] is num ? json['creator_user_id'] as num : 0,
    creatorUsername: json['creator_username'] is String ? json['creator_username'] as String : '',
    threadCreateDate: json['thread_create_date'] is num ? json['thread_create_date'] as num : 0,
    threadUpdateDate: json['thread_update_date'] is num ? json['thread_update_date'] as num : 0,
    userIsIgnored: json['user_is_ignored'] is bool ? json['user_is_ignored'] as bool : false,
    threadPostCount: json['thread_post_count'] is num ? json['thread_post_count'] as num : 0,
    threadIsPublished: json['thread_is_published'] is bool ? json['thread_is_published'] as bool : false,
    threadIsDeleted: json['thread_is_deleted'] is bool ? json['thread_is_deleted'] as bool : false,
    threadIsSticky: json['thread_is_sticky'] is bool ? json['thread_is_sticky'] as bool : false,
    threadIsFollowed: json['thread_is_followed'] is bool ? json['thread_is_followed'] as bool : false,
    firstPost: json['first_post'] is Map<String, dynamic>
            ? ManagingCreateClaimResponseThreadFirstPost.fromJson(json['first_post'] as Map<String, dynamic>)
            : ManagingCreateClaimResponseThreadFirstPost.fromJson(const {}),
    threadPrefixes: json['thread_prefixes'] is List ? json['thread_prefixes'] as List<dynamic> : const [],
    threadTags: json['thread_tags'] is List ? json['thread_tags'] as List<dynamic> : const [],
    links: json['links'] is Map<String, dynamic>
            ? ManagingCreateClaimResponseThreadLinks.fromJson(json['links'] as Map<String, dynamic>)
            : ManagingCreateClaimResponseThreadLinks.fromJson(const {}),
    permissions: json['permissions'] is Map<String, dynamic>
            ? ManagingCreateClaimResponseThreadPermissions.fromJson(json['permissions'] as Map<String, dynamic>)
            : ManagingCreateClaimResponseThreadPermissions.fromJson(const {}),
    forum: json['forum'] is Map<String, dynamic>
            ? ManagingCreateClaimResponseThreadForum.fromJson(json['forum'] as Map<String, dynamic>)
            : ManagingCreateClaimResponseThreadForum.fromJson(const {}),
  );
}

class ManagingCreateClaimResponseSystemInfo {
  final num visitorId;
  final num time;

  const ManagingCreateClaimResponseSystemInfo({
    required this.visitorId,
    required this.time,
  });

  factory ManagingCreateClaimResponseSystemInfo.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponseSystemInfo(
    visitorId: json['visitor_id'] is num ? json['visitor_id'] as num : 0,
    time: json['time'] is num ? json['time'] as num : 0,
  );
}

class ManagingCreateClaimResponse {
  final ManagingCreateClaimResponseThread thread;
  final ManagingCreateClaimResponseSystemInfo systemInfo;

  const ManagingCreateClaimResponse({
    required this.thread,
    required this.systemInfo,
  });

  factory ManagingCreateClaimResponse.fromJson(Map<String, dynamic> json) => ManagingCreateClaimResponse(
    thread: json['thread'] is Map<String, dynamic>
            ? ManagingCreateClaimResponseThread.fromJson(json['thread'] as Map<String, dynamic>)
            : ManagingCreateClaimResponseThread.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? ManagingCreateClaimResponseSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : ManagingCreateClaimResponseSystemInfo.fromJson(const {}),
  );
}

class ManagingBulkGetBody {
  /// Item id.
  final List<num>? itemId;
  /// Parse same item ids.
  final bool? parseSameItemIds;

  const ManagingBulkGetBody({
    this.itemId,
    this.parseSameItemIds,
  });

  Map<String, Object?> toMap() {
    return {
      'item_id': itemId,
      'parse_same_item_ids': parseSameItemIds,
    };
  }
}

class ManagingBulkGetResponse {
  final List<Map<String, dynamic>> items;
  final List<num> leftItemId;
  final RespSystemInfo systemInfo;

  const ManagingBulkGetResponse({
    required this.items,
    required this.leftItemId,
    required this.systemInfo,
  });

  factory ManagingBulkGetResponse.fromJson(Map<String, dynamic> json) => ManagingBulkGetResponse(
    items: json['items'] is List ? (json['items'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    leftItemId: json['left_item_id'] is List ? (json['left_item_id'] as List<dynamic>).cast<num>() : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingSteamInventoryValueParams {
  final AppId? appId;
  final Currency? currency;
  /// Ignore cache.
  final bool? ignoreCache;

  const ManagingSteamInventoryValueParams({
    this.appId,
    this.currency,
    this.ignoreCache,
  });

  Map<String, Object?> toMap() {
    return {
      'app_id': appId?.value,
      'currency': currency?.value,
      'ignore_cache': ignoreCache,
    };
  }
}

class ManagingSteamInventoryValueResponseData {
  final dynamic items;
  final String steamId;
  final num appId;
  final String appTitle;
  final num totalValue;
  final num itemCount;
  final num marketableItemCount;
  final String currency;
  final String currencyIcon;
  final String language;
  final num time;

  const ManagingSteamInventoryValueResponseData({
    required this.items,
    required this.steamId,
    required this.appId,
    required this.appTitle,
    required this.totalValue,
    required this.itemCount,
    required this.marketableItemCount,
    required this.currency,
    required this.currencyIcon,
    required this.language,
    required this.time,
  });

  factory ManagingSteamInventoryValueResponseData.fromJson(Map<String, dynamic> json) => ManagingSteamInventoryValueResponseData(
    items: json['items'],
    steamId: json['steam_id'] is String ? json['steam_id'] as String : '',
    appId: json['appId'] is num ? json['appId'] as num : 0,
    appTitle: json['appTitle'] is String ? json['appTitle'] as String : '',
    totalValue: json['totalValue'] is num ? json['totalValue'] as num : 0,
    itemCount: json['itemCount'] is num ? json['itemCount'] as num : 0,
    marketableItemCount: json['marketableItemCount'] is num ? json['marketableItemCount'] as num : 0,
    currency: json['currency'] is String ? json['currency'] as String : '',
    currencyIcon: json['currencyIcon'] is String ? json['currencyIcon'] as String : '',
    language: json['language'] is String ? json['language'] as String : '',
    time: json['time'] is num ? json['time'] as num : 0,
  );
}

class ManagingSteamInventoryValueResponse {
  final String? query;
  final ManagingSteamInventoryValueResponseData? data;
  final num? appId;
  final RespSystemInfo? systemInfo;

  const ManagingSteamInventoryValueResponse({
    this.query,
    this.data,
    this.appId,
    this.systemInfo,
  });

  factory ManagingSteamInventoryValueResponse.fromJson(Map<String, dynamic> json) => ManagingSteamInventoryValueResponse(
    query: json['query'] is String ? json['query'] as String : null,
    data: json['data'] is Map<String, dynamic>
            ? ManagingSteamInventoryValueResponseData.fromJson(json['data'] as Map<String, dynamic>)
            : null,
    appId: json['appId'] is num ? json['appId'] as num : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ManagingSteamValueParams {
  /// Link or id of account. Can be [$1, $1, $1, $1].
  final String link;
  final AppId? appId;
  final Currency? currency;
  /// Ignore cache.
  final bool? ignoreCache;

  const ManagingSteamValueParams({
    required this.link,
    this.appId,
    this.currency,
    this.ignoreCache,
  });

  Map<String, Object?> toMap() {
    return {
      'link': link,
      'app_id': appId?.value,
      'currency': currency?.value,
      'ignore_cache': ignoreCache,
    };
  }
}

class ManagingSteamValueResponseData {
  final dynamic items;
  final String steamId;
  final num appId;
  final String appTitle;
  final num totalValue;
  final num itemCount;
  final num marketableItemCount;
  final String currency;
  final String currencyIcon;
  final String language;
  final num time;

  const ManagingSteamValueResponseData({
    required this.items,
    required this.steamId,
    required this.appId,
    required this.appTitle,
    required this.totalValue,
    required this.itemCount,
    required this.marketableItemCount,
    required this.currency,
    required this.currencyIcon,
    required this.language,
    required this.time,
  });

  factory ManagingSteamValueResponseData.fromJson(Map<String, dynamic> json) => ManagingSteamValueResponseData(
    items: json['items'],
    steamId: json['steam_id'] is String ? json['steam_id'] as String : '',
    appId: json['appId'] is num ? json['appId'] as num : 0,
    appTitle: json['appTitle'] is String ? json['appTitle'] as String : '',
    totalValue: json['totalValue'] is num ? json['totalValue'] as num : 0,
    itemCount: json['itemCount'] is num ? json['itemCount'] as num : 0,
    marketableItemCount: json['marketableItemCount'] is num ? json['marketableItemCount'] as num : 0,
    currency: json['currency'] is String ? json['currency'] as String : '',
    currencyIcon: json['currencyIcon'] is String ? json['currencyIcon'] as String : '',
    language: json['language'] is String ? json['language'] as String : '',
    time: json['time'] is num ? json['time'] as num : 0,
  );
}

class ManagingSteamValueResponse {
  final String? query;
  final ManagingSteamValueResponseData? data;
  final num? appId;
  final RespSystemInfo? systemInfo;

  const ManagingSteamValueResponse({
    this.query,
    this.data,
    this.appId,
    this.systemInfo,
  });

  factory ManagingSteamValueResponse.fromJson(Map<String, dynamic> json) => ManagingSteamValueResponse(
    query: json['query'] is String ? json['query'] as String : null,
    data: json['data'] is Map<String, dynamic>
            ? ManagingSteamValueResponseData.fromJson(json['data'] as Map<String, dynamic>)
            : null,
    appId: json['appId'] is num ? json['appId'] as num : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ManagingSteamPreviewParams {
  final Type? type;

  const ManagingSteamPreviewParams({
    this.type,
  });

  Map<String, Object?> toMap() {
    return {
      'type': type?.value,
    };
  }
}

class ManagingEditBody {
  /// Title of account. If $1 specified and $1 is empty, $1 will be automatically translated to English language.
  final String? title;
  /// English title of account. If $1 specified and $1 is empty, $1 will be automatically translated to Russian language.
  final String? titleEn;
  /// Current price of account in your currency.
  final num? price;
  final Currency? currency;
  final ItemOrigin? itemOrigin;
  /// Email login data (email:password format).
  final String? emailLoginData;
  final EmailType? emailType;
  /// Allow users to ask discount for this account.
  final bool? allowAskDiscount;
  /// Using proxy id for account checking. See GET or POST /proxy to get or edit proxy list.
  final num? proxyId;
  /// Account public description.
  final String? description;
  /// Account private information (visible only for buyer).
  final String? information;

  const ManagingEditBody({
    this.title,
    this.titleEn,
    this.price,
    this.currency,
    this.itemOrigin,
    this.emailLoginData,
    this.emailType,
    this.allowAskDiscount,
    this.proxyId,
    this.description,
    this.information,
  });

  Map<String, Object?> toMap() {
    return {
      'title': title,
      'title_en': titleEn,
      'price': price,
      'currency': currency?.value,
      'item_origin': itemOrigin?.value,
      'email_login_data': emailLoginData,
      'email_type': emailType?.value,
      'allow_ask_discount': allowAskDiscount,
      'proxy_id': proxyId,
      'description': description,
      'information': information,
    };
  }
}

class ManagingEditResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ManagingEditResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ManagingEditResponse.fromJson(Map<String, dynamic> json) => ManagingEditResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ManagingAIPriceResponse {
  final num price;
  final RespSystemInfo systemInfo;

  const ManagingAIPriceResponse({
    required this.price,
    required this.systemInfo,
  });

  factory ManagingAIPriceResponse.fromJson(Map<String, dynamic> json) => ManagingAIPriceResponse(
    price: json['price'] is num ? json['price'] as num : 0,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingAutoBuyPriceResponse {
  final num price;
  final RespSystemInfo systemInfo;

  const ManagingAutoBuyPriceResponse({
    required this.price,
    required this.systemInfo,
  });

  factory ManagingAutoBuyPriceResponse.fromJson(Map<String, dynamic> json) => ManagingAutoBuyPriceResponse(
    price: json['price'] is num ? json['price'] as num : 0,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingNoteBody {
  /// Text of note.
  final String? text;

  const ManagingNoteBody({
    this.text,
  });

  Map<String, Object?> toMap() {
    return {
      'text': text,
    };
  }
}

class ManagingNoteResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const ManagingNoteResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory ManagingNoteResponse.fromJson(Map<String, dynamic> json) => ManagingNoteResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingSteamUpdateValueBody {
  /// Update the entire Steam inventory.
  final bool? all;
  final AppId? appId;
  /// Parse inventory when authorized (Parse trade banned items).
  final bool? authorize;

  const ManagingSteamUpdateValueBody({
    this.all,
    this.appId,
    this.authorize,
  });

  Map<String, Object?> toMap() {
    return {
      'all': all,
      'app_id': appId?.value,
      'authorize': authorize,
    };
  }
}

class ManagingSteamUpdateValueResponse {
  final String status;
  final ItemModel item;
  final RespSystemInfo systemInfo;

  const ManagingSteamUpdateValueResponse({
    required this.status,
    required this.item,
    required this.systemInfo,
  });

  factory ManagingSteamUpdateValueResponse.fromJson(Map<String, dynamic> json) => ManagingSteamUpdateValueResponse(
    status: json['status'] is String ? json['status'] as String : '',
    item: json['item'] is Map<String, dynamic>
            ? ItemModel.fromJson(json['item'] as Map<String, dynamic>)
            : ItemModel.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingBumpResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const ManagingBumpResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory ManagingBumpResponse.fromJson(Map<String, dynamic> json) => ManagingBumpResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingAutoBumpBody {
  /// Interval in hours.
  final num hour;

  const ManagingAutoBumpBody({
    required this.hour,
  });

  Map<String, Object?> toMap() {
    return {
      'hour': hour,
    };
  }
}

class ManagingAutoBumpResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const ManagingAutoBumpResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory ManagingAutoBumpResponse.fromJson(Map<String, dynamic> json) => ManagingAutoBumpResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingAutoBumpDisableResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const ManagingAutoBumpDisableResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory ManagingAutoBumpDisableResponse.fromJson(Map<String, dynamic> json) => ManagingAutoBumpDisableResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingOpenResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const ManagingOpenResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory ManagingOpenResponse.fromJson(Map<String, dynamic> json) => ManagingOpenResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingCloseResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const ManagingCloseResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory ManagingCloseResponse.fromJson(Map<String, dynamic> json) => ManagingCloseResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingImageParams {
  final Type type;

  const ManagingImageParams({
    required this.type,
  });

  Map<String, Object?> toMap() {
    return {
      'type': type.value,
    };
  }
}

class ManagingImageResponse {
  final String base64;
  final RespSystemInfo systemInfo;

  const ManagingImageResponse({
    required this.base64,
    required this.systemInfo,
  });

  factory ManagingImageResponse.fromJson(Map<String, dynamic> json) => ManagingImageResponse(
    base64: json['base64'] is String ? json['base64'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingEmailCodeResponseCodeData {
  final String code;
  final num date;
  final String textPlain;

  const ManagingEmailCodeResponseCodeData({
    required this.code,
    required this.date,
    required this.textPlain,
  });

  factory ManagingEmailCodeResponseCodeData.fromJson(Map<String, dynamic> json) => ManagingEmailCodeResponseCodeData(
    code: json['code'] is String ? json['code'] as String : '',
    date: json['date'] is num ? json['date'] as num : 0,
    textPlain: json['textPlain'] is String ? json['textPlain'] as String : '',
  );
}

class ManagingEmailCodeResponse {
  final ItemModel item;
  final ManagingEmailCodeResponseCodeData codeData;

  const ManagingEmailCodeResponse({
    required this.item,
    required this.codeData,
  });

  factory ManagingEmailCodeResponse.fromJson(Map<String, dynamic> json) => ManagingEmailCodeResponse(
    item: json['item'] is Map<String, dynamic>
            ? ItemModel.fromJson(json['item'] as Map<String, dynamic>)
            : ItemModel.fromJson(const {}),
    codeData: json['codeData'] is Map<String, dynamic>
            ? ManagingEmailCodeResponseCodeData.fromJson(json['codeData'] as Map<String, dynamic>)
            : ManagingEmailCodeResponseCodeData.fromJson(const {}),
  );
}

class ManagingGetLetters2Params {
  /// Email login data (email:password format). Required if both *email* and *password* are not provided.
  final String? emailPassword;
  /// Email. Required if *email_password* is not provided.
  final String? email;
  /// Password. Required if *email_password* is not provided.
  final String? password;
  /// Number of letters to return.
  final num? limit;

  const ManagingGetLetters2Params({
    this.emailPassword,
    this.email,
    this.password,
    this.limit,
  });

  Map<String, Object?> toMap() {
    return {
      'email_password': emailPassword,
      'email': email,
      'password': password,
      'limit': limit,
    };
  }
}

class ManagingGetLetters2Response {
  final String email;
  final List<Map<String, dynamic>> letters;
  final RespSystemInfo systemInfo;

  const ManagingGetLetters2Response({
    required this.email,
    required this.letters,
    required this.systemInfo,
  });

  factory ManagingGetLetters2Response.fromJson(Map<String, dynamic> json) => ManagingGetLetters2Response(
    email: json['email'] is String ? json['email'] as String : '',
    letters: json['letters'] is List ? (json['letters'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingSteamGetMafileResponseMaFileSession {
  final String sessionID;
  final String accessToken;
  final String refreshToken;
  final String steamID;
  final String steamLoginSecure;

  const ManagingSteamGetMafileResponseMaFileSession({
    required this.sessionID,
    required this.accessToken,
    required this.refreshToken,
    required this.steamID,
    required this.steamLoginSecure,
  });

  factory ManagingSteamGetMafileResponseMaFileSession.fromJson(Map<String, dynamic> json) => ManagingSteamGetMafileResponseMaFileSession(
    sessionID: json['SessionID'] is String ? json['SessionID'] as String : '',
    accessToken: json['AccessToken'] is String ? json['AccessToken'] as String : '',
    refreshToken: json['RefreshToken'] is String ? json['RefreshToken'] as String : '',
    steamID: json['SteamID'] is String ? json['SteamID'] as String : '',
    steamLoginSecure: json['SteamLoginSecure'] is String ? json['SteamLoginSecure'] as String : '',
  );
}

class ManagingSteamGetMafileResponseMaFile {
  final String sharedSecret;
  final num serialNumber;
  final String revocationCode;
  final String uri;
  final String accountName;
  final String tokenGid;
  final String identitySecret;
  final String secret_1;
  final String deviceId;
  final bool fullyEnrolled;
  final ManagingSteamGetMafileResponseMaFileSession session;

  const ManagingSteamGetMafileResponseMaFile({
    required this.sharedSecret,
    required this.serialNumber,
    required this.revocationCode,
    required this.uri,
    required this.accountName,
    required this.tokenGid,
    required this.identitySecret,
    required this.secret_1,
    required this.deviceId,
    required this.fullyEnrolled,
    required this.session,
  });

  factory ManagingSteamGetMafileResponseMaFile.fromJson(Map<String, dynamic> json) => ManagingSteamGetMafileResponseMaFile(
    sharedSecret: json['shared_secret'] is String ? json['shared_secret'] as String : '',
    serialNumber: json['serial_number'] is num ? json['serial_number'] as num : 0,
    revocationCode: json['revocation_code'] is String ? json['revocation_code'] as String : '',
    uri: json['uri'] is String ? json['uri'] as String : '',
    accountName: json['account_name'] is String ? json['account_name'] as String : '',
    tokenGid: json['token_gid'] is String ? json['token_gid'] as String : '',
    identitySecret: json['identity_secret'] is String ? json['identity_secret'] as String : '',
    secret_1: json['secret_1'] is String ? json['secret_1'] as String : '',
    deviceId: json['device_id'] is String ? json['device_id'] as String : '',
    fullyEnrolled: json['fully_enrolled'] is bool ? json['fully_enrolled'] as bool : false,
    session: json['Session'] is Map<String, dynamic>
            ? ManagingSteamGetMafileResponseMaFileSession.fromJson(json['Session'] as Map<String, dynamic>)
            : ManagingSteamGetMafileResponseMaFileSession.fromJson(const {}),
  );
}

class ManagingSteamGetMafileResponse {
  final ManagingSteamGetMafileResponseMaFile maFile;
  final RespSystemInfo systemInfo;

  const ManagingSteamGetMafileResponse({
    required this.maFile,
    required this.systemInfo,
  });

  factory ManagingSteamGetMafileResponse.fromJson(Map<String, dynamic> json) => ManagingSteamGetMafileResponse(
    maFile: json['maFile'] is Map<String, dynamic>
            ? ManagingSteamGetMafileResponseMaFile.fromJson(json['maFile'] as Map<String, dynamic>)
            : ManagingSteamGetMafileResponseMaFile.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingSteamAddMafileResponse {
  final String status;
  final String message;
  final ItemModel item;
  final RespSystemInfo systemInfo;

  const ManagingSteamAddMafileResponse({
    required this.status,
    required this.message,
    required this.item,
    required this.systemInfo,
  });

  factory ManagingSteamAddMafileResponse.fromJson(Map<String, dynamic> json) => ManagingSteamAddMafileResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    item: json['item'] is Map<String, dynamic>
            ? ItemModel.fromJson(json['item'] as Map<String, dynamic>)
            : ItemModel.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingSteamRemoveMafileResponse {
  final String status;
  final String message;
  final ItemModel item;
  final RespSystemInfo systemInfo;

  const ManagingSteamRemoveMafileResponse({
    required this.status,
    required this.message,
    required this.item,
    required this.systemInfo,
  });

  factory ManagingSteamRemoveMafileResponse.fromJson(Map<String, dynamic> json) => ManagingSteamRemoveMafileResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    item: json['item'] is Map<String, dynamic>
            ? ItemModel.fromJson(json['item'] as Map<String, dynamic>)
            : ItemModel.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingSteamMafileCodeResponseCodeData {
  final String code;
  final num date;
  final String textPlain;

  const ManagingSteamMafileCodeResponseCodeData({
    required this.code,
    required this.date,
    required this.textPlain,
  });

  factory ManagingSteamMafileCodeResponseCodeData.fromJson(Map<String, dynamic> json) => ManagingSteamMafileCodeResponseCodeData(
    code: json['code'] is String ? json['code'] as String : '',
    date: json['date'] is num ? json['date'] as num : 0,
    textPlain: json['textPlain'] is String ? json['textPlain'] as String : '',
  );
}

class ManagingSteamMafileCodeResponse {
  final ItemModel item;
  final ManagingSteamMafileCodeResponseCodeData codeData;

  const ManagingSteamMafileCodeResponse({
    required this.item,
    required this.codeData,
  });

  factory ManagingSteamMafileCodeResponse.fromJson(Map<String, dynamic> json) => ManagingSteamMafileCodeResponse(
    item: json['item'] is Map<String, dynamic>
            ? ItemModel.fromJson(json['item'] as Map<String, dynamic>)
            : ItemModel.fromJson(const {}),
    codeData: json['codeData'] is Map<String, dynamic>
            ? ManagingSteamMafileCodeResponseCodeData.fromJson(json['codeData'] as Map<String, dynamic>)
            : ManagingSteamMafileCodeResponseCodeData.fromJson(const {}),
  );
}

class ManagingSteamSDABody {
  /// Confirmation id. (Required along with $1 if you want to confirm action).
  final num? id;
  /// Confirmation nonce. (Required along with $1 if you want to confirm action).
  final num? nonce;

  const ManagingSteamSDABody({
    this.id,
    this.nonce,
  });

  Map<String, Object?> toMap() {
    return {
      'id': id,
      'nonce': nonce,
    };
  }
}

class ManagingSteamSDAResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const ManagingSteamSDAResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory ManagingSteamSDAResponse.fromJson(Map<String, dynamic> json) => ManagingSteamSDAResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingTelegramCodeResponseCodes {
  final String? code;
  final num? date;

  const ManagingTelegramCodeResponseCodes({
    this.code,
    this.date,
  });

  factory ManagingTelegramCodeResponseCodes.fromJson(Map<String, dynamic> json) => ManagingTelegramCodeResponseCodes(
    code: json['code'] is String ? json['code'] as String : null,
    date: json['date'] is num ? json['date'] as num : null,
  );
}

class ManagingTelegramCodeResponse {
  final ItemModel item;
  final ManagingTelegramCodeResponseCodes codes;

  const ManagingTelegramCodeResponse({
    required this.item,
    required this.codes,
  });

  factory ManagingTelegramCodeResponse.fromJson(Map<String, dynamic> json) => ManagingTelegramCodeResponse(
    item: json['item'] is Map<String, dynamic>
            ? ItemModel.fromJson(json['item'] as Map<String, dynamic>)
            : ItemModel.fromJson(const {}),
    codes: json['codes'] is Map<String, dynamic>
            ? ManagingTelegramCodeResponseCodes.fromJson(json['codes'] as Map<String, dynamic>)
            : ManagingTelegramCodeResponseCodes.fromJson(const {}),
  );
}

class ManagingTelegramResetAuthResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ManagingTelegramResetAuthResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ManagingTelegramResetAuthResponse.fromJson(Map<String, dynamic> json) => ManagingTelegramResetAuthResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ManagingRefuseGuaranteeResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ManagingRefuseGuaranteeResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ManagingRefuseGuaranteeResponse.fromJson(Map<String, dynamic> json) => ManagingRefuseGuaranteeResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ManagingDeclineVideoRecordingBody {
  /// You voluntarily and with full awareness of your actions waive any claims regarding this account.
  final bool iVoluntarilyAndWithFullAwarenessOfMyActionsWaiveAnyClaimsRegardingThisItem;

  const ManagingDeclineVideoRecordingBody({
    required this.iVoluntarilyAndWithFullAwarenessOfMyActionsWaiveAnyClaimsRegardingThisItem,
  });

  Map<String, Object?> toMap() {
    return {
      'i_voluntarily_and_with_full_awareness_of_my_actions_waive_any_claims_regarding_this_item': iVoluntarilyAndWithFullAwarenessOfMyActionsWaiveAnyClaimsRegardingThisItem,
    };
  }
}

class ManagingDeclineVideoRecordingResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ManagingDeclineVideoRecordingResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ManagingDeclineVideoRecordingResponse.fromJson(Map<String, dynamic> json) => ManagingDeclineVideoRecordingResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ManagingCheckGuaranteeResponse {
  final String message;
  final RespSystemInfo systemInfo;

  const ManagingCheckGuaranteeResponse({
    required this.message,
    required this.systemInfo,
  });

  factory ManagingCheckGuaranteeResponse.fromJson(Map<String, dynamic> json) => ManagingCheckGuaranteeResponse(
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingChangePasswordBody {
  final Cancel? cancel;

  const ManagingChangePasswordBody({
    this.cancel,
  });

  Map<String, Object?> toMap() {
    return {
      '_cancel': cancel?.value,
    };
  }
}

class ManagingChangePasswordResponse {
  final String? status;
  final String? message;
  final String newPassword;

  const ManagingChangePasswordResponse({
    this.status,
    this.message,
    required this.newPassword,
  });

  factory ManagingChangePasswordResponse.fromJson(Map<String, dynamic> json) => ManagingChangePasswordResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    newPassword: json['new_password'] is String ? json['new_password'] as String : '',
  );
}

class ManagingTempEmailPasswordResponseItem {
  final String account;

  const ManagingTempEmailPasswordResponseItem({
    required this.account,
  });

  factory ManagingTempEmailPasswordResponseItem.fromJson(Map<String, dynamic> json) => ManagingTempEmailPasswordResponseItem(
    account: json['account'] is String ? json['account'] as String : '',
  );
}

class ManagingTempEmailPasswordResponse {
  final ManagingTempEmailPasswordResponseItem item;

  const ManagingTempEmailPasswordResponse({
    required this.item,
  });

  factory ManagingTempEmailPasswordResponse.fromJson(Map<String, dynamic> json) => ManagingTempEmailPasswordResponse(
    item: json['item'] is Map<String, dynamic>
            ? ManagingTempEmailPasswordResponseItem.fromJson(json['item'] as Map<String, dynamic>)
            : ManagingTempEmailPasswordResponseItem.fromJson(const {}),
  );
}

class ManagingTagBody {
  /// Tag ID.
  final num tagId;

  const ManagingTagBody({
    required this.tagId,
  });

  Map<String, Object?> toMap() {
    return {
      'tag_id': tagId,
    };
  }
}

class ManagingTagResponseTag {
  final num tagId;
  final String title;
  final bool isDefault;
  final bool forOwnedAccountsOnly;
  final String bc;

  const ManagingTagResponseTag({
    required this.tagId,
    required this.title,
    required this.isDefault,
    required this.forOwnedAccountsOnly,
    required this.bc,
  });

  factory ManagingTagResponseTag.fromJson(Map<String, dynamic> json) => ManagingTagResponseTag(
    tagId: json['tag_id'] is num ? json['tag_id'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
    isDefault: json['isDefault'] is bool ? json['isDefault'] as bool : false,
    forOwnedAccountsOnly: json['forOwnedAccountsOnly'] is bool ? json['forOwnedAccountsOnly'] as bool : false,
    bc: json['bc'] is String ? json['bc'] as String : '',
  );
}

class ManagingTagResponse {
  final num itemId;
  final ManagingTagResponseTag tag;
  final num addedTagId;
  final List<num> deleteTags;
  final RespSystemInfo systemInfo;

  const ManagingTagResponse({
    required this.itemId,
    required this.tag,
    required this.addedTagId,
    required this.deleteTags,
    required this.systemInfo,
  });

  factory ManagingTagResponse.fromJson(Map<String, dynamic> json) => ManagingTagResponse(
    itemId: json['itemId'] is num ? json['itemId'] as num : 0,
    tag: json['tag'] is Map<String, dynamic>
            ? ManagingTagResponseTag.fromJson(json['tag'] as Map<String, dynamic>)
            : ManagingTagResponseTag.fromJson(const {}),
    addedTagId: json['addedTagId'] is num ? json['addedTagId'] as num : 0,
    deleteTags: json['deleteTags'] is List ? (json['deleteTags'] as List<dynamic>).cast<num>() : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingUntagBody {
  /// Tag ID.
  final num tagId;

  const ManagingUntagBody({
    required this.tagId,
  });

  Map<String, Object?> toMap() {
    return {
      'tag_id': tagId,
    };
  }
}

class ManagingUntagResponseTag {
  final num tagId;
  final String title;
  final bool isDefault;
  final bool forOwnedAccountsOnly;
  final String bc;

  const ManagingUntagResponseTag({
    required this.tagId,
    required this.title,
    required this.isDefault,
    required this.forOwnedAccountsOnly,
    required this.bc,
  });

  factory ManagingUntagResponseTag.fromJson(Map<String, dynamic> json) => ManagingUntagResponseTag(
    tagId: json['tag_id'] is num ? json['tag_id'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
    isDefault: json['isDefault'] is bool ? json['isDefault'] as bool : false,
    forOwnedAccountsOnly: json['forOwnedAccountsOnly'] is bool ? json['forOwnedAccountsOnly'] as bool : false,
    bc: json['bc'] is String ? json['bc'] as String : '',
  );
}

class ManagingUntagResponse {
  final num itemId;
  final ManagingUntagResponseTag tag;
  final num addedTagId;
  final List<num> deleteTags;
  final RespSystemInfo systemInfo;

  const ManagingUntagResponse({
    required this.itemId,
    required this.tag,
    required this.addedTagId,
    required this.deleteTags,
    required this.systemInfo,
  });

  factory ManagingUntagResponse.fromJson(Map<String, dynamic> json) => ManagingUntagResponse(
    itemId: json['itemId'] is num ? json['itemId'] as num : 0,
    tag: json['tag'] is Map<String, dynamic>
            ? ManagingUntagResponseTag.fromJson(json['tag'] as Map<String, dynamic>)
            : ManagingUntagResponseTag.fromJson(const {}),
    addedTagId: json['addedTagId'] is num ? json['addedTagId'] as num : 0,
    deleteTags: json['deleteTags'] is List ? (json['deleteTags'] as List<dynamic>).cast<num>() : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingPublicTagBody {
  /// Tag ID.
  final num tagId;

  const ManagingPublicTagBody({
    required this.tagId,
  });

  Map<String, Object?> toMap() {
    return {
      'tag_id': tagId,
    };
  }
}

class ManagingPublicTagResponseTag {
  final num tagId;
  final String title;
  final bool isDefault;
  final bool forOwnedAccountsOnly;
  final String bc;

  const ManagingPublicTagResponseTag({
    required this.tagId,
    required this.title,
    required this.isDefault,
    required this.forOwnedAccountsOnly,
    required this.bc,
  });

  factory ManagingPublicTagResponseTag.fromJson(Map<String, dynamic> json) => ManagingPublicTagResponseTag(
    tagId: json['tag_id'] is num ? json['tag_id'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
    isDefault: json['isDefault'] is bool ? json['isDefault'] as bool : false,
    forOwnedAccountsOnly: json['forOwnedAccountsOnly'] is bool ? json['forOwnedAccountsOnly'] as bool : false,
    bc: json['bc'] is String ? json['bc'] as String : '',
  );
}

class ManagingPublicTagResponse {
  final num itemId;
  final ManagingPublicTagResponseTag tag;
  final num addedTagId;
  final List<num> deleteTags;
  final RespSystemInfo systemInfo;

  const ManagingPublicTagResponse({
    required this.itemId,
    required this.tag,
    required this.addedTagId,
    required this.deleteTags,
    required this.systemInfo,
  });

  factory ManagingPublicTagResponse.fromJson(Map<String, dynamic> json) => ManagingPublicTagResponse(
    itemId: json['itemId'] is num ? json['itemId'] as num : 0,
    tag: json['tag'] is Map<String, dynamic>
            ? ManagingPublicTagResponseTag.fromJson(json['tag'] as Map<String, dynamic>)
            : ManagingPublicTagResponseTag.fromJson(const {}),
    addedTagId: json['addedTagId'] is num ? json['addedTagId'] as num : 0,
    deleteTags: json['deleteTags'] is List ? (json['deleteTags'] as List<dynamic>).cast<num>() : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingPublicUntagBody {
  /// Tag ID.
  final num tagId;

  const ManagingPublicUntagBody({
    required this.tagId,
  });

  Map<String, Object?> toMap() {
    return {
      'tag_id': tagId,
    };
  }
}

class ManagingPublicUntagResponseTag {
  final num tagId;
  final String title;
  final bool isDefault;
  final bool forOwnedAccountsOnly;
  final String bc;

  const ManagingPublicUntagResponseTag({
    required this.tagId,
    required this.title,
    required this.isDefault,
    required this.forOwnedAccountsOnly,
    required this.bc,
  });

  factory ManagingPublicUntagResponseTag.fromJson(Map<String, dynamic> json) => ManagingPublicUntagResponseTag(
    tagId: json['tag_id'] is num ? json['tag_id'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
    isDefault: json['isDefault'] is bool ? json['isDefault'] as bool : false,
    forOwnedAccountsOnly: json['forOwnedAccountsOnly'] is bool ? json['forOwnedAccountsOnly'] as bool : false,
    bc: json['bc'] is String ? json['bc'] as String : '',
  );
}

class ManagingPublicUntagResponse {
  final num itemId;
  final ManagingPublicUntagResponseTag tag;
  final num addedTagId;
  final List<num> deleteTags;
  final RespSystemInfo systemInfo;

  const ManagingPublicUntagResponse({
    required this.itemId,
    required this.tag,
    required this.addedTagId,
    required this.deleteTags,
    required this.systemInfo,
  });

  factory ManagingPublicUntagResponse.fromJson(Map<String, dynamic> json) => ManagingPublicUntagResponse(
    itemId: json['itemId'] is num ? json['itemId'] as num : 0,
    tag: json['tag'] is Map<String, dynamic>
            ? ManagingPublicUntagResponseTag.fromJson(json['tag'] as Map<String, dynamic>)
            : ManagingPublicUntagResponseTag.fromJson(const {}),
    addedTagId: json['addedTagId'] is num ? json['addedTagId'] as num : 0,
    deleteTags: json['deleteTags'] is List ? (json['deleteTags'] as List<dynamic>).cast<num>() : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingFavoriteResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const ManagingFavoriteResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory ManagingFavoriteResponse.fromJson(Map<String, dynamic> json) => ManagingFavoriteResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ManagingUnfavoriteResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ManagingUnfavoriteResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ManagingUnfavoriteResponse.fromJson(Map<String, dynamic> json) => ManagingUnfavoriteResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ManagingStickResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ManagingStickResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ManagingStickResponse.fromJson(Map<String, dynamic> json) => ManagingStickResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ManagingUnstickResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ManagingUnstickResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ManagingUnstickResponse.fromJson(Map<String, dynamic> json) => ManagingUnstickResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ManagingTransferBody {
  /// The username of the new account owner.
  final String username;
  /// Secret answer of your account.
  final String secretAnswer;

  const ManagingTransferBody({
    required this.username,
    required this.secretAnswer,
  });

  Map<String, Object?> toMap() {
    return {
      'username': username,
      'secret_answer': secretAnswer,
    };
  }
}

class ManagingTransferResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ManagingTransferResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ManagingTransferResponse.fromJson(Map<String, dynamic> json) => ManagingTransferResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

// ─── PaymentsApi Types ────────────────────────────────────────

class PaymentsInvoiceGetParams {
  /// Invoice ID.
  final num? invoiceId;
  /// Payment ID.
  final String? paymentId;

  const PaymentsInvoiceGetParams({
    this.invoiceId,
    this.paymentId,
  });

  Map<String, Object?> toMap() {
    return {
      'invoice_id': invoiceId,
      'payment_id': paymentId,
    };
  }
}

class PaymentsInvoiceGetResponse {
  final InvoiceModel invoice;
  final RespSystemInfo systemInfo;

  const PaymentsInvoiceGetResponse({
    required this.invoice,
    required this.systemInfo,
  });

  factory PaymentsInvoiceGetResponse.fromJson(Map<String, dynamic> json) => PaymentsInvoiceGetResponse(
    invoice: json['invoice'] is Map<String, dynamic>
            ? InvoiceModel.fromJson(json['invoice'] as Map<String, dynamic>)
            : InvoiceModel.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsInvoiceCreateBody {
  final Currency currency;
  /// Invoice amount.
  final num amount;
  /// Payment ID in your system (must be unique within the merchant / invoices).
  final String paymentId;
  /// Comment to the invoice.
  final String comment;
  /// URL to redirect to after successful payment.
  final String urlSuccess;
  /// Callback url.
  final String? urlCallback;
  /// Merchant ID.
  final num merchantId;
  /// Telegram User ID for which the invoice was created.
  final num? requiredTelegramId;
  /// Telegram Username (including @) for which the invoice was created (if any).
  final String? requiredTelegramUsername;
  /// Invoice lifetime.. Default: 3600.
  final num? lifetime;
  /// Additional information for you.
  final String? additionalData;
  /// Create a test invoice.
  final bool? isTest;

  const PaymentsInvoiceCreateBody({
    required this.currency,
    required this.amount,
    required this.paymentId,
    required this.comment,
    required this.urlSuccess,
    this.urlCallback,
    required this.merchantId,
    this.requiredTelegramId,
    this.requiredTelegramUsername,
    this.lifetime = 3600,
    this.additionalData,
    this.isTest,
  });

  Map<String, Object?> toMap() {
    return {
      'currency': currency.value,
      'amount': amount,
      'payment_id': paymentId,
      'comment': comment,
      'url_success': urlSuccess,
      'url_callback': urlCallback,
      'merchant_id': merchantId,
      'required_telegram_id': requiredTelegramId,
      'required_telegram_username': requiredTelegramUsername,
      'lifetime': lifetime,
      'additional_data': additionalData,
      'is_test': isTest,
    };
  }
}

class PaymentsInvoiceCreateResponse {
  final InvoiceModel invoice;
  final RespSystemInfo systemInfo;

  const PaymentsInvoiceCreateResponse({
    required this.invoice,
    required this.systemInfo,
  });

  factory PaymentsInvoiceCreateResponse.fromJson(Map<String, dynamic> json) => PaymentsInvoiceCreateResponse(
    invoice: json['invoice'] is Map<String, dynamic>
            ? InvoiceModel.fromJson(json['invoice'] as Map<String, dynamic>)
            : InvoiceModel.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsInvoiceListParams {
  /// The number of the page to display results from.
  final num? page;
  final Currency? currency;
  final Status? status;
  /// Invoice amount.
  final num? amount;
  /// Merchant ID.
  final num? merchantId;

  const PaymentsInvoiceListParams({
    this.page,
    this.currency,
    this.status,
    this.amount,
    this.merchantId,
  });

  Map<String, Object?> toMap() {
    return {
      'page': page,
      'currency': currency?.value,
      'status': status?.value,
      'amount': amount,
      'merchant_id': merchantId,
    };
  }
}

class PaymentsInvoiceListResponse {
  final List<InvoiceModel> invoices;
  final num count;
  final num page;
  final num perPage;
  final RespSystemInfo systemInfo;

  const PaymentsInvoiceListResponse({
    required this.invoices,
    required this.count,
    required this.page,
    required this.perPage,
    required this.systemInfo,
  });

  factory PaymentsInvoiceListResponse.fromJson(Map<String, dynamic> json) => PaymentsInvoiceListResponse(
    invoices: json['invoices'] is List
            ? (json['invoices'] as List<dynamic>)
                .map((e) => InvoiceModel.fromJson(e as Map<String, dynamic>))
                .toList()
            : const [],
    count: json['count'] is num ? json['count'] as num : 0,
    page: json['page'] is num ? json['page'] as num : 0,
    perPage: json['perPage'] is num ? json['perPage'] as num : 0,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsCurrencyResponseCurrencyListBTC {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListBTC({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListBTC.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListBTC(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListETH {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListETH({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListETH.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListETH(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListBNB {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListBNB({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListBNB.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListBNB(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListBCH {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListBCH({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListBCH.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListBCH(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListXMR {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListXMR({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListXMR.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListXMR(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListSOL {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListSOL({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListSOL.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListSOL(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListLTC {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListLTC({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListLTC.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListLTC(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListDASH {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListDASH({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListDASH.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListDASH(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListTON {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListTON({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListTON.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListTON(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListUSDT {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListUSDT({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListUSDT.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListUSDT(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListMATIC {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListMATIC({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListMATIC.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListMATIC(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListTRX {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListTRX({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListTRX.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListTRX(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListDOGE {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListDOGE({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListDOGE.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListDOGE(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListKWD {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListKWD({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListKWD.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListKWD(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListGBP {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListGBP({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListGBP.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListGBP(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListCHF {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListCHF({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListCHF.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListCHF(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListEUR {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListEUR({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListEUR.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListEUR(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListUSD {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListUSD({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListUSD.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListUSD(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListSGD {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListSGD({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListSGD.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListSGD(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListCAD {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListCAD({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListCAD.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListCAD(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListAUD {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListAUD({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListAUD.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListAUD(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListNZD {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListNZD({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListNZD.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListNZD(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListBGN {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListBGN({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListBGN.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListBGN(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListGEL {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListGEL({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListGEL.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListGEL(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListILS {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListILS({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListILS.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListILS(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListQAR {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListQAR({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListQAR.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListQAR(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListPEN {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListPEN({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListPEN.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListPEN(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListAED {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListAED({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListAED.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListAED(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListSAR {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListSAR({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListSAR.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListSAR(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListPLN {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListPLN({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListPLN.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListPLN(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListMYR {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListMYR({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListMYR.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListMYR(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListRON {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListRON({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListRON.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListRON(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListBRL {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListBRL({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListBRL.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListBRL(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListDKK {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListDKK({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListDKK.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListDKK(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListCNY {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListCNY({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListCNY.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListCNY(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListHKD {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListHKD({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListHKD.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListHKD(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListSEK {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListSEK({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListSEK.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListSEK(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListNOK {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListNOK({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListNOK.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListNOK(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListZAR {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListZAR({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListZAR.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListZAR(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListMXN {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListMXN({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListMXN.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListMXN(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListCZK {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListCZK({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListCZK.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListCZK(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListTWD {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListTWD({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListTWD.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListTWD(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListTHB {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListTHB({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListTHB.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListTHB(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListTRY {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListTRY({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListTRY.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListTRY(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListUAH {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListUAH({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListUAH.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListUAH(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListUYU {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListUYU({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListUYU.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListUYU(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListPHP {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListPHP({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListPHP.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListPHP(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListINR {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListINR({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListINR.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListINR(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListRUB {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListRUB({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListRUB.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListRUB(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListRSD {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListRSD({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListRSD.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListRSD(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListJPY {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListJPY({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListJPY.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListJPY(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListHUF {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListHUF({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListHUF.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListHUF(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListKZT {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListKZT({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListKZT.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListKZT(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListCRC {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListCRC({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListCRC.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListCRC(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListCLP {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListCLP({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListCLP.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListCLP(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListARS {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListARS({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListARS.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListARS(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListKRW {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListKRW({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListKRW.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListKRW(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListCOP {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListCOP({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListCOP.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListCOP(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListIDR {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListIDR({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListIDR.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListIDR(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyListVND {
  final String title;
  final num rate;
  final String formattedRate;
  final String symbol;

  const PaymentsCurrencyResponseCurrencyListVND({
    required this.title,
    required this.rate,
    required this.formattedRate,
    required this.symbol,
  });

  factory PaymentsCurrencyResponseCurrencyListVND.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyListVND(
    title: json['title'] is String ? json['title'] as String : '',
    rate: json['rate'] is num ? json['rate'] as num : 0,
    formattedRate: json['formattedRate'] is String ? json['formattedRate'] as String : '',
    symbol: json['symbol'] is String ? json['symbol'] as String : '',
  );
}

class PaymentsCurrencyResponseCurrencyList {
  final PaymentsCurrencyResponseCurrencyListBTC bTC;
  final PaymentsCurrencyResponseCurrencyListETH eTH;
  final PaymentsCurrencyResponseCurrencyListBNB bNB;
  final PaymentsCurrencyResponseCurrencyListBCH bCH;
  final PaymentsCurrencyResponseCurrencyListXMR xMR;
  final PaymentsCurrencyResponseCurrencyListSOL sOL;
  final PaymentsCurrencyResponseCurrencyListLTC lTC;
  final PaymentsCurrencyResponseCurrencyListDASH dASH;
  final PaymentsCurrencyResponseCurrencyListTON tON;
  final PaymentsCurrencyResponseCurrencyListUSDT uSDT;
  final PaymentsCurrencyResponseCurrencyListMATIC mATIC;
  final PaymentsCurrencyResponseCurrencyListTRX tRX;
  final PaymentsCurrencyResponseCurrencyListDOGE dOGE;
  final PaymentsCurrencyResponseCurrencyListKWD kWD;
  final PaymentsCurrencyResponseCurrencyListGBP gBP;
  final PaymentsCurrencyResponseCurrencyListCHF cHF;
  final PaymentsCurrencyResponseCurrencyListEUR eUR;
  final PaymentsCurrencyResponseCurrencyListUSD uSD;
  final PaymentsCurrencyResponseCurrencyListSGD sGD;
  final PaymentsCurrencyResponseCurrencyListCAD cAD;
  final PaymentsCurrencyResponseCurrencyListAUD aUD;
  final PaymentsCurrencyResponseCurrencyListNZD nZD;
  final PaymentsCurrencyResponseCurrencyListBGN bGN;
  final PaymentsCurrencyResponseCurrencyListGEL gEL;
  final PaymentsCurrencyResponseCurrencyListILS iLS;
  final PaymentsCurrencyResponseCurrencyListQAR qAR;
  final PaymentsCurrencyResponseCurrencyListPEN pEN;
  final PaymentsCurrencyResponseCurrencyListAED aED;
  final PaymentsCurrencyResponseCurrencyListSAR sAR;
  final PaymentsCurrencyResponseCurrencyListPLN pLN;
  final PaymentsCurrencyResponseCurrencyListMYR mYR;
  final PaymentsCurrencyResponseCurrencyListRON rON;
  final PaymentsCurrencyResponseCurrencyListBRL bRL;
  final PaymentsCurrencyResponseCurrencyListDKK dKK;
  final PaymentsCurrencyResponseCurrencyListCNY cNY;
  final PaymentsCurrencyResponseCurrencyListHKD hKD;
  final PaymentsCurrencyResponseCurrencyListSEK sEK;
  final PaymentsCurrencyResponseCurrencyListNOK nOK;
  final PaymentsCurrencyResponseCurrencyListZAR zAR;
  final PaymentsCurrencyResponseCurrencyListMXN mXN;
  final PaymentsCurrencyResponseCurrencyListCZK cZK;
  final PaymentsCurrencyResponseCurrencyListTWD tWD;
  final PaymentsCurrencyResponseCurrencyListTHB tHB;
  final PaymentsCurrencyResponseCurrencyListTRY tRY;
  final PaymentsCurrencyResponseCurrencyListUAH uAH;
  final PaymentsCurrencyResponseCurrencyListUYU uYU;
  final PaymentsCurrencyResponseCurrencyListPHP pHP;
  final PaymentsCurrencyResponseCurrencyListINR iNR;
  final PaymentsCurrencyResponseCurrencyListRUB rUB;
  final PaymentsCurrencyResponseCurrencyListRSD rSD;
  final PaymentsCurrencyResponseCurrencyListJPY jPY;
  final PaymentsCurrencyResponseCurrencyListHUF hUF;
  final PaymentsCurrencyResponseCurrencyListKZT kZT;
  final PaymentsCurrencyResponseCurrencyListCRC cRC;
  final PaymentsCurrencyResponseCurrencyListCLP cLP;
  final PaymentsCurrencyResponseCurrencyListARS aRS;
  final PaymentsCurrencyResponseCurrencyListKRW kRW;
  final PaymentsCurrencyResponseCurrencyListCOP cOP;
  final PaymentsCurrencyResponseCurrencyListIDR iDR;
  final PaymentsCurrencyResponseCurrencyListVND vND;

  const PaymentsCurrencyResponseCurrencyList({
    required this.bTC,
    required this.eTH,
    required this.bNB,
    required this.bCH,
    required this.xMR,
    required this.sOL,
    required this.lTC,
    required this.dASH,
    required this.tON,
    required this.uSDT,
    required this.mATIC,
    required this.tRX,
    required this.dOGE,
    required this.kWD,
    required this.gBP,
    required this.cHF,
    required this.eUR,
    required this.uSD,
    required this.sGD,
    required this.cAD,
    required this.aUD,
    required this.nZD,
    required this.bGN,
    required this.gEL,
    required this.iLS,
    required this.qAR,
    required this.pEN,
    required this.aED,
    required this.sAR,
    required this.pLN,
    required this.mYR,
    required this.rON,
    required this.bRL,
    required this.dKK,
    required this.cNY,
    required this.hKD,
    required this.sEK,
    required this.nOK,
    required this.zAR,
    required this.mXN,
    required this.cZK,
    required this.tWD,
    required this.tHB,
    required this.tRY,
    required this.uAH,
    required this.uYU,
    required this.pHP,
    required this.iNR,
    required this.rUB,
    required this.rSD,
    required this.jPY,
    required this.hUF,
    required this.kZT,
    required this.cRC,
    required this.cLP,
    required this.aRS,
    required this.kRW,
    required this.cOP,
    required this.iDR,
    required this.vND,
  });

  factory PaymentsCurrencyResponseCurrencyList.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponseCurrencyList(
    bTC: json['BTC'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListBTC.fromJson(json['BTC'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListBTC.fromJson(const {}),
    eTH: json['ETH'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListETH.fromJson(json['ETH'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListETH.fromJson(const {}),
    bNB: json['BNB'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListBNB.fromJson(json['BNB'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListBNB.fromJson(const {}),
    bCH: json['BCH'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListBCH.fromJson(json['BCH'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListBCH.fromJson(const {}),
    xMR: json['XMR'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListXMR.fromJson(json['XMR'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListXMR.fromJson(const {}),
    sOL: json['SOL'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListSOL.fromJson(json['SOL'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListSOL.fromJson(const {}),
    lTC: json['LTC'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListLTC.fromJson(json['LTC'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListLTC.fromJson(const {}),
    dASH: json['DASH'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListDASH.fromJson(json['DASH'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListDASH.fromJson(const {}),
    tON: json['TON'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListTON.fromJson(json['TON'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListTON.fromJson(const {}),
    uSDT: json['USDT'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListUSDT.fromJson(json['USDT'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListUSDT.fromJson(const {}),
    mATIC: json['MATIC'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListMATIC.fromJson(json['MATIC'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListMATIC.fromJson(const {}),
    tRX: json['TRX'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListTRX.fromJson(json['TRX'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListTRX.fromJson(const {}),
    dOGE: json['DOGE'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListDOGE.fromJson(json['DOGE'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListDOGE.fromJson(const {}),
    kWD: json['KWD'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListKWD.fromJson(json['KWD'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListKWD.fromJson(const {}),
    gBP: json['GBP'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListGBP.fromJson(json['GBP'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListGBP.fromJson(const {}),
    cHF: json['CHF'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListCHF.fromJson(json['CHF'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListCHF.fromJson(const {}),
    eUR: json['EUR'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListEUR.fromJson(json['EUR'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListEUR.fromJson(const {}),
    uSD: json['USD'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListUSD.fromJson(json['USD'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListUSD.fromJson(const {}),
    sGD: json['SGD'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListSGD.fromJson(json['SGD'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListSGD.fromJson(const {}),
    cAD: json['CAD'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListCAD.fromJson(json['CAD'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListCAD.fromJson(const {}),
    aUD: json['AUD'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListAUD.fromJson(json['AUD'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListAUD.fromJson(const {}),
    nZD: json['NZD'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListNZD.fromJson(json['NZD'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListNZD.fromJson(const {}),
    bGN: json['BGN'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListBGN.fromJson(json['BGN'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListBGN.fromJson(const {}),
    gEL: json['GEL'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListGEL.fromJson(json['GEL'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListGEL.fromJson(const {}),
    iLS: json['ILS'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListILS.fromJson(json['ILS'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListILS.fromJson(const {}),
    qAR: json['QAR'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListQAR.fromJson(json['QAR'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListQAR.fromJson(const {}),
    pEN: json['PEN'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListPEN.fromJson(json['PEN'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListPEN.fromJson(const {}),
    aED: json['AED'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListAED.fromJson(json['AED'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListAED.fromJson(const {}),
    sAR: json['SAR'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListSAR.fromJson(json['SAR'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListSAR.fromJson(const {}),
    pLN: json['PLN'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListPLN.fromJson(json['PLN'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListPLN.fromJson(const {}),
    mYR: json['MYR'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListMYR.fromJson(json['MYR'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListMYR.fromJson(const {}),
    rON: json['RON'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListRON.fromJson(json['RON'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListRON.fromJson(const {}),
    bRL: json['BRL'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListBRL.fromJson(json['BRL'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListBRL.fromJson(const {}),
    dKK: json['DKK'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListDKK.fromJson(json['DKK'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListDKK.fromJson(const {}),
    cNY: json['CNY'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListCNY.fromJson(json['CNY'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListCNY.fromJson(const {}),
    hKD: json['HKD'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListHKD.fromJson(json['HKD'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListHKD.fromJson(const {}),
    sEK: json['SEK'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListSEK.fromJson(json['SEK'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListSEK.fromJson(const {}),
    nOK: json['NOK'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListNOK.fromJson(json['NOK'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListNOK.fromJson(const {}),
    zAR: json['ZAR'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListZAR.fromJson(json['ZAR'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListZAR.fromJson(const {}),
    mXN: json['MXN'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListMXN.fromJson(json['MXN'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListMXN.fromJson(const {}),
    cZK: json['CZK'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListCZK.fromJson(json['CZK'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListCZK.fromJson(const {}),
    tWD: json['TWD'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListTWD.fromJson(json['TWD'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListTWD.fromJson(const {}),
    tHB: json['THB'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListTHB.fromJson(json['THB'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListTHB.fromJson(const {}),
    tRY: json['TRY'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListTRY.fromJson(json['TRY'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListTRY.fromJson(const {}),
    uAH: json['UAH'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListUAH.fromJson(json['UAH'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListUAH.fromJson(const {}),
    uYU: json['UYU'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListUYU.fromJson(json['UYU'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListUYU.fromJson(const {}),
    pHP: json['PHP'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListPHP.fromJson(json['PHP'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListPHP.fromJson(const {}),
    iNR: json['INR'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListINR.fromJson(json['INR'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListINR.fromJson(const {}),
    rUB: json['RUB'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListRUB.fromJson(json['RUB'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListRUB.fromJson(const {}),
    rSD: json['RSD'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListRSD.fromJson(json['RSD'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListRSD.fromJson(const {}),
    jPY: json['JPY'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListJPY.fromJson(json['JPY'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListJPY.fromJson(const {}),
    hUF: json['HUF'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListHUF.fromJson(json['HUF'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListHUF.fromJson(const {}),
    kZT: json['KZT'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListKZT.fromJson(json['KZT'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListKZT.fromJson(const {}),
    cRC: json['CRC'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListCRC.fromJson(json['CRC'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListCRC.fromJson(const {}),
    cLP: json['CLP'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListCLP.fromJson(json['CLP'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListCLP.fromJson(const {}),
    aRS: json['ARS'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListARS.fromJson(json['ARS'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListARS.fromJson(const {}),
    kRW: json['KRW'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListKRW.fromJson(json['KRW'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListKRW.fromJson(const {}),
    cOP: json['COP'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListCOP.fromJson(json['COP'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListCOP.fromJson(const {}),
    iDR: json['IDR'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListIDR.fromJson(json['IDR'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListIDR.fromJson(const {}),
    vND: json['VND'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyListVND.fromJson(json['VND'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyListVND.fromJson(const {}),
  );
}

class PaymentsCurrencyResponse {
  final PaymentsCurrencyResponseCurrencyList currencyList;
  final num lastUpdate;
  final String visitorCurrency;
  final RespSystemInfo systemInfo;

  const PaymentsCurrencyResponse({
    required this.currencyList,
    required this.lastUpdate,
    required this.visitorCurrency,
    required this.systemInfo,
  });

  factory PaymentsCurrencyResponse.fromJson(Map<String, dynamic> json) => PaymentsCurrencyResponse(
    currencyList: json['currencyList'] is Map<String, dynamic>
            ? PaymentsCurrencyResponseCurrencyList.fromJson(json['currencyList'] as Map<String, dynamic>)
            : PaymentsCurrencyResponseCurrencyList.fromJson(const {}),
    lastUpdate: json['lastUpdate'] is num ? json['lastUpdate'] as num : 0,
    visitorCurrency: json['visitorCurrency'] is String ? json['visitorCurrency'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsBalanceListResponseFromBalance {
  final String balance;
  final num convertedBalance;
  final String fullTitle;
  final String title;
  final String type;

  const PaymentsBalanceListResponseFromBalance({
    required this.balance,
    required this.convertedBalance,
    required this.fullTitle,
    required this.title,
    required this.type,
  });

  factory PaymentsBalanceListResponseFromBalance.fromJson(Map<String, dynamic> json) => PaymentsBalanceListResponseFromBalance(
    balance: json['balance'] is String ? json['balance'] as String : '',
    convertedBalance: json['convertedBalance'] is num ? json['convertedBalance'] as num : 0,
    fullTitle: json['fullTitle'] is String ? json['fullTitle'] as String : '',
    title: json['title'] is String ? json['title'] as String : '',
    type: json['type'] is String ? json['type'] as String : '',
  );
}

class PaymentsBalanceListResponseFrom {
  final PaymentsBalanceListResponseFromBalance balance;
  final BalanceModel n12345;

  const PaymentsBalanceListResponseFrom({
    required this.balance,
    required this.n12345,
  });

  factory PaymentsBalanceListResponseFrom.fromJson(Map<String, dynamic> json) => PaymentsBalanceListResponseFrom(
    balance: json['balance'] is Map<String, dynamic>
            ? PaymentsBalanceListResponseFromBalance.fromJson(json['balance'] as Map<String, dynamic>)
            : PaymentsBalanceListResponseFromBalance.fromJson(const {}),
    n12345: json['12345'] is Map<String, dynamic>
            ? BalanceModel.fromJson(json['12345'] as Map<String, dynamic>)
            : BalanceModel.fromJson(const {}),
  );
}

class PaymentsBalanceListResponseTo {
  final UserModel balance;

  const PaymentsBalanceListResponseTo({
    required this.balance,
  });

  factory PaymentsBalanceListResponseTo.fromJson(Map<String, dynamic> json) => PaymentsBalanceListResponseTo(
    balance: json['balance'] is Map<String, dynamic>
            ? UserModel.fromJson(json['balance'] as Map<String, dynamic>)
            : UserModel.fromJson(const {}),
  );
}

class PaymentsBalanceListResponse {
  final PaymentsBalanceListResponseFrom from;
  final PaymentsBalanceListResponseTo to;
  final RespSystemInfo systemInfo;

  const PaymentsBalanceListResponse({
    required this.from,
    required this.to,
    required this.systemInfo,
  });

  factory PaymentsBalanceListResponse.fromJson(Map<String, dynamic> json) => PaymentsBalanceListResponse(
    from: json['from'] is Map<String, dynamic>
            ? PaymentsBalanceListResponseFrom.fromJson(json['from'] as Map<String, dynamic>)
            : PaymentsBalanceListResponseFrom.fromJson(const {}),
    to: json['to'] is Map<String, dynamic>
            ? PaymentsBalanceListResponseTo.fromJson(json['to'] as Map<String, dynamic>)
            : PaymentsBalanceListResponseTo.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsBalanceExchangeBody {
  /// Source balance type.
  final String fromBalance;
  /// Target balance type.
  final String toBalance;
  /// Amount to exchange.
  final num amount;

  const PaymentsBalanceExchangeBody({
    required this.fromBalance,
    required this.toBalance,
    required this.amount,
  });

  Map<String, Object?> toMap() {
    return {
      'from_balance': fromBalance,
      'to_balance': toBalance,
      'amount': amount,
    };
  }
}

class PaymentsBalanceExchangeResponseFromBalance {
  final String balance;
  final num convertedBalance;
  final String fullTitle;
  final String title;
  final String type;

  const PaymentsBalanceExchangeResponseFromBalance({
    required this.balance,
    required this.convertedBalance,
    required this.fullTitle,
    required this.title,
    required this.type,
  });

  factory PaymentsBalanceExchangeResponseFromBalance.fromJson(Map<String, dynamic> json) => PaymentsBalanceExchangeResponseFromBalance(
    balance: json['balance'] is String ? json['balance'] as String : '',
    convertedBalance: json['convertedBalance'] is num ? json['convertedBalance'] as num : 0,
    fullTitle: json['fullTitle'] is String ? json['fullTitle'] as String : '',
    title: json['title'] is String ? json['title'] as String : '',
    type: json['type'] is String ? json['type'] as String : '',
  );
}

class PaymentsBalanceExchangeResponseFrom {
  final PaymentsBalanceExchangeResponseFromBalance balance;
  final BalanceModel n12345;

  const PaymentsBalanceExchangeResponseFrom({
    required this.balance,
    required this.n12345,
  });

  factory PaymentsBalanceExchangeResponseFrom.fromJson(Map<String, dynamic> json) => PaymentsBalanceExchangeResponseFrom(
    balance: json['balance'] is Map<String, dynamic>
            ? PaymentsBalanceExchangeResponseFromBalance.fromJson(json['balance'] as Map<String, dynamic>)
            : PaymentsBalanceExchangeResponseFromBalance.fromJson(const {}),
    n12345: json['12345'] is Map<String, dynamic>
            ? BalanceModel.fromJson(json['12345'] as Map<String, dynamic>)
            : BalanceModel.fromJson(const {}),
  );
}

class PaymentsBalanceExchangeResponseTo {
  final UserModel balance;

  const PaymentsBalanceExchangeResponseTo({
    required this.balance,
  });

  factory PaymentsBalanceExchangeResponseTo.fromJson(Map<String, dynamic> json) => PaymentsBalanceExchangeResponseTo(
    balance: json['balance'] is Map<String, dynamic>
            ? UserModel.fromJson(json['balance'] as Map<String, dynamic>)
            : UserModel.fromJson(const {}),
  );
}

class PaymentsBalanceExchangeResponse {
  final PaymentsBalanceExchangeResponseFrom from;
  final PaymentsBalanceExchangeResponseTo to;
  final RespSystemInfo systemInfo;

  const PaymentsBalanceExchangeResponse({
    required this.from,
    required this.to,
    required this.systemInfo,
  });

  factory PaymentsBalanceExchangeResponse.fromJson(Map<String, dynamic> json) => PaymentsBalanceExchangeResponse(
    from: json['from'] is Map<String, dynamic>
            ? PaymentsBalanceExchangeResponseFrom.fromJson(json['from'] as Map<String, dynamic>)
            : PaymentsBalanceExchangeResponseFrom.fromJson(const {}),
    to: json['to'] is Map<String, dynamic>
            ? PaymentsBalanceExchangeResponseTo.fromJson(json['to'] as Map<String, dynamic>)
            : PaymentsBalanceExchangeResponseTo.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsTransferBody {
  /// User id of receiver. If $1 specified, $1 is not required.
  final num? userId;
  /// Username of receiver. If $1 specified, $1 is not required.
  final String? username;
  /// Amount to send in your currency.
  final num amount;
  final Currency currency;
  /// Transfer comment.
  final String? comment;
  /// Is the deal happening on Telegram?.
  final bool? telegramDeal;
  /// Telegram username of the user you are dialoguing with.
  final String? telegramUsername;
  /// Hold transfer or not.
  final bool? transferHold;
  /// Hold length value.
  final num? holdLengthValue;
  final HoldLengthOption? holdLengthOption;

  const PaymentsTransferBody({
    this.userId,
    this.username,
    required this.amount,
    required this.currency,
    this.comment,
    this.telegramDeal,
    this.telegramUsername,
    this.transferHold,
    this.holdLengthValue,
    this.holdLengthOption,
  });

  Map<String, Object?> toMap() {
    return {
      'user_id': userId,
      'username': username,
      'amount': amount,
      'currency': currency.value,
      'comment': comment,
      'telegram_deal': telegramDeal,
      'telegram_username': telegramUsername,
      'transfer_hold': transferHold,
      'hold_length_value': holdLengthValue,
      'hold_length_option': holdLengthOption?.value,
    };
  }
}

class PaymentsTransferResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const PaymentsTransferResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory PaymentsTransferResponse.fromJson(Map<String, dynamic> json) => PaymentsTransferResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsFeeParams {
  /// Amount you want to send in your currency.
  final num? amount;

  const PaymentsFeeParams({
    this.amount,
  });

  Map<String, Object?> toMap() {
    return {
      'amount': amount,
    };
  }
}

class PaymentsFeeResponseCalculator {
  final num inputAmount;
  final num commissionAmount;
  final num totalOutputAmount;

  const PaymentsFeeResponseCalculator({
    required this.inputAmount,
    required this.commissionAmount,
    required this.totalOutputAmount,
  });

  factory PaymentsFeeResponseCalculator.fromJson(Map<String, dynamic> json) => PaymentsFeeResponseCalculator(
    inputAmount: json['inputAmount'] is num ? json['inputAmount'] as num : 0,
    commissionAmount: json['commissionAmount'] is num ? json['commissionAmount'] as num : 0,
    totalOutputAmount: json['totalOutputAmount'] is num ? json['totalOutputAmount'] as num : 0,
  );
}

class PaymentsFeeResponse {
  final num commissionPercentage;
  final num spentCurrentMonth;
  final PaymentsFeeResponseCalculator calculator;
  final RespSystemInfo systemInfo;

  const PaymentsFeeResponse({
    required this.commissionPercentage,
    required this.spentCurrentMonth,
    required this.calculator,
    required this.systemInfo,
  });

  factory PaymentsFeeResponse.fromJson(Map<String, dynamic> json) => PaymentsFeeResponse(
    commissionPercentage: json['commission_percentage'] is num ? json['commission_percentage'] as num : 0,
    spentCurrentMonth: json['spentCurrentMonth'] is num ? json['spentCurrentMonth'] as num : 0,
    calculator: json['calculator'] is Map<String, dynamic>
            ? PaymentsFeeResponseCalculator.fromJson(json['calculator'] as Map<String, dynamic>)
            : PaymentsFeeResponseCalculator.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsCancelBody {
  /// Payment id.
  final num paymentId;

  const PaymentsCancelBody({
    required this.paymentId,
  });

  Map<String, Object?> toMap() {
    return {
      'payment_id': paymentId,
    };
  }
}

class PaymentsCancelResponse {
  final String status;
  final String message;
  final RespSystemInfo systemInfo;

  const PaymentsCancelResponse({
    required this.status,
    required this.message,
    required this.systemInfo,
  });

  factory PaymentsCancelResponse.fromJson(Map<String, dynamic> json) => PaymentsCancelResponse(
    status: json['status'] is String ? json['status'] as String : '',
    message: json['message'] is String ? json['message'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsHistoryParams {
  final Type? type;
  /// Minimal price of account (Inclusive).
  final num? pmin;
  /// Maximum price of account (Inclusive).
  final num? pmax;
  final Currency? currency;
  /// The number of the page to display results from.
  final num? page;
  /// Id of the operation from which the result begins.
  final num? operationIdLt;
  /// Username of user, which receive money from you.
  final String? receiver;
  /// Username of user, which sent money to you.
  final String? sender;
  /// Returns payments that are done via API.
  final bool? isApi;
  /// Start date of operation (RFC 3339 date format).
  final String? startDate;
  /// End date of operation (RFC 3339 date format).
  final String? endDate;
  /// Wallet, which used for money payouts.
  final String? wallet;
  /// Comment for money transfers.
  final String? comment;
  /// Display hold operations.
  final bool? isHold;
  /// Display payment stats for selected period (outgoing value, incoming value).
  final bool? showPaymentStats;

  const PaymentsHistoryParams({
    this.type,
    this.pmin,
    this.pmax,
    this.currency,
    this.page,
    this.operationIdLt,
    this.receiver,
    this.sender,
    this.isApi,
    this.startDate,
    this.endDate,
    this.wallet,
    this.comment,
    this.isHold,
    this.showPaymentStats,
  });

  Map<String, Object?> toMap() {
    return {
      'type': type?.value,
      'pmin': pmin,
      'pmax': pmax,
      'currency': currency?.value,
      'page': page,
      'operation_id_lt': operationIdLt,
      'receiver': receiver,
      'sender': sender,
      'is_api': isApi,
      'startDate': startDate,
      'endDate': endDate,
      'wallet': wallet,
      'comment': comment,
      'is_hold': isHold,
      'show_payment_stats': showPaymentStats,
    };
  }
}

class PaymentsHistoryResponsePageNavParams {
  final String type;
  final String startDate;
  final String endDate;

  const PaymentsHistoryResponsePageNavParams({
    required this.type,
    required this.startDate,
    required this.endDate,
  });

  factory PaymentsHistoryResponsePageNavParams.fromJson(Map<String, dynamic> json) => PaymentsHistoryResponsePageNavParams(
    type: json['type'] is String ? json['type'] as String : '',
    startDate: json['startDate'] is String ? json['startDate'] as String : '',
    endDate: json['endDate'] is String ? json['endDate'] as String : '',
  );
}

class PaymentsHistoryResponseInput {
  final num userId;
  final String type;
  final String startDate;
  final String endDate;
  final num page;
  final String periodLabel;
  final String receiver;
  final String sender;
  final String comment;
  final String pmin;
  final String pmax;
  final num categoryId;
  final String wallet;
  final bool isHold;
  final String currency;
  final num operationIdLt;

  const PaymentsHistoryResponseInput({
    required this.userId,
    required this.type,
    required this.startDate,
    required this.endDate,
    required this.page,
    required this.periodLabel,
    required this.receiver,
    required this.sender,
    required this.comment,
    required this.pmin,
    required this.pmax,
    required this.categoryId,
    required this.wallet,
    required this.isHold,
    required this.currency,
    required this.operationIdLt,
  });

  factory PaymentsHistoryResponseInput.fromJson(Map<String, dynamic> json) => PaymentsHistoryResponseInput(
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
    type: json['type'] is String ? json['type'] as String : '',
    startDate: json['startDate'] is String ? json['startDate'] as String : '',
    endDate: json['endDate'] is String ? json['endDate'] as String : '',
    page: json['page'] is num ? json['page'] as num : 0,
    periodLabel: json['period_label'] is String ? json['period_label'] as String : '',
    receiver: json['receiver'] is String ? json['receiver'] as String : '',
    sender: json['sender'] is String ? json['sender'] as String : '',
    comment: json['comment'] is String ? json['comment'] as String : '',
    pmin: json['pmin'] is String ? json['pmin'] as String : '',
    pmax: json['pmax'] is String ? json['pmax'] as String : '',
    categoryId: json['category_id'] is num ? json['category_id'] as num : 0,
    wallet: json['wallet'] is String ? json['wallet'] as String : '',
    isHold: json['is_hold'] is bool ? json['is_hold'] as bool : false,
    currency: json['currency'] is String ? json['currency'] as String : '',
    operationIdLt: json['operation_id_lt'] is num ? json['operation_id_lt'] as num : 0,
  );
}

class PaymentsHistoryResponse {
  final dynamic payments;
  final String perPage;
  final num page;
  final String pageNavLink;
  final PaymentsHistoryResponsePageNavParams pageNavParams;
  final String periodLabel;
  final String periodLabelPhrase;
  final bool filterDatesDefault;
  final PaymentsHistoryResponseInput input;
  final dynamic paymentStats;
  final bool hasNextPage;
  final num lastOperationId;
  final String nextPageHref;
  final RespSystemInfo systemInfo;

  const PaymentsHistoryResponse({
    required this.payments,
    required this.perPage,
    required this.page,
    required this.pageNavLink,
    required this.pageNavParams,
    required this.periodLabel,
    required this.periodLabelPhrase,
    required this.filterDatesDefault,
    required this.input,
    required this.paymentStats,
    required this.hasNextPage,
    required this.lastOperationId,
    required this.nextPageHref,
    required this.systemInfo,
  });

  factory PaymentsHistoryResponse.fromJson(Map<String, dynamic> json) => PaymentsHistoryResponse(
    payments: json['payments'],
    perPage: json['perPage'] is String ? json['perPage'] as String : '',
    page: json['page'] is num ? json['page'] as num : 0,
    pageNavLink: json['pageNavLink'] is String ? json['pageNavLink'] as String : '',
    pageNavParams: json['pageNavParams'] is Map<String, dynamic>
            ? PaymentsHistoryResponsePageNavParams.fromJson(json['pageNavParams'] as Map<String, dynamic>)
            : PaymentsHistoryResponsePageNavParams.fromJson(const {}),
    periodLabel: json['periodLabel'] is String ? json['periodLabel'] as String : '',
    periodLabelPhrase: json['periodLabelPhrase'] is String ? json['periodLabelPhrase'] as String : '',
    filterDatesDefault: json['filterDatesDefault'] is bool ? json['filterDatesDefault'] as bool : false,
    input: json['input'] is Map<String, dynamic>
            ? PaymentsHistoryResponseInput.fromJson(json['input'] as Map<String, dynamic>)
            : PaymentsHistoryResponseInput.fromJson(const {}),
    paymentStats: json['paymentStats'],
    hasNextPage: json['hasNextPage'] is bool ? json['hasNextPage'] as bool : false,
    lastOperationId: json['lastOperationId'] is num ? json['lastOperationId'] as num : 0,
    nextPageHref: json['nextPageHref'] is String ? json['nextPageHref'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsPayoutServicesResponse {
  final List<Map<String, dynamic>> systems;
  final RespSystemInfo systemInfo;

  const PaymentsPayoutServicesResponse({
    required this.systems,
    required this.systemInfo,
  });

  factory PaymentsPayoutServicesResponse.fromJson(Map<String, dynamic> json) => PaymentsPayoutServicesResponse(
    systems: json['systems'] is List ? (json['systems'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PaymentsPayoutBody {
  final String paymentSystem;
  final String wallet;
  final num amount;
  final Currency currency;
  final bool? includeFee;
  final Map<String, dynamic>? extra;

  const PaymentsPayoutBody({
    required this.paymentSystem,
    required this.wallet,
    required this.amount,
    required this.currency,
    this.includeFee,
    this.extra,
  });

  Map<String, Object?> toMap() {
    return {
      'payment_system': paymentSystem,
      'wallet': wallet,
      'amount': amount,
      'currency': currency.value,
      'include_fee': includeFee,
      'extra': extra,
    };
  }
}

class PaymentsPayoutResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const PaymentsPayoutResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory PaymentsPayoutResponse.fromJson(Map<String, dynamic> json) => PaymentsPayoutResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

// ─── ProfileApi Types ────────────────────────────────────────

class ProfileClaimsParams {
  final Type? type;
  final ClaimState? claimState;

  const ProfileClaimsParams({
    this.type,
    this.claimState,
  });

  Map<String, Object?> toMap() {
    return {
      'type': type?.value,
      'claim_state': claimState?.value,
    };
  }
}

class ProfileClaimsResponseStatsMarket {
  final num total;
  final num solved;
  final num settled;
  final num rejected;

  const ProfileClaimsResponseStatsMarket({
    required this.total,
    required this.solved,
    required this.settled,
    required this.rejected,
  });

  factory ProfileClaimsResponseStatsMarket.fromJson(Map<String, dynamic> json) => ProfileClaimsResponseStatsMarket(
    total: json['total'] is num ? json['total'] as num : 0,
    solved: json['solved'] is num ? json['solved'] as num : 0,
    settled: json['settled'] is num ? json['settled'] as num : 0,
    rejected: json['rejected'] is num ? json['rejected'] as num : 0,
  );
}

class ProfileClaimsResponseStatsNoMarket {
  final num total;
  final num solved;
  final num settled;
  final num rejected;

  const ProfileClaimsResponseStatsNoMarket({
    required this.total,
    required this.solved,
    required this.settled,
    required this.rejected,
  });

  factory ProfileClaimsResponseStatsNoMarket.fromJson(Map<String, dynamic> json) => ProfileClaimsResponseStatsNoMarket(
    total: json['total'] is num ? json['total'] as num : 0,
    solved: json['solved'] is num ? json['solved'] as num : 0,
    settled: json['settled'] is num ? json['settled'] as num : 0,
    rejected: json['rejected'] is num ? json['rejected'] as num : 0,
  );
}

class ProfileClaimsResponseStats {
  final ProfileClaimsResponseStatsMarket market;
  final ProfileClaimsResponseStatsNoMarket noMarket;

  const ProfileClaimsResponseStats({
    required this.market,
    required this.noMarket,
  });

  factory ProfileClaimsResponseStats.fromJson(Map<String, dynamic> json) => ProfileClaimsResponseStats(
    market: json['market'] is Map<String, dynamic>
            ? ProfileClaimsResponseStatsMarket.fromJson(json['market'] as Map<String, dynamic>)
            : ProfileClaimsResponseStatsMarket.fromJson(const {}),
    noMarket: json['noMarket'] is Map<String, dynamic>
            ? ProfileClaimsResponseStatsNoMarket.fromJson(json['noMarket'] as Map<String, dynamic>)
            : ProfileClaimsResponseStatsNoMarket.fromJson(const {}),
  );
}

class ProfileClaimsResponse {
  final List<Map<String, dynamic>> claims;
  final ProfileClaimsResponseStats stats;
  final RespSystemInfo systemInfo;

  const ProfileClaimsResponse({
    required this.claims,
    required this.stats,
    required this.systemInfo,
  });

  factory ProfileClaimsResponse.fromJson(Map<String, dynamic> json) => ProfileClaimsResponse(
    claims: json['claims'] is List ? (json['claims'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    stats: json['stats'] is Map<String, dynamic>
            ? ProfileClaimsResponseStats.fromJson(json['stats'] as Map<String, dynamic>)
            : ProfileClaimsResponseStats.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ProfileGetParams {
  /// List of hidden fields to include.
  final List<String>? fieldsInclude;

  const ProfileGetParams({
    this.fieldsInclude,
  });

  Map<String, Object?> toMap() {
    return {
      'fields_include': fieldsInclude,
    };
  }
}

class ProfileGetResponse {
  final UserModel user;
  final RespSystemInfo systemInfo;

  const ProfileGetResponse({
    required this.user,
    required this.systemInfo,
  });

  factory ProfileGetResponse.fromJson(Map<String, dynamic> json) => ProfileGetResponse(
    user: json['user'] is Map<String, dynamic>
            ? UserModel.fromJson(json['user'] as Map<String, dynamic>)
            : UserModel.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ProfileEditBody {
  final Map<String, dynamic>? user;
  final Map<String, dynamic>? option;
  /// Usernames who can transfer market accounts to you. Separate values with a comma.
  final List<String>? allowAcceptAccounts;
  /// Telegram api id.
  final String? telegramApiId;
  /// Telegram api hash.
  final String? telegramApiHash;
  /// Telegram device model.
  final String? telegramDeviceModel;
  /// Telegram system version.
  final String? telegramSystemVersion;
  /// Telegram app version.
  final String? telegramAppVersion;
  /// Telegram lang pack.
  final String? telegramLangPack;
  /// Telegram lang code.
  final String? telegramLangCode;
  /// Telegram system lang code.
  final String? telegramSystemLangCode;
  /// Clear Telegram data.
  final bool? clearTelegramClient;

  const ProfileEditBody({
    this.user,
    this.option,
    this.allowAcceptAccounts,
    this.telegramApiId,
    this.telegramApiHash,
    this.telegramDeviceModel,
    this.telegramSystemVersion,
    this.telegramAppVersion,
    this.telegramLangPack,
    this.telegramLangCode,
    this.telegramSystemLangCode,
    this.clearTelegramClient,
  });

  Map<String, Object?> toMap() {
    return {
      'user': user,
      'option': option,
      'allow_accept_accounts': allowAcceptAccounts,
      'telegram_api_id': telegramApiId,
      'telegram_api_hash': telegramApiHash,
      'telegram_device_model': telegramDeviceModel,
      'telegram_system_version': telegramSystemVersion,
      'telegram_app_version': telegramAppVersion,
      'telegram_lang_pack': telegramLangPack,
      'telegram_lang_code': telegramLangCode,
      'telegram_system_lang_code': telegramSystemLangCode,
      'clear_telegram_client': clearTelegramClient,
    };
  }
}

class ProfileEditResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ProfileEditResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ProfileEditResponse.fromJson(Map<String, dynamic> json) => ProfileEditResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

// ─── ProxyApi Types ────────────────────────────────────────

class ProxyGetResponse {
  final List<Map<String, dynamic>> proxies;
  final RespSystemInfo systemInfo;

  const ProxyGetResponse({
    required this.proxies,
    required this.systemInfo,
  });

  factory ProxyGetResponse.fromJson(Map<String, dynamic> json) => ProxyGetResponse(
    proxies: json['proxies'] is List ? (json['proxies'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class ProxyAddBody {
  /// Proxy ip or host. Required if $1 is not specified.
  final String? proxyIp;
  /// Proxy port. Required if $1 is not specified.
  final num? proxyPort;
  /// Proxy username. Required if $1 is not specified.
  final String? proxyUser;
  /// Proxy password. Required if $1 is not specified.
  final String? proxyPass;
  /// Proxy list in String format ip:port:user:pass. Each proxy must be start with new line (use \r\n separator).
  final String? proxyRow;

  const ProxyAddBody({
    this.proxyIp,
    this.proxyPort,
    this.proxyUser,
    this.proxyPass,
    this.proxyRow,
  });

  Map<String, Object?> toMap() {
    return {
      'proxy_ip': proxyIp,
      'proxy_port': proxyPort,
      'proxy_user': proxyUser,
      'proxy_pass': proxyPass,
      'proxy_row': proxyRow,
    };
  }
}

class ProxyAddResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ProxyAddResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ProxyAddResponse.fromJson(Map<String, dynamic> json) => ProxyAddResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class ProxyDeleteBody {
  /// Id of an existing proxy.
  final num? proxyId;
  /// Delete all proxies.
  final bool? deleteAll;

  const ProxyDeleteBody({
    this.proxyId,
    this.deleteAll,
  });

  Map<String, Object?> toMap() {
    return {
      'proxy_id': proxyId,
      'delete_all': deleteAll,
    };
  }
}

class ProxyDeleteResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const ProxyDeleteResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory ProxyDeleteResponse.fromJson(Map<String, dynamic> json) => ProxyDeleteResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

// ─── PublishingApi Types ────────────────────────────────────────

class PublishingFastSellBody {
  /// Title of account. If $1 specified and $1 is empty, $1 will be automatically translated to English language.
  final String? title;
  /// English title of account. If $1 specified and $1 is empty, $1 will be automatically translated to Russian language.
  final String? titleEn;
  /// Current price of account in your currency.
  final num price;
  final CategoryId categoryId;
  final Currency currency;
  final ItemOrigin itemOrigin;
  final ExtendedGuarantee? extendedGuarantee;
  /// Allow users to ask discount for this account.
  final bool? allowAskDiscount;
  /// Proxy id that will be used to check account.
  final num? proxyId;
  /// Set this parameter to $1 so that the Market will take a random proxy from its pool for each of your requests. Otherwise, if this parameter is set to $1 or not set, the Market will take a specific proxy from its pool, which is predefined for each item. > This parameter only works with proxies from the Market pool. If you want to use your own proxies, use the proxy_id or extra[proxy] parameter.
  final bool? randomProxy;
  /// Account public description.
  final String? description;
  /// Account private information (visible only for buyer).
  final String? information;
  /// Account login (or email).
  final String? login;
  /// Account password.
  final String? password;
  /// Account login data (login:password format).
  final String? loginPassword;
  /// Required if a $1 is one of list of Required email login data categories.
  final bool? hasEmailLoginData;
  /// Required if a $1 is one of list of Required email login data categories. Email login data (email:password format).
  final String? emailLoginData;
  final EmailType? emailType;
  final Map<String, dynamic>? extra;

  const PublishingFastSellBody({
    this.title,
    this.titleEn,
    required this.price,
    required this.categoryId,
    required this.currency,
    required this.itemOrigin,
    this.extendedGuarantee,
    this.allowAskDiscount,
    this.proxyId,
    this.randomProxy,
    this.description,
    this.information,
    this.login,
    this.password,
    this.loginPassword,
    this.hasEmailLoginData,
    this.emailLoginData,
    this.emailType,
    this.extra,
  });

  Map<String, Object?> toMap() {
    return {
      'title': title,
      'title_en': titleEn,
      'price': price,
      'category_id': categoryId.value,
      'currency': currency.value,
      'item_origin': itemOrigin.value,
      'extended_guarantee': extendedGuarantee?.value,
      'allow_ask_discount': allowAskDiscount,
      'proxy_id': proxyId,
      'random_proxy': randomProxy,
      'description': description,
      'information': information,
      'login': login,
      'password': password,
      'login_password': loginPassword,
      'has_email_login_data': hasEmailLoginData,
      'email_login_data': emailLoginData,
      'email_type': emailType?.value,
      'extra': extra,
    };
  }
}

class PublishingFastSellResponse {
  final ItemModel item;
  final String itemLink;
  final RespSystemInfo systemInfo;

  const PublishingFastSellResponse({
    required this.item,
    required this.itemLink,
    required this.systemInfo,
  });

  factory PublishingFastSellResponse.fromJson(Map<String, dynamic> json) => PublishingFastSellResponse(
    item: json['item'] is Map<String, dynamic>
            ? ItemModel.fromJson(json['item'] as Map<String, dynamic>)
            : ItemModel.fromJson(const {}),
    itemLink: json['itemLink'] is String ? json['itemLink'] as String : '',
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PublishingAddBody {
  /// Title of account. If $1 specified and $1 is empty, $1 will be automatically translated to English language.
  final String? title;
  /// English title of account. If $1 specified and $1 is empty, $1 will be automatically translated to Russian language.
  final String? titleEn;
  /// Current price of account in your currency.
  final num price;
  final CategoryId categoryId;
  final Currency currency;
  final ItemOrigin itemOrigin;
  final ExtendedGuarantee? extendedGuarantee;
  /// Account public description.
  final String? description;
  /// Account private information (visible only for buyer).
  final String? information;
  /// Get temporary email if not required by category. Available for Supercell, Fortnite and Epic Games categories.
  final bool? forceTempEmail;
  /// Put item id, if you are trying to resell item. This is useful to pass temporary email from reselling item to new item. You will get same temporary email from reselling account.
  final num? resellItemId;
  /// Required if a $1 is one of list of Required email login data categories.
  final bool? hasEmailLoginData;
  /// Required if a $1 is one of list of Required email login data categories. Email login data (email:password format).
  final String? emailLoginData;
  final EmailType? emailType;
  /// Allow users to ask discount for this account.
  final bool? allowAskDiscount;
  /// Proxy id that will be used to check account.
  final num? proxyId;
  /// Set this parameter to $1 so that the Market will take a random proxy from its pool for each of your requests. Otherwise, if this parameter is set to $1 or not set, the Market will take a specific proxy from its pool, which is predefined for each item. > This parameter only works with proxies from the Market pool. If you want to use your own proxies, use the proxy_id or extra[proxy] parameter.
  final bool? randomProxy;

  const PublishingAddBody({
    this.title,
    this.titleEn,
    required this.price,
    required this.categoryId,
    required this.currency,
    required this.itemOrigin,
    this.extendedGuarantee,
    this.description,
    this.information,
    this.forceTempEmail,
    this.resellItemId,
    this.hasEmailLoginData,
    this.emailLoginData,
    this.emailType,
    this.allowAskDiscount,
    this.proxyId,
    this.randomProxy,
  });

  Map<String, Object?> toMap() {
    return {
      'title': title,
      'title_en': titleEn,
      'price': price,
      'category_id': categoryId.value,
      'currency': currency.value,
      'item_origin': itemOrigin.value,
      'extended_guarantee': extendedGuarantee?.value,
      'description': description,
      'information': information,
      'forceTempEmail': forceTempEmail,
      'resell_item_id': resellItemId,
      'has_email_login_data': hasEmailLoginData,
      'email_login_data': emailLoginData,
      'email_type': emailType?.value,
      'allow_ask_discount': allowAskDiscount,
      'proxy_id': proxyId,
      'random_proxy': randomProxy,
    };
  }
}

class PublishingAddResponse {
  final String status;
  final ItemModel item;
  final RespSystemInfo systemInfo;

  const PublishingAddResponse({
    required this.status,
    required this.item,
    required this.systemInfo,
  });

  factory PublishingAddResponse.fromJson(Map<String, dynamic> json) => PublishingAddResponse(
    status: json['status'] is String ? json['status'] as String : '',
    item: json['item'] is Map<String, dynamic>
            ? ItemModel.fromJson(json['item'] as Map<String, dynamic>)
            : ItemModel.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PublishingCheckBody {
  /// Put if you are trying to resell an account.
  final num? resellItemId;
  /// Set this parameter to $1 so that the Market will take a random proxy from its pool for each of your requests. Otherwise, if this parameter is set to $1 or not set, the Market will take a specific proxy from its pool, which is predefined for each item. > This parameter only works with proxies from the Market pool. If you want to use your own proxies, use the proxy_id or extra[proxy] parameter.
  final bool? randomProxy;
  /// Account login (or email).
  final String? login;
  /// Account password.
  final String? password;
  /// Account login data (login:password format).
  final String? loginPassword;
  /// Required if a $1 is one of list of Required email login data categories.
  final bool? hasEmailLoginData;
  /// Required if a $1 is one of list of Required email login data categories. Email login data (email:password format).
  final String? emailLoginData;
  final EmailType? emailType;
  final Map<String, dynamic>? extra;

  const PublishingCheckBody({
    this.resellItemId,
    this.randomProxy,
    this.login,
    this.password,
    this.loginPassword,
    this.hasEmailLoginData,
    this.emailLoginData,
    this.emailType,
    this.extra,
  });

  Map<String, Object?> toMap() {
    return {
      'resell_item_id': resellItemId,
      'random_proxy': randomProxy,
      'login': login,
      'password': password,
      'login_password': loginPassword,
      'has_email_login_data': hasEmailLoginData,
      'email_login_data': emailLoginData,
      'email_type': emailType?.value,
      'extra': extra,
    };
  }
}

class PublishingCheckResponseItemGuarantee {
  final num duration;
  final String class$;
  final String durationPhrase;
  final num endDate;
  final bool active;
  final bool cancelled;
  final num remainingTime;
  final String remainingTimePhrase;

  const PublishingCheckResponseItemGuarantee({
    required this.duration,
    required this.class$,
    required this.durationPhrase,
    required this.endDate,
    required this.active,
    required this.cancelled,
    required this.remainingTime,
    required this.remainingTimePhrase,
  });

  factory PublishingCheckResponseItemGuarantee.fromJson(Map<String, dynamic> json) => PublishingCheckResponseItemGuarantee(
    duration: json['duration'] is num ? json['duration'] as num : 0,
    class$: json['class'] is String ? json['class'] as String : '',
    durationPhrase: json['durationPhrase'] is String ? json['durationPhrase'] as String : '',
    endDate: json['endDate'] is num ? json['endDate'] as num : 0,
    active: json['active'] is bool ? json['active'] as bool : false,
    cancelled: json['cancelled'] is bool ? json['cancelled'] as bool : false,
    remainingTime: json['remainingTime'] is num ? json['remainingTime'] as num : 0,
    remainingTimePhrase: json['remainingTimePhrase'] is String ? json['remainingTimePhrase'] as String : '',
  );
}

class PublishingCheckResponseItemLoginData {
  final String raw;
  final String encodedRaw;
  final String login;
  final String password;
  final String encodedPassword;
  final String oldPassword;
  final dynamic encodedOldPassword;

  const PublishingCheckResponseItemLoginData({
    required this.raw,
    required this.encodedRaw,
    required this.login,
    required this.password,
    required this.encodedPassword,
    required this.oldPassword,
    required this.encodedOldPassword,
  });

  factory PublishingCheckResponseItemLoginData.fromJson(Map<String, dynamic> json) => PublishingCheckResponseItemLoginData(
    raw: json['raw'] is String ? json['raw'] as String : '',
    encodedRaw: json['encodedRaw'] is String ? json['encodedRaw'] as String : '',
    login: json['login'] is String ? json['login'] as String : '',
    password: json['password'] is String ? json['password'] as String : '',
    encodedPassword: json['encodedPassword'] is String ? json['encodedPassword'] as String : '',
    oldPassword: json['oldPassword'] is String ? json['oldPassword'] as String : '',
    encodedOldPassword: json['encodedOldPassword'],
  );
}

class PublishingCheckResponseItemEmailLoginData {
  final String raw;
  final String encodedRaw;
  final String login;
  final String password;
  final String encodedPassword;
  final String oldPassword;
  final String encodedOldPassword;

  const PublishingCheckResponseItemEmailLoginData({
    required this.raw,
    required this.encodedRaw,
    required this.login,
    required this.password,
    required this.encodedPassword,
    required this.oldPassword,
    required this.encodedOldPassword,
  });

  factory PublishingCheckResponseItemEmailLoginData.fromJson(Map<String, dynamic> json) => PublishingCheckResponseItemEmailLoginData(
    raw: json['raw'] is String ? json['raw'] as String : '',
    encodedRaw: json['encodedRaw'] is String ? json['encodedRaw'] as String : '',
    login: json['login'] is String ? json['login'] as String : '',
    password: json['password'] is String ? json['password'] as String : '',
    encodedPassword: json['encodedPassword'] is String ? json['encodedPassword'] as String : '',
    oldPassword: json['oldPassword'] is String ? json['oldPassword'] as String : '',
    encodedOldPassword: json['encodedOldPassword'] is String ? json['encodedOldPassword'] as String : '',
  );
}

class PublishingCheckResponseItemBuyer {
  final num userId;
  final num operationDate;
  final bool visitorIsBuyer;
  final String username;
  final num isBanned;
  final num displayStyleGroupId;
  final String uniqUsernameCss;
  final num userGroupId;

  const PublishingCheckResponseItemBuyer({
    required this.userId,
    required this.operationDate,
    required this.visitorIsBuyer,
    required this.username,
    required this.isBanned,
    required this.displayStyleGroupId,
    required this.uniqUsernameCss,
    required this.userGroupId,
  });

  factory PublishingCheckResponseItemBuyer.fromJson(Map<String, dynamic> json) => PublishingCheckResponseItemBuyer(
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
    operationDate: json['operation_date'] is num ? json['operation_date'] as num : 0,
    visitorIsBuyer: json['visitorIsBuyer'] is bool ? json['visitorIsBuyer'] as bool : false,
    username: json['username'] is String ? json['username'] as String : '',
    isBanned: json['is_banned'] is num ? json['is_banned'] as num : 0,
    displayStyleGroupId: json['display_style_group_id'] is num ? json['display_style_group_id'] as num : 0,
    uniqUsernameCss: json['uniq_username_css'] is String ? json['uniq_username_css'] as String : '',
    userGroupId: json['user_group_id'] is num ? json['user_group_id'] as num : 0,
  );
}

class PublishingCheckResponseItemBumpSettings {
  final bool canBumpItem;
  final bool canBumpItemGlobally;
  final dynamic shortErrorPhrase;
  final dynamic errorPhrase;

  const PublishingCheckResponseItemBumpSettings({
    required this.canBumpItem,
    required this.canBumpItemGlobally,
    required this.shortErrorPhrase,
    required this.errorPhrase,
  });

  factory PublishingCheckResponseItemBumpSettings.fromJson(Map<String, dynamic> json) => PublishingCheckResponseItemBumpSettings(
    canBumpItem: json['canBumpItem'] is bool ? json['canBumpItem'] as bool : false,
    canBumpItemGlobally: json['canBumpItemGlobally'] is bool ? json['canBumpItemGlobally'] as bool : false,
    shortErrorPhrase: json['shortErrorPhrase'],
    errorPhrase: json['errorPhrase'],
  );
}

class PublishingCheckResponseItemSeller {
  final num userId;
  final String username;
  final num avatarDate;
  final num isBanned;
  final num displayStyleGroupId;
  final num joinedDate;
  final num soldItemsCount;
  final num activeItemsCount;
  final String restoreData;
  final num restorePercents;
  final bool isOnline;

  const PublishingCheckResponseItemSeller({
    required this.userId,
    required this.username,
    required this.avatarDate,
    required this.isBanned,
    required this.displayStyleGroupId,
    required this.joinedDate,
    required this.soldItemsCount,
    required this.activeItemsCount,
    required this.restoreData,
    required this.restorePercents,
    required this.isOnline,
  });

  factory PublishingCheckResponseItemSeller.fromJson(Map<String, dynamic> json) => PublishingCheckResponseItemSeller(
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
    username: json['username'] is String ? json['username'] as String : '',
    avatarDate: json['avatar_date'] is num ? json['avatar_date'] as num : 0,
    isBanned: json['is_banned'] is num ? json['is_banned'] as num : 0,
    displayStyleGroupId: json['display_style_group_id'] is num ? json['display_style_group_id'] as num : 0,
    joinedDate: json['joined_date'] is num ? json['joined_date'] as num : 0,
    soldItemsCount: json['sold_items_count'] is num ? json['sold_items_count'] as num : 0,
    activeItemsCount: json['active_items_count'] is num ? json['active_items_count'] as num : 0,
    restoreData: json['restore_data'] is String ? json['restore_data'] as String : '',
    restorePercents: json['restore_percents'] is num ? json['restore_percents'] as num : 0,
    isOnline: json['isOnline'] is bool ? json['isOnline'] as bool : false,
  );
}

class PublishingCheckResponseItem {
  final num itemId;
  final String itemState;
  final num categoryId;
  final num publishedDate;
  final String title;
  final String description;
  final num price;
  final num updateStatDate;
  final num refreshedDate;
  final num editDate;
  final String login;
  final String tempEmail;
  final num viewCount;
  final num isSticky;
  final String information;
  final String itemOrigin;
  final num extendedGuarantee;
  final num nsb;
  final num allowAskDiscount;
  final String titleEn;
  final String descriptionEn;
  final String informationEn;
  final String emailType;
  final String emailProvider;
  final String itemDomain;
  final String resaleItemOrigin;
  final num userAllowAskDiscount;
  final num maxDiscountPercent;
  final String marketCustomTitle;
  final String feedbackData;
  final num buyerAvatarDate;
  final num buyerUserGroupId;
  final num priceWithSellerFee;
  final PublishingCheckResponseItemGuarantee guarantee;
  final bool canViewLoginData;
  final bool canUpdateItemStats;
  final bool canReportItem;
  final PublishingCheckResponseItemLoginData loginData;
  final bool canViewEmailLoginData;
  final PublishingCheckResponseItemEmailLoginData emailLoginData;
  final bool showGetEmailCodeButton;
  final String getEmailCodeDisplayLogin;
  final PublishingCheckResponseItemBuyer buyer;
  final bool isPersonalAccount;
  final num soldItemsCategoryCount;
  final num restoreItemsCategoryCount;
  final num rubPrice;
  final String priceCurrency;
  final bool canValidateAccount;
  final bool canResellItemAfterPurchase;
  final num accountLastActivity;
  final bool displayConvertedBalance;
  final bool canViewAccountLink;
  final List<Map<String, dynamic>> accountLinks;
  final String accountLink;
  final String emailLoginUrl;
  final bool canChangePassword;
  final String itemOriginPhrase;
  final bool visitorIsAuthor;
  final bool canAskDiscount;
  final dynamic tags;
  final List<dynamic> customFields;
  final List<dynamic> externalAuth;
  final bool isTrusted;
  final bool isBirthdayToday;
  final bool isIgnored;
  final num deposit;
  final List<Map<String, dynamic>> extraPrices;
  final bool canViewAccountLoginAndTempEmail;
  final PublishingCheckResponseItemBumpSettings bumpSettings;
  final bool canCheckGuarantee;
  final bool needToRequireVideoToViewLoginData;
  final String descriptionHtml;
  final String descriptionEnHtml;
  final String descriptionPlain;
  final String descriptionEnPlain;
  final PublishingCheckResponseItemSeller seller;

  const PublishingCheckResponseItem({
    required this.itemId,
    required this.itemState,
    required this.categoryId,
    required this.publishedDate,
    required this.title,
    required this.description,
    required this.price,
    required this.updateStatDate,
    required this.refreshedDate,
    required this.editDate,
    required this.login,
    required this.tempEmail,
    required this.viewCount,
    required this.isSticky,
    required this.information,
    required this.itemOrigin,
    required this.extendedGuarantee,
    required this.nsb,
    required this.allowAskDiscount,
    required this.titleEn,
    required this.descriptionEn,
    required this.informationEn,
    required this.emailType,
    required this.emailProvider,
    required this.itemDomain,
    required this.resaleItemOrigin,
    required this.userAllowAskDiscount,
    required this.maxDiscountPercent,
    required this.marketCustomTitle,
    required this.feedbackData,
    required this.buyerAvatarDate,
    required this.buyerUserGroupId,
    required this.priceWithSellerFee,
    required this.guarantee,
    required this.canViewLoginData,
    required this.canUpdateItemStats,
    required this.canReportItem,
    required this.loginData,
    required this.canViewEmailLoginData,
    required this.emailLoginData,
    required this.showGetEmailCodeButton,
    required this.getEmailCodeDisplayLogin,
    required this.buyer,
    required this.isPersonalAccount,
    required this.soldItemsCategoryCount,
    required this.restoreItemsCategoryCount,
    required this.rubPrice,
    required this.priceCurrency,
    required this.canValidateAccount,
    required this.canResellItemAfterPurchase,
    required this.accountLastActivity,
    required this.displayConvertedBalance,
    required this.canViewAccountLink,
    required this.accountLinks,
    required this.accountLink,
    required this.emailLoginUrl,
    required this.canChangePassword,
    required this.itemOriginPhrase,
    required this.visitorIsAuthor,
    required this.canAskDiscount,
    required this.tags,
    required this.customFields,
    required this.externalAuth,
    required this.isTrusted,
    required this.isBirthdayToday,
    required this.isIgnored,
    required this.deposit,
    required this.extraPrices,
    required this.canViewAccountLoginAndTempEmail,
    required this.bumpSettings,
    required this.canCheckGuarantee,
    required this.needToRequireVideoToViewLoginData,
    required this.descriptionHtml,
    required this.descriptionEnHtml,
    required this.descriptionPlain,
    required this.descriptionEnPlain,
    required this.seller,
  });

  factory PublishingCheckResponseItem.fromJson(Map<String, dynamic> json) => PublishingCheckResponseItem(
    itemId: json['item_id'] is num ? json['item_id'] as num : 0,
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    categoryId: json['category_id'] is num ? json['category_id'] as num : 0,
    publishedDate: json['published_date'] is num ? json['published_date'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
    description: json['description'] is String ? json['description'] as String : '',
    price: json['price'] is num ? json['price'] as num : 0,
    updateStatDate: json['update_stat_date'] is num ? json['update_stat_date'] as num : 0,
    refreshedDate: json['refreshed_date'] is num ? json['refreshed_date'] as num : 0,
    editDate: json['edit_date'] is num ? json['edit_date'] as num : 0,
    login: json['login'] is String ? json['login'] as String : '',
    tempEmail: json['temp_email'] is String ? json['temp_email'] as String : '',
    viewCount: json['view_count'] is num ? json['view_count'] as num : 0,
    isSticky: json['is_sticky'] is num ? json['is_sticky'] as num : 0,
    information: json['information'] is String ? json['information'] as String : '',
    itemOrigin: json['item_origin'] is String ? json['item_origin'] as String : '',
    extendedGuarantee: json['extended_guarantee'] is num ? json['extended_guarantee'] as num : 0,
    nsb: json['nsb'] is num ? json['nsb'] as num : 0,
    allowAskDiscount: json['allow_ask_discount'] is num ? json['allow_ask_discount'] as num : 0,
    titleEn: json['title_en'] is String ? json['title_en'] as String : '',
    descriptionEn: json['description_en'] is String ? json['description_en'] as String : '',
    informationEn: json['information_en'] is String ? json['information_en'] as String : '',
    emailType: json['email_type'] is String ? json['email_type'] as String : '',
    emailProvider: json['email_provider'] is String ? json['email_provider'] as String : '',
    itemDomain: json['item_domain'] is String ? json['item_domain'] as String : '',
    resaleItemOrigin: json['resale_item_origin'] is String ? json['resale_item_origin'] as String : '',
    userAllowAskDiscount: json['user_allow_ask_discount'] is num ? json['user_allow_ask_discount'] as num : 0,
    maxDiscountPercent: json['max_discount_percent'] is num ? json['max_discount_percent'] as num : 0,
    marketCustomTitle: json['market_custom_title'] is String ? json['market_custom_title'] as String : '',
    feedbackData: json['feedback_data'] is String ? json['feedback_data'] as String : '',
    buyerAvatarDate: json['buyer_avatar_date'] is num ? json['buyer_avatar_date'] as num : 0,
    buyerUserGroupId: json['buyer_user_group_id'] is num ? json['buyer_user_group_id'] as num : 0,
    priceWithSellerFee: json['priceWithSellerFee'] is num ? json['priceWithSellerFee'] as num : 0,
    guarantee: json['guarantee'] is Map<String, dynamic>
            ? PublishingCheckResponseItemGuarantee.fromJson(json['guarantee'] as Map<String, dynamic>)
            : PublishingCheckResponseItemGuarantee.fromJson(const {}),
    canViewLoginData: json['canViewLoginData'] is bool ? json['canViewLoginData'] as bool : false,
    canUpdateItemStats: json['canUpdateItemStats'] is bool ? json['canUpdateItemStats'] as bool : false,
    canReportItem: json['canReportItem'] is bool ? json['canReportItem'] as bool : false,
    loginData: json['loginData'] is Map<String, dynamic>
            ? PublishingCheckResponseItemLoginData.fromJson(json['loginData'] as Map<String, dynamic>)
            : PublishingCheckResponseItemLoginData.fromJson(const {}),
    canViewEmailLoginData: json['canViewEmailLoginData'] is bool ? json['canViewEmailLoginData'] as bool : false,
    emailLoginData: json['emailLoginData'] is Map<String, dynamic>
            ? PublishingCheckResponseItemEmailLoginData.fromJson(json['emailLoginData'] as Map<String, dynamic>)
            : PublishingCheckResponseItemEmailLoginData.fromJson(const {}),
    showGetEmailCodeButton: json['showGetEmailCodeButton'] is bool ? json['showGetEmailCodeButton'] as bool : false,
    getEmailCodeDisplayLogin: json['getEmailCodeDisplayLogin'] is String ? json['getEmailCodeDisplayLogin'] as String : '',
    buyer: json['buyer'] is Map<String, dynamic>
            ? PublishingCheckResponseItemBuyer.fromJson(json['buyer'] as Map<String, dynamic>)
            : PublishingCheckResponseItemBuyer.fromJson(const {}),
    isPersonalAccount: json['isPersonalAccount'] is bool ? json['isPersonalAccount'] as bool : false,
    soldItemsCategoryCount: json['sold_items_category_count'] is num ? json['sold_items_category_count'] as num : 0,
    restoreItemsCategoryCount: json['restore_items_category_count'] is num ? json['restore_items_category_count'] as num : 0,
    rubPrice: json['rub_price'] is num ? json['rub_price'] as num : 0,
    priceCurrency: json['price_currency'] is String ? json['price_currency'] as String : '',
    canValidateAccount: json['canValidateAccount'] is bool ? json['canValidateAccount'] as bool : false,
    canResellItemAfterPurchase: json['canResellItemAfterPurchase'] is bool ? json['canResellItemAfterPurchase'] as bool : false,
    accountLastActivity: json['account_last_activity'] is num ? json['account_last_activity'] as num : 0,
    displayConvertedBalance: json['displayConvertedBalance'] is bool ? json['displayConvertedBalance'] as bool : false,
    canViewAccountLink: json['canViewAccountLink'] is bool ? json['canViewAccountLink'] as bool : false,
    accountLinks: json['accountLinks'] is List ? (json['accountLinks'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    accountLink: json['accountLink'] is String ? json['accountLink'] as String : '',
    emailLoginUrl: json['emailLoginUrl'] is String ? json['emailLoginUrl'] as String : '',
    canChangePassword: json['canChangePassword'] is bool ? json['canChangePassword'] as bool : false,
    itemOriginPhrase: json['itemOriginPhrase'] is String ? json['itemOriginPhrase'] as String : '',
    visitorIsAuthor: json['visitorIsAuthor'] is bool ? json['visitorIsAuthor'] as bool : false,
    canAskDiscount: json['canAskDiscount'] is bool ? json['canAskDiscount'] as bool : false,
    tags: json['tags'],
    customFields: json['customFields'] is List ? json['customFields'] as List<dynamic> : const [],
    externalAuth: json['externalAuth'] is List ? json['externalAuth'] as List<dynamic> : const [],
    isTrusted: json['isTrusted'] is bool ? json['isTrusted'] as bool : false,
    isBirthdayToday: json['isBirthdayToday'] is bool ? json['isBirthdayToday'] as bool : false,
    isIgnored: json['isIgnored'] is bool ? json['isIgnored'] as bool : false,
    deposit: json['deposit'] is num ? json['deposit'] as num : 0,
    extraPrices: json['extraPrices'] is List ? (json['extraPrices'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    canViewAccountLoginAndTempEmail: json['canViewAccountLoginAndTempEmail'] is bool ? json['canViewAccountLoginAndTempEmail'] as bool : false,
    bumpSettings: json['bumpSettings'] is Map<String, dynamic>
            ? PublishingCheckResponseItemBumpSettings.fromJson(json['bumpSettings'] as Map<String, dynamic>)
            : PublishingCheckResponseItemBumpSettings.fromJson(const {}),
    canCheckGuarantee: json['canCheckGuarantee'] is bool ? json['canCheckGuarantee'] as bool : false,
    needToRequireVideoToViewLoginData: json['needToRequireVideoToViewLoginData'] is bool ? json['needToRequireVideoToViewLoginData'] as bool : false,
    descriptionHtml: json['descriptionHtml'] is String ? json['descriptionHtml'] as String : '',
    descriptionEnHtml: json['descriptionEnHtml'] is String ? json['descriptionEnHtml'] as String : '',
    descriptionPlain: json['descriptionPlain'] is String ? json['descriptionPlain'] as String : '',
    descriptionEnPlain: json['descriptionEnPlain'] is String ? json['descriptionEnPlain'] as String : '',
    seller: json['seller'] is Map<String, dynamic>
            ? PublishingCheckResponseItemSeller.fromJson(json['seller'] as Map<String, dynamic>)
            : PublishingCheckResponseItemSeller.fromJson(const {}),
  );
}

class PublishingCheckResponse {
  final String status;
  final PublishingCheckResponseItem item;
  final RespSystemInfo systemInfo;

  const PublishingCheckResponse({
    required this.status,
    required this.item,
    required this.systemInfo,
  });

  factory PublishingCheckResponse.fromJson(Map<String, dynamic> json) => PublishingCheckResponse(
    status: json['status'] is String ? json['status'] as String : '',
    item: json['item'] is Map<String, dynamic>
            ? PublishingCheckResponseItem.fromJson(json['item'] as Map<String, dynamic>)
            : PublishingCheckResponseItem.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PublishingExternalBody {
  final Type type;
  /// Account login data (login:password format).
  final String? login;
  /// Email login data (email:password format).
  final String? emailLoginData;
  /// Cookies.
  final String? cookies;

  const PublishingExternalBody({
    required this.type,
    this.login,
    this.emailLoginData,
    this.cookies,
  });

  Map<String, Object?> toMap() {
    return {
      'type': type.value,
      'login': login,
      'email_login_data': emailLoginData,
      'cookies': cookies,
    };
  }
}

class PublishingExternalResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const PublishingExternalResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory PublishingExternalResponse.fromJson(Map<String, dynamic> json) => PublishingExternalResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

// ─── PurchasingApi Types ────────────────────────────────────────

class PurchasingFastBuyBody {
  /// Current price of account in your currency.
  final num? price;
  /// Balance ID that will be used to purchase specified item.
  final num? balanceId;

  const PurchasingFastBuyBody({
    this.price,
    this.balanceId,
  });

  Map<String, Object?> toMap() {
    return {
      'price': price,
      'balance_id': balanceId,
    };
  }
}

class PurchasingFastBuyResponseItemGuarantee {
  final num duration;
  final String class$;
  final String durationPhrase;
  final num endDate;
  final bool active;
  final bool cancelled;
  final num remainingTime;
  final String remainingTimePhrase;

  const PurchasingFastBuyResponseItemGuarantee({
    required this.duration,
    required this.class$,
    required this.durationPhrase,
    required this.endDate,
    required this.active,
    required this.cancelled,
    required this.remainingTime,
    required this.remainingTimePhrase,
  });

  factory PurchasingFastBuyResponseItemGuarantee.fromJson(Map<String, dynamic> json) => PurchasingFastBuyResponseItemGuarantee(
    duration: json['duration'] is num ? json['duration'] as num : 0,
    class$: json['class'] is String ? json['class'] as String : '',
    durationPhrase: json['durationPhrase'] is String ? json['durationPhrase'] as String : '',
    endDate: json['endDate'] is num ? json['endDate'] as num : 0,
    active: json['active'] is bool ? json['active'] as bool : false,
    cancelled: json['cancelled'] is bool ? json['cancelled'] as bool : false,
    remainingTime: json['remainingTime'] is num ? json['remainingTime'] as num : 0,
    remainingTimePhrase: json['remainingTimePhrase'] is String ? json['remainingTimePhrase'] as String : '',
  );
}

class PurchasingFastBuyResponseItemLoginData {
  final String raw;
  final String encodedRaw;
  final String login;
  final String password;
  final String encodedPassword;
  final String oldPassword;
  final dynamic encodedOldPassword;

  const PurchasingFastBuyResponseItemLoginData({
    required this.raw,
    required this.encodedRaw,
    required this.login,
    required this.password,
    required this.encodedPassword,
    required this.oldPassword,
    required this.encodedOldPassword,
  });

  factory PurchasingFastBuyResponseItemLoginData.fromJson(Map<String, dynamic> json) => PurchasingFastBuyResponseItemLoginData(
    raw: json['raw'] is String ? json['raw'] as String : '',
    encodedRaw: json['encodedRaw'] is String ? json['encodedRaw'] as String : '',
    login: json['login'] is String ? json['login'] as String : '',
    password: json['password'] is String ? json['password'] as String : '',
    encodedPassword: json['encodedPassword'] is String ? json['encodedPassword'] as String : '',
    oldPassword: json['oldPassword'] is String ? json['oldPassword'] as String : '',
    encodedOldPassword: json['encodedOldPassword'],
  );
}

class PurchasingFastBuyResponseItemEmailLoginData {
  final String raw;
  final String encodedRaw;
  final String login;
  final String password;
  final String encodedPassword;
  final String oldPassword;
  final String encodedOldPassword;

  const PurchasingFastBuyResponseItemEmailLoginData({
    required this.raw,
    required this.encodedRaw,
    required this.login,
    required this.password,
    required this.encodedPassword,
    required this.oldPassword,
    required this.encodedOldPassword,
  });

  factory PurchasingFastBuyResponseItemEmailLoginData.fromJson(Map<String, dynamic> json) => PurchasingFastBuyResponseItemEmailLoginData(
    raw: json['raw'] is String ? json['raw'] as String : '',
    encodedRaw: json['encodedRaw'] is String ? json['encodedRaw'] as String : '',
    login: json['login'] is String ? json['login'] as String : '',
    password: json['password'] is String ? json['password'] as String : '',
    encodedPassword: json['encodedPassword'] is String ? json['encodedPassword'] as String : '',
    oldPassword: json['oldPassword'] is String ? json['oldPassword'] as String : '',
    encodedOldPassword: json['encodedOldPassword'] is String ? json['encodedOldPassword'] as String : '',
  );
}

class PurchasingFastBuyResponseItemBuyer {
  final num userId;
  final num operationDate;
  final bool visitorIsBuyer;
  final String username;
  final num isBanned;
  final num displayStyleGroupId;
  final String uniqUsernameCss;
  final num userGroupId;

  const PurchasingFastBuyResponseItemBuyer({
    required this.userId,
    required this.operationDate,
    required this.visitorIsBuyer,
    required this.username,
    required this.isBanned,
    required this.displayStyleGroupId,
    required this.uniqUsernameCss,
    required this.userGroupId,
  });

  factory PurchasingFastBuyResponseItemBuyer.fromJson(Map<String, dynamic> json) => PurchasingFastBuyResponseItemBuyer(
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
    operationDate: json['operation_date'] is num ? json['operation_date'] as num : 0,
    visitorIsBuyer: json['visitorIsBuyer'] is bool ? json['visitorIsBuyer'] as bool : false,
    username: json['username'] is String ? json['username'] as String : '',
    isBanned: json['is_banned'] is num ? json['is_banned'] as num : 0,
    displayStyleGroupId: json['display_style_group_id'] is num ? json['display_style_group_id'] as num : 0,
    uniqUsernameCss: json['uniq_username_css'] is String ? json['uniq_username_css'] as String : '',
    userGroupId: json['user_group_id'] is num ? json['user_group_id'] as num : 0,
  );
}

class PurchasingFastBuyResponseItemBumpSettings {
  final bool canBumpItem;
  final bool canBumpItemGlobally;
  final dynamic shortErrorPhrase;
  final dynamic errorPhrase;

  const PurchasingFastBuyResponseItemBumpSettings({
    required this.canBumpItem,
    required this.canBumpItemGlobally,
    required this.shortErrorPhrase,
    required this.errorPhrase,
  });

  factory PurchasingFastBuyResponseItemBumpSettings.fromJson(Map<String, dynamic> json) => PurchasingFastBuyResponseItemBumpSettings(
    canBumpItem: json['canBumpItem'] is bool ? json['canBumpItem'] as bool : false,
    canBumpItemGlobally: json['canBumpItemGlobally'] is bool ? json['canBumpItemGlobally'] as bool : false,
    shortErrorPhrase: json['shortErrorPhrase'],
    errorPhrase: json['errorPhrase'],
  );
}

class PurchasingFastBuyResponseItemSeller {
  final num userId;
  final String username;
  final num avatarDate;
  final num isBanned;
  final num displayStyleGroupId;
  final num joinedDate;
  final num soldItemsCount;
  final num activeItemsCount;
  final String restoreData;
  final num restorePercents;
  final bool isOnline;

  const PurchasingFastBuyResponseItemSeller({
    required this.userId,
    required this.username,
    required this.avatarDate,
    required this.isBanned,
    required this.displayStyleGroupId,
    required this.joinedDate,
    required this.soldItemsCount,
    required this.activeItemsCount,
    required this.restoreData,
    required this.restorePercents,
    required this.isOnline,
  });

  factory PurchasingFastBuyResponseItemSeller.fromJson(Map<String, dynamic> json) => PurchasingFastBuyResponseItemSeller(
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
    username: json['username'] is String ? json['username'] as String : '',
    avatarDate: json['avatar_date'] is num ? json['avatar_date'] as num : 0,
    isBanned: json['is_banned'] is num ? json['is_banned'] as num : 0,
    displayStyleGroupId: json['display_style_group_id'] is num ? json['display_style_group_id'] as num : 0,
    joinedDate: json['joined_date'] is num ? json['joined_date'] as num : 0,
    soldItemsCount: json['sold_items_count'] is num ? json['sold_items_count'] as num : 0,
    activeItemsCount: json['active_items_count'] is num ? json['active_items_count'] as num : 0,
    restoreData: json['restore_data'] is String ? json['restore_data'] as String : '',
    restorePercents: json['restore_percents'] is num ? json['restore_percents'] as num : 0,
    isOnline: json['isOnline'] is bool ? json['isOnline'] as bool : false,
  );
}

class PurchasingFastBuyResponseItem {
  final num itemId;
  final String itemState;
  final num categoryId;
  final num publishedDate;
  final String title;
  final String description;
  final num price;
  final num updateStatDate;
  final num refreshedDate;
  final num editDate;
  final String login;
  final String tempEmail;
  final num viewCount;
  final num isSticky;
  final String information;
  final String itemOrigin;
  final num extendedGuarantee;
  final num nsb;
  final num allowAskDiscount;
  final String titleEn;
  final String descriptionEn;
  final String informationEn;
  final String emailType;
  final String emailProvider;
  final String itemDomain;
  final String resaleItemOrigin;
  final num userAllowAskDiscount;
  final num maxDiscountPercent;
  final String marketCustomTitle;
  final String feedbackData;
  final num buyerAvatarDate;
  final num buyerUserGroupId;
  final num priceWithSellerFee;
  final PurchasingFastBuyResponseItemGuarantee guarantee;
  final bool canViewLoginData;
  final bool canUpdateItemStats;
  final bool canReportItem;
  final PurchasingFastBuyResponseItemLoginData loginData;
  final bool canViewEmailLoginData;
  final PurchasingFastBuyResponseItemEmailLoginData emailLoginData;
  final bool showGetEmailCodeButton;
  final String getEmailCodeDisplayLogin;
  final PurchasingFastBuyResponseItemBuyer buyer;
  final bool isPersonalAccount;
  final num soldItemsCategoryCount;
  final num restoreItemsCategoryCount;
  final num rubPrice;
  final String priceCurrency;
  final bool canValidateAccount;
  final bool canResellItemAfterPurchase;
  final num accountLastActivity;
  final bool displayConvertedBalance;
  final bool canViewAccountLink;
  final List<Map<String, dynamic>> accountLinks;
  final String accountLink;
  final String emailLoginUrl;
  final bool canChangePassword;
  final String itemOriginPhrase;
  final bool visitorIsAuthor;
  final bool canAskDiscount;
  final dynamic tags;
  final List<dynamic> customFields;
  final List<dynamic> externalAuth;
  final bool isTrusted;
  final bool isBirthdayToday;
  final bool isIgnored;
  final num deposit;
  final List<Map<String, dynamic>> extraPrices;
  final bool canViewAccountLoginAndTempEmail;
  final PurchasingFastBuyResponseItemBumpSettings bumpSettings;
  final bool canCheckGuarantee;
  final bool needToRequireVideoToViewLoginData;
  final String descriptionHtml;
  final String descriptionEnHtml;
  final String descriptionPlain;
  final String descriptionEnPlain;
  final PurchasingFastBuyResponseItemSeller seller;

  const PurchasingFastBuyResponseItem({
    required this.itemId,
    required this.itemState,
    required this.categoryId,
    required this.publishedDate,
    required this.title,
    required this.description,
    required this.price,
    required this.updateStatDate,
    required this.refreshedDate,
    required this.editDate,
    required this.login,
    required this.tempEmail,
    required this.viewCount,
    required this.isSticky,
    required this.information,
    required this.itemOrigin,
    required this.extendedGuarantee,
    required this.nsb,
    required this.allowAskDiscount,
    required this.titleEn,
    required this.descriptionEn,
    required this.informationEn,
    required this.emailType,
    required this.emailProvider,
    required this.itemDomain,
    required this.resaleItemOrigin,
    required this.userAllowAskDiscount,
    required this.maxDiscountPercent,
    required this.marketCustomTitle,
    required this.feedbackData,
    required this.buyerAvatarDate,
    required this.buyerUserGroupId,
    required this.priceWithSellerFee,
    required this.guarantee,
    required this.canViewLoginData,
    required this.canUpdateItemStats,
    required this.canReportItem,
    required this.loginData,
    required this.canViewEmailLoginData,
    required this.emailLoginData,
    required this.showGetEmailCodeButton,
    required this.getEmailCodeDisplayLogin,
    required this.buyer,
    required this.isPersonalAccount,
    required this.soldItemsCategoryCount,
    required this.restoreItemsCategoryCount,
    required this.rubPrice,
    required this.priceCurrency,
    required this.canValidateAccount,
    required this.canResellItemAfterPurchase,
    required this.accountLastActivity,
    required this.displayConvertedBalance,
    required this.canViewAccountLink,
    required this.accountLinks,
    required this.accountLink,
    required this.emailLoginUrl,
    required this.canChangePassword,
    required this.itemOriginPhrase,
    required this.visitorIsAuthor,
    required this.canAskDiscount,
    required this.tags,
    required this.customFields,
    required this.externalAuth,
    required this.isTrusted,
    required this.isBirthdayToday,
    required this.isIgnored,
    required this.deposit,
    required this.extraPrices,
    required this.canViewAccountLoginAndTempEmail,
    required this.bumpSettings,
    required this.canCheckGuarantee,
    required this.needToRequireVideoToViewLoginData,
    required this.descriptionHtml,
    required this.descriptionEnHtml,
    required this.descriptionPlain,
    required this.descriptionEnPlain,
    required this.seller,
  });

  factory PurchasingFastBuyResponseItem.fromJson(Map<String, dynamic> json) => PurchasingFastBuyResponseItem(
    itemId: json['item_id'] is num ? json['item_id'] as num : 0,
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    categoryId: json['category_id'] is num ? json['category_id'] as num : 0,
    publishedDate: json['published_date'] is num ? json['published_date'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
    description: json['description'] is String ? json['description'] as String : '',
    price: json['price'] is num ? json['price'] as num : 0,
    updateStatDate: json['update_stat_date'] is num ? json['update_stat_date'] as num : 0,
    refreshedDate: json['refreshed_date'] is num ? json['refreshed_date'] as num : 0,
    editDate: json['edit_date'] is num ? json['edit_date'] as num : 0,
    login: json['login'] is String ? json['login'] as String : '',
    tempEmail: json['temp_email'] is String ? json['temp_email'] as String : '',
    viewCount: json['view_count'] is num ? json['view_count'] as num : 0,
    isSticky: json['is_sticky'] is num ? json['is_sticky'] as num : 0,
    information: json['information'] is String ? json['information'] as String : '',
    itemOrigin: json['item_origin'] is String ? json['item_origin'] as String : '',
    extendedGuarantee: json['extended_guarantee'] is num ? json['extended_guarantee'] as num : 0,
    nsb: json['nsb'] is num ? json['nsb'] as num : 0,
    allowAskDiscount: json['allow_ask_discount'] is num ? json['allow_ask_discount'] as num : 0,
    titleEn: json['title_en'] is String ? json['title_en'] as String : '',
    descriptionEn: json['description_en'] is String ? json['description_en'] as String : '',
    informationEn: json['information_en'] is String ? json['information_en'] as String : '',
    emailType: json['email_type'] is String ? json['email_type'] as String : '',
    emailProvider: json['email_provider'] is String ? json['email_provider'] as String : '',
    itemDomain: json['item_domain'] is String ? json['item_domain'] as String : '',
    resaleItemOrigin: json['resale_item_origin'] is String ? json['resale_item_origin'] as String : '',
    userAllowAskDiscount: json['user_allow_ask_discount'] is num ? json['user_allow_ask_discount'] as num : 0,
    maxDiscountPercent: json['max_discount_percent'] is num ? json['max_discount_percent'] as num : 0,
    marketCustomTitle: json['market_custom_title'] is String ? json['market_custom_title'] as String : '',
    feedbackData: json['feedback_data'] is String ? json['feedback_data'] as String : '',
    buyerAvatarDate: json['buyer_avatar_date'] is num ? json['buyer_avatar_date'] as num : 0,
    buyerUserGroupId: json['buyer_user_group_id'] is num ? json['buyer_user_group_id'] as num : 0,
    priceWithSellerFee: json['priceWithSellerFee'] is num ? json['priceWithSellerFee'] as num : 0,
    guarantee: json['guarantee'] is Map<String, dynamic>
            ? PurchasingFastBuyResponseItemGuarantee.fromJson(json['guarantee'] as Map<String, dynamic>)
            : PurchasingFastBuyResponseItemGuarantee.fromJson(const {}),
    canViewLoginData: json['canViewLoginData'] is bool ? json['canViewLoginData'] as bool : false,
    canUpdateItemStats: json['canUpdateItemStats'] is bool ? json['canUpdateItemStats'] as bool : false,
    canReportItem: json['canReportItem'] is bool ? json['canReportItem'] as bool : false,
    loginData: json['loginData'] is Map<String, dynamic>
            ? PurchasingFastBuyResponseItemLoginData.fromJson(json['loginData'] as Map<String, dynamic>)
            : PurchasingFastBuyResponseItemLoginData.fromJson(const {}),
    canViewEmailLoginData: json['canViewEmailLoginData'] is bool ? json['canViewEmailLoginData'] as bool : false,
    emailLoginData: json['emailLoginData'] is Map<String, dynamic>
            ? PurchasingFastBuyResponseItemEmailLoginData.fromJson(json['emailLoginData'] as Map<String, dynamic>)
            : PurchasingFastBuyResponseItemEmailLoginData.fromJson(const {}),
    showGetEmailCodeButton: json['showGetEmailCodeButton'] is bool ? json['showGetEmailCodeButton'] as bool : false,
    getEmailCodeDisplayLogin: json['getEmailCodeDisplayLogin'] is String ? json['getEmailCodeDisplayLogin'] as String : '',
    buyer: json['buyer'] is Map<String, dynamic>
            ? PurchasingFastBuyResponseItemBuyer.fromJson(json['buyer'] as Map<String, dynamic>)
            : PurchasingFastBuyResponseItemBuyer.fromJson(const {}),
    isPersonalAccount: json['isPersonalAccount'] is bool ? json['isPersonalAccount'] as bool : false,
    soldItemsCategoryCount: json['sold_items_category_count'] is num ? json['sold_items_category_count'] as num : 0,
    restoreItemsCategoryCount: json['restore_items_category_count'] is num ? json['restore_items_category_count'] as num : 0,
    rubPrice: json['rub_price'] is num ? json['rub_price'] as num : 0,
    priceCurrency: json['price_currency'] is String ? json['price_currency'] as String : '',
    canValidateAccount: json['canValidateAccount'] is bool ? json['canValidateAccount'] as bool : false,
    canResellItemAfterPurchase: json['canResellItemAfterPurchase'] is bool ? json['canResellItemAfterPurchase'] as bool : false,
    accountLastActivity: json['account_last_activity'] is num ? json['account_last_activity'] as num : 0,
    displayConvertedBalance: json['displayConvertedBalance'] is bool ? json['displayConvertedBalance'] as bool : false,
    canViewAccountLink: json['canViewAccountLink'] is bool ? json['canViewAccountLink'] as bool : false,
    accountLinks: json['accountLinks'] is List ? (json['accountLinks'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    accountLink: json['accountLink'] is String ? json['accountLink'] as String : '',
    emailLoginUrl: json['emailLoginUrl'] is String ? json['emailLoginUrl'] as String : '',
    canChangePassword: json['canChangePassword'] is bool ? json['canChangePassword'] as bool : false,
    itemOriginPhrase: json['itemOriginPhrase'] is String ? json['itemOriginPhrase'] as String : '',
    visitorIsAuthor: json['visitorIsAuthor'] is bool ? json['visitorIsAuthor'] as bool : false,
    canAskDiscount: json['canAskDiscount'] is bool ? json['canAskDiscount'] as bool : false,
    tags: json['tags'],
    customFields: json['customFields'] is List ? json['customFields'] as List<dynamic> : const [],
    externalAuth: json['externalAuth'] is List ? json['externalAuth'] as List<dynamic> : const [],
    isTrusted: json['isTrusted'] is bool ? json['isTrusted'] as bool : false,
    isBirthdayToday: json['isBirthdayToday'] is bool ? json['isBirthdayToday'] as bool : false,
    isIgnored: json['isIgnored'] is bool ? json['isIgnored'] as bool : false,
    deposit: json['deposit'] is num ? json['deposit'] as num : 0,
    extraPrices: json['extraPrices'] is List ? (json['extraPrices'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    canViewAccountLoginAndTempEmail: json['canViewAccountLoginAndTempEmail'] is bool ? json['canViewAccountLoginAndTempEmail'] as bool : false,
    bumpSettings: json['bumpSettings'] is Map<String, dynamic>
            ? PurchasingFastBuyResponseItemBumpSettings.fromJson(json['bumpSettings'] as Map<String, dynamic>)
            : PurchasingFastBuyResponseItemBumpSettings.fromJson(const {}),
    canCheckGuarantee: json['canCheckGuarantee'] is bool ? json['canCheckGuarantee'] as bool : false,
    needToRequireVideoToViewLoginData: json['needToRequireVideoToViewLoginData'] is bool ? json['needToRequireVideoToViewLoginData'] as bool : false,
    descriptionHtml: json['descriptionHtml'] is String ? json['descriptionHtml'] as String : '',
    descriptionEnHtml: json['descriptionEnHtml'] is String ? json['descriptionEnHtml'] as String : '',
    descriptionPlain: json['descriptionPlain'] is String ? json['descriptionPlain'] as String : '',
    descriptionEnPlain: json['descriptionEnPlain'] is String ? json['descriptionEnPlain'] as String : '',
    seller: json['seller'] is Map<String, dynamic>
            ? PurchasingFastBuyResponseItemSeller.fromJson(json['seller'] as Map<String, dynamic>)
            : PurchasingFastBuyResponseItemSeller.fromJson(const {}),
  );
}

class PurchasingFastBuyResponse {
  final String status;
  final PurchasingFastBuyResponseItem item;
  final RespSystemInfo systemInfo;

  const PurchasingFastBuyResponse({
    required this.status,
    required this.item,
    required this.systemInfo,
  });

  factory PurchasingFastBuyResponse.fromJson(Map<String, dynamic> json) => PurchasingFastBuyResponse(
    status: json['status'] is String ? json['status'] as String : '',
    item: json['item'] is Map<String, dynamic>
            ? PurchasingFastBuyResponseItem.fromJson(json['item'] as Map<String, dynamic>)
            : PurchasingFastBuyResponseItem.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PurchasingCheckResponseItemGuarantee {
  final num duration;
  final String class$;
  final String durationPhrase;
  final dynamic endDate;
  final dynamic active;
  final dynamic cancelled;
  final dynamic remainingTime;

  const PurchasingCheckResponseItemGuarantee({
    required this.duration,
    required this.class$,
    required this.durationPhrase,
    required this.endDate,
    required this.active,
    required this.cancelled,
    required this.remainingTime,
  });

  factory PurchasingCheckResponseItemGuarantee.fromJson(Map<String, dynamic> json) => PurchasingCheckResponseItemGuarantee(
    duration: json['duration'] is num ? json['duration'] as num : 0,
    class$: json['class'] is String ? json['class'] as String : '',
    durationPhrase: json['durationPhrase'] is String ? json['durationPhrase'] as String : '',
    endDate: json['endDate'],
    active: json['active'],
    cancelled: json['cancelled'],
    remainingTime: json['remainingTime'],
  );
}

class PurchasingCheckResponseItemBumpSettings {
  final bool canBumpItem;
  final bool canBumpItemGlobally;
  final dynamic shortErrorPhrase;
  final dynamic errorPhrase;

  const PurchasingCheckResponseItemBumpSettings({
    required this.canBumpItem,
    required this.canBumpItemGlobally,
    required this.shortErrorPhrase,
    required this.errorPhrase,
  });

  factory PurchasingCheckResponseItemBumpSettings.fromJson(Map<String, dynamic> json) => PurchasingCheckResponseItemBumpSettings(
    canBumpItem: json['canBumpItem'] is bool ? json['canBumpItem'] as bool : false,
    canBumpItemGlobally: json['canBumpItemGlobally'] is bool ? json['canBumpItemGlobally'] as bool : false,
    shortErrorPhrase: json['shortErrorPhrase'],
    errorPhrase: json['errorPhrase'],
  );
}

class PurchasingCheckResponseItemSeller {
  final num userId;
  final String username;
  final num avatarDate;
  final num isBanned;
  final num displayStyleGroupId;
  final num joinedDate;
  final num soldItemsCount;
  final num activeItemsCount;
  final String restoreData;
  final dynamic restorePercents;
  final bool isOnline;

  const PurchasingCheckResponseItemSeller({
    required this.userId,
    required this.username,
    required this.avatarDate,
    required this.isBanned,
    required this.displayStyleGroupId,
    required this.joinedDate,
    required this.soldItemsCount,
    required this.activeItemsCount,
    required this.restoreData,
    required this.restorePercents,
    required this.isOnline,
  });

  factory PurchasingCheckResponseItemSeller.fromJson(Map<String, dynamic> json) => PurchasingCheckResponseItemSeller(
    userId: json['user_id'] is num ? json['user_id'] as num : 0,
    username: json['username'] is String ? json['username'] as String : '',
    avatarDate: json['avatar_date'] is num ? json['avatar_date'] as num : 0,
    isBanned: json['is_banned'] is num ? json['is_banned'] as num : 0,
    displayStyleGroupId: json['display_style_group_id'] is num ? json['display_style_group_id'] as num : 0,
    joinedDate: json['joined_date'] is num ? json['joined_date'] as num : 0,
    soldItemsCount: json['sold_items_count'] is num ? json['sold_items_count'] as num : 0,
    activeItemsCount: json['active_items_count'] is num ? json['active_items_count'] as num : 0,
    restoreData: json['restore_data'] is String ? json['restore_data'] as String : '',
    restorePercents: json['restore_percents'],
    isOnline: json['isOnline'] is bool ? json['isOnline'] as bool : false,
  );
}

class PurchasingCheckResponseItem {
  final num itemId;
  final String itemState;
  final num categoryId;
  final num publishedDate;
  final String title;
  final String description;
  final num price;
  final num updateStatDate;
  final num refreshedDate;
  final num editDate;
  final num viewCount;
  final num isSticky;
  final String itemOrigin;
  final num extendedGuarantee;
  final num nsb;
  final num allowAskDiscount;
  final String titleEn;
  final String descriptionEn;
  final String emailType;
  final String emailProvider;
  final String itemDomain;
  final String resaleItemOrigin;
  final num userAllowAskDiscount;
  final num maxDiscountPercent;
  final String marketCustomTitle;
  final String feedbackData;
  final String categoryTitle;
  final String categoryUrl;
  final num requireTempEmail;
  final num availableTempEmail;
  final num checkButtonEnabled;
  final num checkerEnabled;
  final num buyWithoutValidation;
  final num hasGuarantee;
  final num requireVideoRecording;
  final num canBeResold;
  final String loginType;
  final num requireEmailLoginData;
  final num categoryPrefixId;
  final dynamic askUserId;
  final dynamic askItemId;
  final dynamic askDate;
  final dynamic discountPrice;
  final dynamic discountAccepted;
  final dynamic userAlerted;
  final dynamic message;
  final num minPrice;
  final num priceWithSellerFee;
  final PurchasingCheckResponseItemGuarantee guarantee;
  final bool canViewLoginData;
  final bool canUpdateItemStats;
  final bool canReportItem;
  final bool canViewEmailLoginData;
  final bool showGetEmailCodeButton;
  final bool isPersonalAccount;
  final num rubPrice;
  final String priceCurrency;
  final bool canValidateAccount;
  final bool canResellItemAfterPurchase;
  final bool isSmallExf;
  final num accountLastActivity;
  final bool canViewAccountLink;
  final List<Map<String, dynamic>> accountLinks;
  final String accountLink;
  final String emailLoginUrl;
  final bool canChangePassword;
  final String itemOriginPhrase;
  final bool visitorIsAuthor;
  final bool canAskDiscount;
  final List<dynamic> tags;
  final List<dynamic> customFields;
  final List<dynamic> externalAuth;
  final bool isTrusted;
  final bool isBirthdayToday;
  final bool isIgnored;
  final num deposit;
  final List<Map<String, dynamic>> extraPrices;
  final bool canViewAccountLoginAndTempEmail;
  final PurchasingCheckResponseItemBumpSettings bumpSettings;
  final bool canCheckGuarantee;
  final bool needToRequireVideoToViewLoginData;
  final String descriptionHtml;
  final String descriptionEnHtml;
  final String descriptionPlain;
  final String descriptionEnPlain;
  final PurchasingCheckResponseItemSeller seller;

  const PurchasingCheckResponseItem({
    required this.itemId,
    required this.itemState,
    required this.categoryId,
    required this.publishedDate,
    required this.title,
    required this.description,
    required this.price,
    required this.updateStatDate,
    required this.refreshedDate,
    required this.editDate,
    required this.viewCount,
    required this.isSticky,
    required this.itemOrigin,
    required this.extendedGuarantee,
    required this.nsb,
    required this.allowAskDiscount,
    required this.titleEn,
    required this.descriptionEn,
    required this.emailType,
    required this.emailProvider,
    required this.itemDomain,
    required this.resaleItemOrigin,
    required this.userAllowAskDiscount,
    required this.maxDiscountPercent,
    required this.marketCustomTitle,
    required this.feedbackData,
    required this.categoryTitle,
    required this.categoryUrl,
    required this.requireTempEmail,
    required this.availableTempEmail,
    required this.checkButtonEnabled,
    required this.checkerEnabled,
    required this.buyWithoutValidation,
    required this.hasGuarantee,
    required this.requireVideoRecording,
    required this.canBeResold,
    required this.loginType,
    required this.requireEmailLoginData,
    required this.categoryPrefixId,
    required this.askUserId,
    required this.askItemId,
    required this.askDate,
    required this.discountPrice,
    required this.discountAccepted,
    required this.userAlerted,
    required this.message,
    required this.minPrice,
    required this.priceWithSellerFee,
    required this.guarantee,
    required this.canViewLoginData,
    required this.canUpdateItemStats,
    required this.canReportItem,
    required this.canViewEmailLoginData,
    required this.showGetEmailCodeButton,
    required this.isPersonalAccount,
    required this.rubPrice,
    required this.priceCurrency,
    required this.canValidateAccount,
    required this.canResellItemAfterPurchase,
    required this.isSmallExf,
    required this.accountLastActivity,
    required this.canViewAccountLink,
    required this.accountLinks,
    required this.accountLink,
    required this.emailLoginUrl,
    required this.canChangePassword,
    required this.itemOriginPhrase,
    required this.visitorIsAuthor,
    required this.canAskDiscount,
    required this.tags,
    required this.customFields,
    required this.externalAuth,
    required this.isTrusted,
    required this.isBirthdayToday,
    required this.isIgnored,
    required this.deposit,
    required this.extraPrices,
    required this.canViewAccountLoginAndTempEmail,
    required this.bumpSettings,
    required this.canCheckGuarantee,
    required this.needToRequireVideoToViewLoginData,
    required this.descriptionHtml,
    required this.descriptionEnHtml,
    required this.descriptionPlain,
    required this.descriptionEnPlain,
    required this.seller,
  });

  factory PurchasingCheckResponseItem.fromJson(Map<String, dynamic> json) => PurchasingCheckResponseItem(
    itemId: json['item_id'] is num ? json['item_id'] as num : 0,
    itemState: json['item_state'] is String ? json['item_state'] as String : '',
    categoryId: json['category_id'] is num ? json['category_id'] as num : 0,
    publishedDate: json['published_date'] is num ? json['published_date'] as num : 0,
    title: json['title'] is String ? json['title'] as String : '',
    description: json['description'] is String ? json['description'] as String : '',
    price: json['price'] is num ? json['price'] as num : 0,
    updateStatDate: json['update_stat_date'] is num ? json['update_stat_date'] as num : 0,
    refreshedDate: json['refreshed_date'] is num ? json['refreshed_date'] as num : 0,
    editDate: json['edit_date'] is num ? json['edit_date'] as num : 0,
    viewCount: json['view_count'] is num ? json['view_count'] as num : 0,
    isSticky: json['is_sticky'] is num ? json['is_sticky'] as num : 0,
    itemOrigin: json['item_origin'] is String ? json['item_origin'] as String : '',
    extendedGuarantee: json['extended_guarantee'] is num ? json['extended_guarantee'] as num : 0,
    nsb: json['nsb'] is num ? json['nsb'] as num : 0,
    allowAskDiscount: json['allow_ask_discount'] is num ? json['allow_ask_discount'] as num : 0,
    titleEn: json['title_en'] is String ? json['title_en'] as String : '',
    descriptionEn: json['description_en'] is String ? json['description_en'] as String : '',
    emailType: json['email_type'] is String ? json['email_type'] as String : '',
    emailProvider: json['email_provider'] is String ? json['email_provider'] as String : '',
    itemDomain: json['item_domain'] is String ? json['item_domain'] as String : '',
    resaleItemOrigin: json['resale_item_origin'] is String ? json['resale_item_origin'] as String : '',
    userAllowAskDiscount: json['user_allow_ask_discount'] is num ? json['user_allow_ask_discount'] as num : 0,
    maxDiscountPercent: json['max_discount_percent'] is num ? json['max_discount_percent'] as num : 0,
    marketCustomTitle: json['market_custom_title'] is String ? json['market_custom_title'] as String : '',
    feedbackData: json['feedback_data'] is String ? json['feedback_data'] as String : '',
    categoryTitle: json['category_title'] is String ? json['category_title'] as String : '',
    categoryUrl: json['category_url'] is String ? json['category_url'] as String : '',
    requireTempEmail: json['require_temp_email'] is num ? json['require_temp_email'] as num : 0,
    availableTempEmail: json['available_temp_email'] is num ? json['available_temp_email'] as num : 0,
    checkButtonEnabled: json['check_button_enabled'] is num ? json['check_button_enabled'] as num : 0,
    checkerEnabled: json['checker_enabled'] is num ? json['checker_enabled'] as num : 0,
    buyWithoutValidation: json['buy_without_validation'] is num ? json['buy_without_validation'] as num : 0,
    hasGuarantee: json['has_guarantee'] is num ? json['has_guarantee'] as num : 0,
    requireVideoRecording: json['require_video_recording'] is num ? json['require_video_recording'] as num : 0,
    canBeResold: json['can_be_resold'] is num ? json['can_be_resold'] as num : 0,
    loginType: json['login_type'] is String ? json['login_type'] as String : '',
    requireEmailLoginData: json['require_email_login_data'] is num ? json['require_email_login_data'] as num : 0,
    categoryPrefixId: json['category_prefix_id'] is num ? json['category_prefix_id'] as num : 0,
    askUserId: json['ask_user_id'],
    askItemId: json['ask_item_id'],
    askDate: json['ask_date'],
    discountPrice: json['discount_price'],
    discountAccepted: json['discount_accepted'],
    userAlerted: json['user_alerted'],
    message: json['message'],
    minPrice: json['min_price'] is num ? json['min_price'] as num : 0,
    priceWithSellerFee: json['priceWithSellerFee'] is num ? json['priceWithSellerFee'] as num : 0,
    guarantee: json['guarantee'] is Map<String, dynamic>
            ? PurchasingCheckResponseItemGuarantee.fromJson(json['guarantee'] as Map<String, dynamic>)
            : PurchasingCheckResponseItemGuarantee.fromJson(const {}),
    canViewLoginData: json['canViewLoginData'] is bool ? json['canViewLoginData'] as bool : false,
    canUpdateItemStats: json['canUpdateItemStats'] is bool ? json['canUpdateItemStats'] as bool : false,
    canReportItem: json['canReportItem'] is bool ? json['canReportItem'] as bool : false,
    canViewEmailLoginData: json['canViewEmailLoginData'] is bool ? json['canViewEmailLoginData'] as bool : false,
    showGetEmailCodeButton: json['showGetEmailCodeButton'] is bool ? json['showGetEmailCodeButton'] as bool : false,
    isPersonalAccount: json['isPersonalAccount'] is bool ? json['isPersonalAccount'] as bool : false,
    rubPrice: json['rub_price'] is num ? json['rub_price'] as num : 0,
    priceCurrency: json['price_currency'] is String ? json['price_currency'] as String : '',
    canValidateAccount: json['canValidateAccount'] is bool ? json['canValidateAccount'] as bool : false,
    canResellItemAfterPurchase: json['canResellItemAfterPurchase'] is bool ? json['canResellItemAfterPurchase'] as bool : false,
    isSmallExf: json['isSmallExf'] is bool ? json['isSmallExf'] as bool : false,
    accountLastActivity: json['account_last_activity'] is num ? json['account_last_activity'] as num : 0,
    canViewAccountLink: json['canViewAccountLink'] is bool ? json['canViewAccountLink'] as bool : false,
    accountLinks: json['accountLinks'] is List ? (json['accountLinks'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    accountLink: json['accountLink'] is String ? json['accountLink'] as String : '',
    emailLoginUrl: json['emailLoginUrl'] is String ? json['emailLoginUrl'] as String : '',
    canChangePassword: json['canChangePassword'] is bool ? json['canChangePassword'] as bool : false,
    itemOriginPhrase: json['itemOriginPhrase'] is String ? json['itemOriginPhrase'] as String : '',
    visitorIsAuthor: json['visitorIsAuthor'] is bool ? json['visitorIsAuthor'] as bool : false,
    canAskDiscount: json['canAskDiscount'] is bool ? json['canAskDiscount'] as bool : false,
    tags: json['tags'] is List ? json['tags'] as List<dynamic> : const [],
    customFields: json['customFields'] is List ? json['customFields'] as List<dynamic> : const [],
    externalAuth: json['externalAuth'] is List ? json['externalAuth'] as List<dynamic> : const [],
    isTrusted: json['isTrusted'] is bool ? json['isTrusted'] as bool : false,
    isBirthdayToday: json['isBirthdayToday'] is bool ? json['isBirthdayToday'] as bool : false,
    isIgnored: json['isIgnored'] is bool ? json['isIgnored'] as bool : false,
    deposit: json['deposit'] is num ? json['deposit'] as num : 0,
    extraPrices: json['extraPrices'] is List ? (json['extraPrices'] as List<dynamic>).cast<Map<String, dynamic>>() : const [],
    canViewAccountLoginAndTempEmail: json['canViewAccountLoginAndTempEmail'] is bool ? json['canViewAccountLoginAndTempEmail'] as bool : false,
    bumpSettings: json['bumpSettings'] is Map<String, dynamic>
            ? PurchasingCheckResponseItemBumpSettings.fromJson(json['bumpSettings'] as Map<String, dynamic>)
            : PurchasingCheckResponseItemBumpSettings.fromJson(const {}),
    canCheckGuarantee: json['canCheckGuarantee'] is bool ? json['canCheckGuarantee'] as bool : false,
    needToRequireVideoToViewLoginData: json['needToRequireVideoToViewLoginData'] is bool ? json['needToRequireVideoToViewLoginData'] as bool : false,
    descriptionHtml: json['descriptionHtml'] is String ? json['descriptionHtml'] as String : '',
    descriptionEnHtml: json['descriptionEnHtml'] is String ? json['descriptionEnHtml'] as String : '',
    descriptionPlain: json['descriptionPlain'] is String ? json['descriptionPlain'] as String : '',
    descriptionEnPlain: json['descriptionEnPlain'] is String ? json['descriptionEnPlain'] as String : '',
    seller: json['seller'] is Map<String, dynamic>
            ? PurchasingCheckResponseItemSeller.fromJson(json['seller'] as Map<String, dynamic>)
            : PurchasingCheckResponseItemSeller.fromJson(const {}),
  );
}

class PurchasingCheckResponse {
  final String status;
  final PurchasingCheckResponseItem item;
  final bool requireVideoRecording;
  final RespSystemInfo systemInfo;

  const PurchasingCheckResponse({
    required this.status,
    required this.item,
    required this.requireVideoRecording,
    required this.systemInfo,
  });

  factory PurchasingCheckResponse.fromJson(Map<String, dynamic> json) => PurchasingCheckResponse(
    status: json['status'] is String ? json['status'] as String : '',
    item: json['item'] is Map<String, dynamic>
            ? PurchasingCheckResponseItem.fromJson(json['item'] as Map<String, dynamic>)
            : PurchasingCheckResponseItem.fromJson(const {}),
    requireVideoRecording: json['requireVideoRecording'] is bool ? json['requireVideoRecording'] as bool : false,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PurchasingConfirmBody {
  /// Current price of account in your currency.
  final num? price;
  /// Balance ID that will be used to purchase specified item.
  final num? balanceId;

  const PurchasingConfirmBody({
    this.price,
    this.balanceId,
  });

  Map<String, Object?> toMap() {
    return {
      'price': price,
      'balance_id': balanceId,
    };
  }
}

class PurchasingConfirmResponseItemLoginData {
  final String raw;
  final String encodedRaw;
  final String login;
  final String password;
  final String encodedPassword;
  final String oldPassword;
  final String encodedOldPassword;
  final bool adviceToChangePassword;

  const PurchasingConfirmResponseItemLoginData({
    required this.raw,
    required this.encodedRaw,
    required this.login,
    required this.password,
    required this.encodedPassword,
    required this.oldPassword,
    required this.encodedOldPassword,
    required this.adviceToChangePassword,
  });

  factory PurchasingConfirmResponseItemLoginData.fromJson(Map<String, dynamic> json) => PurchasingConfirmResponseItemLoginData(
    raw: json['raw'] is String ? json['raw'] as String : '',
    encodedRaw: json['encodedRaw'] is String ? json['encodedRaw'] as String : '',
    login: json['login'] is String ? json['login'] as String : '',
    password: json['password'] is String ? json['password'] as String : '',
    encodedPassword: json['encodedPassword'] is String ? json['encodedPassword'] as String : '',
    oldPassword: json['oldPassword'] is String ? json['oldPassword'] as String : '',
    encodedOldPassword: json['encodedOldPassword'] is String ? json['encodedOldPassword'] as String : '',
    adviceToChangePassword: json['adviceToChangePassword'] is bool ? json['adviceToChangePassword'] as bool : false,
  );
}

class PurchasingConfirmResponseItem {
  final PurchasingConfirmResponseItemLoginData loginData;

  const PurchasingConfirmResponseItem({
    required this.loginData,
  });

  factory PurchasingConfirmResponseItem.fromJson(Map<String, dynamic> json) => PurchasingConfirmResponseItem(
    loginData: json['loginData'] is Map<String, dynamic>
            ? PurchasingConfirmResponseItemLoginData.fromJson(json['loginData'] as Map<String, dynamic>)
            : PurchasingConfirmResponseItemLoginData.fromJson(const {}),
  );
}

class PurchasingConfirmResponse {
  final String? status;
  final PurchasingConfirmResponseItem item;
  final RespSystemInfo systemInfo;

  const PurchasingConfirmResponse({
    this.status,
    required this.item,
    required this.systemInfo,
  });

  factory PurchasingConfirmResponse.fromJson(Map<String, dynamic> json) => PurchasingConfirmResponse(
    status: json['status'] is String ? json['status'] as String : null,
    item: json['item'] is Map<String, dynamic>
            ? PurchasingConfirmResponseItem.fromJson(json['item'] as Map<String, dynamic>)
            : PurchasingConfirmResponseItem.fromJson(const {}),
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : RespSystemInfo.fromJson(const {}),
  );
}

class PurchasingDiscountRequestBody {
  /// Requested discounted price.
  final num discountPrice;
  /// Message to the seller.
  final String? message;

  const PurchasingDiscountRequestBody({
    required this.discountPrice,
    this.message,
  });

  Map<String, Object?> toMap() {
    return {
      'discount_price': discountPrice,
      'message': message,
    };
  }
}

class PurchasingDiscountRequestResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const PurchasingDiscountRequestResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory PurchasingDiscountRequestResponse.fromJson(Map<String, dynamic> json) => PurchasingDiscountRequestResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

class PurchasingDiscountCancelResponse {
  final String? status;
  final String? message;
  final RespSystemInfo? systemInfo;

  const PurchasingDiscountCancelResponse({
    this.status,
    this.message,
    this.systemInfo,
  });

  factory PurchasingDiscountCancelResponse.fromJson(Map<String, dynamic> json) => PurchasingDiscountCancelResponse(
    status: json['status'] is String ? json['status'] as String : null,
    message: json['message'] is String ? json['message'] as String : null,
    systemInfo: json['system_info'] is Map<String, dynamic>
            ? RespSystemInfo.fromJson(json['system_info'] as Map<String, dynamic>)
            : null,
  );
}

